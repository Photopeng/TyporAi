...
import { findClaudeCLIPath as findClaudeCLIPathWithProbe } from '@/providers/claude/cli/findClaudeCLIPath';
...
const fileProbe = {
  ...
  isFile: (filePath: string) => fs.statSync(filePath).isFile(),
  readText: (filePath: string) => fs.readFileSync(filePath, 'utf8'),
  list: (filePath: string) => fs.readdirSync(filePath, { withFileTypes: true })
    .map(entry => ({ name: entry.name, isFile: entry.isFile() })),
};
const findClaudeCLIPath = (pathValue?: string): string | null =>
  findClaudeCLIPathWithProbe(pathValue, {
    fileProbe,
...
describe('utils.ts', () => {
  ...
  describe('expandHomePath', () => {
    const envKey = 'TYPORAI_TEST_PATH';
    const envValue = path.join(os.tmpdir(), 'typorai-env');
    ...
    it('should leave unknown environment variables untouched', () => {
      expect(expandHomePath('%TYPORAI_MISSING_VAR%')).toBe('%TYPORAI_MISSING_VAR%');
      expect(expandHomePath('$TYPORAI_MISSING_VAR')).toBe('$TYPORAI_MISSING_VAR');
    });
  });
  ...
  describe('normalizePathForFilesystem', () => {
    ...
    it('expands environment variables before filesystem use', () => {
      const envKey = 'TYPORAI_FS_TEST_PATH';
      const originalValue = process.env[envKey];
      process.env[envKey] = '/tmp/typorai-test';

      try {
        expect(normalizePathForFilesystem(`$${envKey}/notes/file.md`)).toBe(path.normalize('/tmp/typorai-test/notes/file.md'));
      } finally {
        if (originalValue === undefined) {
          delete process.env[envKey];
        } else {
    ...
    it('handles chained home and environment variable expansions', () => {
      const envKey = 'TYPORAI_TEST_SUBDIR';
      const originalValue = process.env[envKey];
  ...
  describe('findClaudeCLIPath', () => {
    ...
    describe('on Windows', () => {
      const joinWin = (...parts: string[]): string => parts.join('\\');
      ...
      it('should prefer .exe when both .exe and cli-wrapper.cjs exist', () => {
        ...
        const exePath = joinWin('C:\\Users\\test', '.claude', 'local', 'claude.exe');
        const cliWrapperPath = joinWin('C:\\Users\\test', 'AppData', 'Roaming', 'npm', 'node_modules', '@anthropic-ai', 'claude-code', 'cli-wrapper.cjs');
      ...
      it('should prioritize cli-wrapper.cjs over .cmd files on Windows', () => {
        jest.spyOn(os, 'homedir').mockReturnValue('C:\\Users\\test');
        const cliWrapperPath = joinWin('C:\\Users\\test', 'AppData', 'Roaming', 'npm', 'node_modules', '@anthropic-ai', 'claude-code', 'cli-wrapper.cjs');
        const cmdPath = joinWin('C:\\Users\\test', 'AppData', 'Roaming', 'npm', 'claude.cmd');
      ...
      it('should fall back to .exe if package entrypoint is not found', () => {
        jest.spyOn(os, 'homedir').mockReturnValue('C:\\Users\\test');
        const expectedPath = joinWin('C:\\Users\\test', '.claude', 'local', 'claude.exe');
        mockExistingFile(expectedPath);
      ...
      it('should ignore .cmd fallback on Windows', () => {
        jest.spyOn(os, 'homedir').mockReturnValue('C:\\Users\\test');
        const expectedPath = joinWin('C:\\Users\\test', 'AppData', 'Roaming', 'npm', 'claude.cmd');
        mockExistingFile(expectedPath);
      ...
      it('should resolve cli-wrapper.cjs from custom PATH npm prefix', () => {
        ...
        const cliWrapperPath = joinWin(npmBin, 'node_modules', '@anthropic-ai', 'claude-code', 'cli-wrapper.cjs');
      ...
      it('should not return a directory path even if it exists', () => {
        jest.spyOn(os, 'homedir').mockReturnValue('C:\\Users\\test');
        const dirPath = joinWin('C:\\Users\\test', '.claude', 'local', 'claude');
