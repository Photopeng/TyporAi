...
describe('validateAgentName', () => {
  ...
  it('returns error for name exceeding max length', () => {
    ...
    expect(validateAgentName(longName)).toBe('tooLong');
  });
  ...
  it('returns error for uppercase letters', () => {
    expect(validateAgentName('CodeReviewer')).toBe('invalidChars');
  });

  it('returns error for spaces', () => {
    expect(validateAgentName('code reviewer')).toBe('invalidChars');
  });

  it('returns error for underscores', () => {
    expect(validateAgentName('code_reviewer')).toBe('invalidChars');
  });
  ...
  it.each(['true', 'false', 'null', 'yes', 'no', 'on', 'off'])(
    ...
    (word) => {
      expect(validateAgentName(word)).toBe('reservedWord');
    }
  );
});
