import type { TyporaFile,TyporaHostApp } from '@/typora/platform';
import { replaceImageEmbedsWithHtml } from '@/utils/imageEmbed';

type LinkResolver = (
  linkPath: string,
  sourcePath: string,
  files: Map<string, TyporaFile>
) => TyporaFile | null | undefined;

// Mock TyporaHostApp and vault for testing
function createMockApp(files: Map<string, string> = new Map(), resolveLink?: LinkResolver): TyporaHostApp {
  const mockFiles = new Map<string, TyporaFile>();
  ...
  return {
    vault: {
      ...
      getResourcePath: (file: TyporaFile) => files.get(file.path) || `app://local/${file.path}`,
    },
    metadataCache: {
...
describe('replaceImageEmbedsWithHtml', () => {
  describe('basic image embeds', () => {
    it('replaces simple image embed with img tag', () => {
      ...
      expect(result).toContain('class="typorai-embedded-image"');
    });
  ...
  describe('file not found (fallback)', () => {
    it('shows fallback when image file not found', () => {
      ...
      expect(result).toContain('class="typorai-embedded-image-fallback"');
  ...
  describe('error handling', () => {
    ...
    it('returns unchanged markdown when vault is missing', () => {
      const app = { metadataCache: {} } as unknown as TyporaHostApp;
      const result = replaceImageEmbedsWithHtml('![[image.png]]', app);
    ...
    it('returns unchanged markdown when metadataCache is missing', () => {
      const app = { vault: {} } as unknown as TyporaHostApp;
      const result = replaceImageEmbedsWithHtml('![[image.png]]', app);
    ...
    it('returns fallback when getResourcePath throws', () => {
      const mockFile = { path: 'test.png', basename: 'test' } as TyporaFile;
      const app = {
        vault: {
