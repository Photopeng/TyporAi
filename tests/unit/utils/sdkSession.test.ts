...
import {
  ...
  deleteSDKSession as deleteSDKSessionWithProbe,
  encodeVaultPathForSDK,
  filterActiveBranch,
  getSDKProjectsPath,
  getSDKSessionPath,
  isValidSessionId,
  loadSDKSessionMessages as loadSDKSessionMessagesWithProbe,
  loadSubagentFinalResult as loadSubagentFinalResultWithProbe,
  loadSubagentToolCalls as loadSubagentToolCallsWithProbe,
  parseSDKMessageToChat,
  readSDKSession as readSDKSessionWithProbe,
  resolveToolUseResultStatus,
  type SDKNativeMessage,
  sdkSessionExists as sdkSessionExistsWithProbe,
...
const historyFileProbe = {
  exists: (target: string) => mockExistsSync(target),
  isFile: () => true,
  list: () => [],
  readText: (target: string) => mockFsPromises.readFile(target, 'utf-8') as Promise<string>,
  remove: (target: string) => mockFsPromises.unlink(target),
};

const sdkSessionExists = (vaultPath: string, sessionId: string) =>
  sdkSessionExistsWithProbe(vaultPath, sessionId, historyFileProbe);
const deleteSDKSession = (vaultPath: string, sessionId: string) =>
  deleteSDKSessionWithProbe(vaultPath, sessionId, historyFileProbe);
const readSDKSession = (vaultPath: string, sessionId: string) =>
  readSDKSessionWithProbe(vaultPath, sessionId, historyFileProbe);
const loadSubagentToolCalls = (vaultPath: string, sessionId: string, agentId: string) =>
  loadSubagentToolCallsWithProbe(vaultPath, sessionId, agentId, historyFileProbe);
const loadSubagentFinalResult = (vaultPath: string, sessionId: string, agentId: string) =>
  loadSubagentFinalResultWithProbe(vaultPath, sessionId, agentId, historyFileProbe);
const loadSDKSessionMessages = (vaultPath: string, sessionId: string, resumeAtMessageId?: string) =>
  loadSDKSessionMessagesWithProbe(vaultPath, sessionId, resumeAtMessageId, historyFileProbe);
...
describe('sdkSession', () => {
  ...
  describe('encodeVaultPathForSDK', () => {
    ...
    it('handles paths with spaces and special characters', () => {
      const encoded = encodeVaultPathForSDK("/Users/test/My TyporaWorkspace's~Data");
      expect(encoded).toBe('-Users-test-My-TyporaWorkspace-s-Data');
    });
