...
describe('getVaultPath', () => {
  ...
  it('preserves empty and platform-specific base paths', () => {
    ...
    expect(getVaultPath({ vault: { adapter: { basePath: '/Users/test/My Obsidian TyporaWorkspace' } } } as any)).toBe(
      '/Users/test/My Obsidian TyporaWorkspace',
    );
...
describe('parsePathEntries', () => {
  ...
  it('splits on platform separator', () => {
    ...
    expect(result).toEqual(isWindows ? ['A:', 'B:', 'C:'] : ['/a', '/b', '/c']);
  });
...
describe('findClaudeCLIPath', () => {
  ...
  it('falls back to PATH environment when common and npm paths fail', () => {
    ...
    process.env.PATH = `${envDir}${path.delimiter}${originalPath}`;
