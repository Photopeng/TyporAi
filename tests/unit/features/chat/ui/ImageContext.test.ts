...
const mockNoticeShow = jest.fn();

jest.mock('@/ui/NoticeAdapter', () => ({
  NoticeAdapter: jest.fn().mockImplementation(() => ({ show: mockNoticeShow })),
}));
...
function createContainerWithInputWrapper(): { container: any; inputWrapper: any } {
  ...
  const inputWrapper = container.createDiv({ cls: 'typorai-input-wrapper' });
...
describe('ImageContextManager', () => {
  ...
  describe('constructor with previewContainerEl', () => {
    it('should use previewContainerEl when provided', () => {
      ...
      const previewEl = previewContainer.querySelector('.typorai-image-preview');
      expect(previewEl).not.toBeNull();
    });

    it('should insert image preview before file indicator if present', () => {
      const previewContainer = createMockEl();
      const fileIndicator = previewContainer.createDiv({ cls: 'typorai-file-indicator' });
      ...
      const previewIdx = children.findIndex((el: any) => el.hasClass?.('typorai-image-preview'));
...
describe('ImageContextManager - Private Helpers', () => {
  ...
  describe('notifyImageError', () => {
    it('should create a Notice with the message', () => {
      ...
      expect(mockNoticeShow).toHaveBeenCalledWith('Test error', 'error');
    });

    it('should append file not found for ENOENT error', () => {
      const error = new Error('ENOENT: no such file or directory');
      manager['notifyImageError']('Failed to load image.', error);
      expect(mockNoticeShow).toHaveBeenCalledWith('Failed to load image. (File not found)', 'error');
    });

    it('should append permission denied for EACCES error', () => {
      const error = new Error('EACCES: permission denied');
      manager['notifyImageError']('Failed to load image.', error);
      expect(mockNoticeShow).toHaveBeenCalledWith('Failed to load image. (Permission denied)', 'error');
    });
  ...
  describe('addImageFromFile', () => {
    it('should reject files exceeding size limit', async () => {
      ...
      expect(mockNoticeShow).toHaveBeenCalledWith(expect.stringContaining('limit'), 'error');
    });
    ...
    it('should reject files with unsupported media type', async () => {
      ...
      expect(result).toBe(false);
      expect(mockNoticeShow).toHaveBeenCalledWith('Unsupported image type.', 'error');
    });
    ...
    it('should handle arrayBuffer failure gracefully', async () => {
      ...
      const result = await manager['addImageFromFile'](file, 'drop');
      expect(result).toBe(false);
      expect(mockNoticeShow).toHaveBeenCalledWith('Failed to attach image.', 'error');
    });
  ...
  describe('Image preview rendering', () => {
    ...
    it('renderImagePreview should create chip with thumbnail, info, and remove button', () => {
      ...
      expect(chipEl.hasClass('typorai-image-chip')).toBe(true);

      const thumbEl = chipEl.querySelector('.typorai-image-thumb');
      expect(thumbEl).not.toBeNull();

      const infoEl = chipEl.querySelector('.typorai-image-info');
      expect(infoEl).not.toBeNull();

      const removeEl = chipEl.querySelector('.typorai-image-remove');
    ...
    it('remove button should delete the image and update preview', () => {
      ...
      const removeEl = firstChip.querySelector('.typorai-image-remove');
