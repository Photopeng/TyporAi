...
describe('ModelSelector', () => {
  ...
  it('should create a container with model-selector class', () => {
    const container = parentEl.querySelector('.typorai-model-selector');
    expect(container).not.toBeNull();
  });

  it('should display current model label', () => {
    // Default model is 'sonnet' which maps to 'Sonnet'
    const btn = parentEl.querySelector('.typorai-model-btn');
    expect(btn).not.toBeNull();
    const label = btn?.querySelector('.typorai-model-label');
  ...
  it('should display first model when current model not found', () => {
    ...
    const label = parentEl.querySelector('.typorai-model-label');
    expect(label?.textContent).toBe('Haiku');
  });

  it('should render model options in reverse order', () => {
    const dropdown = parentEl.querySelector('.typorai-model-dropdown');
  ...
  it('should render group separators when models have group field', () => {
    ...
    const groups = children.filter((c: any) => c.hasClass('typorai-model-group'));
  ...
  it('should show 1M variants instead of standard variants when enabled', () => {
    ...
    expect(parentEl.querySelector('.typorai-model-label')?.textContent).toBe('Opus 1M');
  });
});
...
describe('ModeSelector', () => {
  ...
  it('should create a container with mode-selector class', () => {
    const container = parentEl.querySelector('.typorai-mode-selector');
    expect(container).not.toBeNull();
  });

  it('should display the current mode label', () => {
    const label = parentEl.querySelector('.typorai-mode-label');
    expect(label?.textContent).toBe('Build');
  });

  it('should call onModeChange when the toggle is clicked', async () => {
    const toggle = parentEl.querySelector('.typorai-toggle-switch');
  ...
  it('should show the active style when the configured active mode is selected', () => {
    ...
    const label = parentEl2.querySelector('.typorai-mode-label');
    const toggle = parentEl2.querySelector('.typorai-toggle-switch');
    expect(label?.textContent).toBe('Build');
...
describe('ThinkingBudgetSelector', () => {
  ...
  describe('adaptive mode (Claude models)', () => {
    ...
    it('should create a container with thinking-selector class', () => {
      const container = parentEl.querySelector('.typorai-thinking-selector');
      expect(container).not.toBeNull();
    });

    it('should show effort selector for Claude models', () => {
      const effort = parentEl.querySelector('.typorai-thinking-effort');
      expect(effort).not.toBeNull();
      expect(effort?.style?.display).not.toBe('none');
    });

    it('should hide budget selector for Claude models', () => {
      const budget = parentEl.querySelector('.typorai-thinking-budget');
      expect(budget?.style?.display).toBe('none');
    });

    it('should display current effort level for Claude models', () => {
      const current = parentEl.querySelector('.typorai-thinking-current');
  ...
  describe('legacy mode (custom models)', () => {
    ...
    it('should render budget options in reverse order', () => {
      const options = parentEl.querySelector('.typorai-thinking-options');
    ...
    it('should not expose token counts in reasoning option tooltips', () => {
      const options = parentEl.querySelector('.typorai-thinking-options');
      const gears = options?.children || [];
      const highGear = gears.find((g: any) => g.textContent === 'High');
      expect(highGear?.getAttribute('title')).toBeNull();
    });

    it('should not leave a token-related tooltip on the off option', () => {
      const options = parentEl.querySelector('.typorai-thinking-options');
      const gears = options?.children || [];
      const offGear = gears.find((g: any) => g.textContent === 'Off');
      expect(offGear?.getAttribute('title')).toBeNull();
    });
  });
});
...
describe('PermissionToggle', () => {
  ...
  it('should create a container with permission-toggle class', () => {
    const container = parentEl.querySelector('.typorai-permission-toggle');
    expect(container).not.toBeNull();
  });

  it('should display SAFE with the document-edit switch off in normal mode', () => {
    const label = parentEl.querySelector('.typorai-permission-label');
    const toggle = parentEl.querySelector('.typorai-toggle-switch');
  ...
  it('should display YOLO label when in yolo mode', () => {
    ...
    const label = parentEl2.querySelector('.typorai-permission-label');
  ...
  it('should show the document-edit switch as on when in yolo mode', () => {
    callbacks.getSettings.mockReturnValue({
      model: 'sonnet',
      thinkingBudget: 'low',
      serviceTier: 'default',
  ...
  it('should not have active class in normal SAFE mode', () => {
    const toggle = parentEl.querySelector('.typorai-toggle-switch');
    expect(toggle?.hasClass('active')).toBe(false);
  });
  ...
  it('should hide the control when provider exposes no permission toggle UI', () => {
    ...
    const container = parentEl2.querySelector('.typorai-permission-toggle');
    expect(container?.style.display).toBe('none');
  });
...
describe('ServiceTierToggle', () => {
  ...
  it('shows the control when the provider exposes service tier options', () => {
    ...
    expect(container).not.toBeNull();
    expect(container?.hasClass('typorai-hidden')).toBe(false);
  });

  it('renders the icon button in the inactive state when fast mode is off', () => {
    const button = parentEl.querySelector('.typorai-service-tier-button');
    const icon = parentEl.querySelector('.typorai-service-tier-icon');
    const container = parentEl.querySelector('.typorai-service-tier-toggle');
...
describe('McpServerSelector', () => {
  ...
  it('should create container with mcp-selector class', () => {
    const container = parentEl.querySelector('.typorai-mcp-selector');
    expect(container).not.toBeNull();
  });
  ...
  it('should show empty message when all servers are disabled', () => {
    ...
    const empty = parentEl.querySelector('.typorai-mcp-selector-empty');
  ...
  it('should show badge when more than 1 server enabled', () => {
    ...
    const badge = parentEl.querySelector('.typorai-mcp-selector-badge');
  ...
  it('should add active class to icon when servers are enabled', () => {
    ...
    const icon = parentEl.querySelector('.typorai-mcp-selector-icon');
...
describe('McpServerSelector - toggle and badges', () => {
  ...
  it('should render context-saving badge for servers with contextSaving', () => {
    ...
    const csBadge = parentEl.querySelector('.typorai-mcp-selector-cs-badge');
  ...
  it('should toggle server on mousedown and update display', () => {
    ...
    const item = parentEl.querySelector('.typorai-mcp-selector-item');
...
describe('createInputToolbar', () => {
  it('should return all toolbar components', () => {
    ...
    expect(parentEl.querySelector('.typorai-context-meter')).toBeNull();
  ...
  it('should place the mode selector after the permission toggle in toolbar order', () => {
    ...
    const permissionIndex = parentEl.children.findIndex((child: any) => child.hasClass('typorai-permission-toggle'));
    const modeIndex = parentEl.children.findIndex((child: any) => child.hasClass('typorai-mode-selector'));
...
describe('CursorFlowToggle localization', () => {
  afterEach(() => {
    setLocale('en');
  });

  it('refreshes every visible string when the locale changes', () => {
    const parentEl = createMockEl();
    setLocale('ja');
    const toggle = new CursorFlowToggle(parentEl);
