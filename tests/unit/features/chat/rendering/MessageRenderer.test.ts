...
jest.mock('@/ui/NoticeAdapter', () => ({
  NoticeAdapter: class {
    show(message: string): void { mockNotice(message); }
  },
}));

const MockContextMenu = (jest.requireMock('@/ui/ContextMenu') as {
  ContextMenu: { instances: unknown[] };
}).ContextMenu as any;
...
describe('MessageRenderer', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    MockContextMenu.instances.length = 0;
  });
  ...
  it('renders welcome element and calls renderStoredMessage for each message', () => {
    ...
    expect(welcomeEl.hasClass('typorai-welcome')).toBe(true);
  ...
  it('renders interrupt messages with interrupt styling instead of user bubble', () => {
    ...
    expect(msgEl.hasClass('typorai-message-assistant')).toBe(true);
    ...
    expect(interruptedEl.hasClass('typorai-interrupted')).toBe(true);
  ...
  it('renders bare interrupt marker for empty interrupted assistant message', () => {
    ...
    const contentEl = msgEl.children[0];
    const textEl = contentEl.children[0];
    expect(textEl.children[0].hasClass('typorai-interrupted')).toBe(true);
  });
  ...
  it('renders user message with text content', () => {
    ...
    expect(messagesEl.children.length).toBe(1);
    const msgEl = messagesEl.children[0];
    expect(msgEl.hasClass('typorai-message-user')).toBe(true);
  });
  ...
  it('skips empty user message bubble (image-only)', () => {
    ...
    const bubbles = messagesEl.children.filter(
      (c: any) => c.hasClass('typorai-message')
    );
  ...
  it('renders user message with images above bubble', () => {
    ...
    const groupEl = messagesEl.children[0];
    expect(groupEl.hasClass('typorai-message-user-stack')).toBe(true);
  });
  ...
  it('adds a rewind button for eligible stored user messages', () => {
    ...
    expect(messagesEl.querySelector('.typorai-message-rewind-btn')).not.toBeNull();
  });
  ...
  it('adds rewind but not fork for a completed first user message', () => {
    ...
    expect(messagesEl.querySelector('.typorai-message-rewind-btn')).not.toBeNull();
    expect(messagesEl.querySelector('.typorai-message-fork-btn')).toBeNull();
  ...
  it('does not add a rewind button when stored render is called without context', () => {
    ...
    expect(messagesEl.querySelector('.typorai-message-rewind-btn')).toBeNull();
  });
  ...
  it('shows rewind mode menu for eligible streamed user messages', async () => {
    ...
    const btn = messagesEl.querySelector('.typorai-message-rewind-btn');
    expect(btn).not.toBeNull();

    btn!.click();
    const menu = MockContextMenu.instances[0];
  ...
  it('renders response duration footer when durationSeconds is present', () => {
    ...
    const msgEl = messagesEl.children[0];
    const contentEl = msgEl.children[0]; // typorai-message-content
    const footerEl = contentEl.children.find((c: any) => c.hasClass('typorai-response-footer'));
  ...
  it('renderMessageImages creates image elements', () => {
    ...
    expect(imagesContainer.hasClass('typorai-message-images')).toBe(true);
  ...
  it('showFullImage creates overlay with image', () => {
    ...
    try {
      ...
      expect(mockBody.createDiv).toHaveBeenCalledWith({ cls: 'typorai-image-modal-overlay' });
    } finally {
  ...
  it('addTextCopyButton adds a copy button element', () => {
    ...
    expect(copyBtn.hasClass('typorai-text-copy-btn')).toBe(true);
  });
  ...
  it('renderContent escapes math delimiters only when requested for streaming', async () => {
      const { renderMarkdown } = await import('@/ui/MarkdownRenderer');
  ...
  it('renderMessages should store table-of-contents title from displayContent before content', () => {
    ...
    const msgEl = messagesEl.querySelector('.typorai-message-user');
  ...
  describe('showFullImage - close behaviors', () => {
    ...
    it('closeBtn click removes overlay', () => {
      ...
      try {
        ...
        const modalEl = overlayEl.children[0]; // typorai-image-modal
        // Children: img (index 0), closeBtn (index 1)
        const closeBtn = modalEl.children[1];
        expect(closeBtn.hasClass('typorai-image-modal-close')).toBe(true);
  ...
  describe('renderContent - error handling', () => {
    it('renderContent shows error div when MarkdownRenderer throws', async () => {
      ...
      const errorDiv = el.children.find(
        (c: any) => c.hasClass('typorai-render-error')
      );
  ...
  describe('renderContent - code block wrapping', () => {
    ...
    it('should wrap pre elements in code wrapper divs', async () => {
      ...
      // The pre should be wrapped in a typorai-code-wrapper
    ...
    it('should skip wrapping already-wrapped pre elements', async () => {
      ...
      (MarkdownRenderer.renderMarkdown as jest.Mock).mockImplementationOnce(
        async (_md: string, container: any) => {
          const wrapper = container.createDiv({ cls: 'typorai-code-wrapper' });
  ...
  describe('renderMessageSelection', () => {
    it('renders a selection row above user message bubble in renderStoredMessage', () => {
      const messagesEl = createMockEl();
      const { renderer } = createRenderer(messagesEl);
      jest.spyOn(renderer, 'renderContent').mockResolvedValue(undefined);
      ...
      // Expect: selection row and bubble grouped into one user-message stack.
      expect(messagesEl.children.length).toBe(1);
      const groupEl = messagesEl.children[0];
      expect(groupEl.hasClass('typorai-message-user-stack')).toBe(true);
      expect(groupEl.children.length).toBe(2);
      const selEl = groupEl.children[0];
      expect(selEl.hasClass('typorai-message-selection')).toBe(true);
      const bubbleEl = groupEl.children[1];
      expect(bubbleEl.hasClass('typorai-message-user')).toBe(true);
    });

    it('omits selection row when msg.selection is not set', () => {
      const messagesEl = createMockEl();
      const { renderer } = createRenderer(messagesEl);
      jest.spyOn(renderer, 'renderContent').mockResolvedValue(undefined);
      ...
      expect(messagesEl.children.length).toBe(1);
      expect(messagesEl.children[0].hasClass('typorai-message-user')).toBe(true);
    });

    it('omits selection row for assistant messages even if selection is set', () => {
      const messagesEl = createMockEl();
      const { renderer } = createRenderer(messagesEl);
      jest.spyOn(renderer, 'renderContent').mockResolvedValue(undefined);
      ...
      const msg: ChatMessage = {
        ...
        selection: {
          ...
          notePath: 'notes/bar.md',
        },
      };

      renderer.renderStoredMessage(msg);

      expect(messagesEl.querySelector('.typorai-message-selection')).toBeNull();
    });

    it('renders selection row from addMessage for streaming', () => {
      const messagesEl = createMockEl();
      const { renderer } = createRenderer(messagesEl);
      jest.spyOn(renderer, 'renderContent').mockResolvedValue(undefined);
      ...
      expect(messagesEl.querySelector('.typorai-message-selection')).not.toBeNull();
    });

    it('shows truncated selection text as the visible line, with no label or meta', () => {
      const { renderer } = createRenderer();
      const selEl = renderer.renderMessageSelection(createMockEl(), {
        text: 'multi\nline\nselection',
      ...
      expect(selEl.hasClass('typorai-message-selection')).toBe(true);
      const headerEl = selEl.querySelector('.typorai-message-selection-header');
      expect(headerEl).not.toBeNull();
      expect(headerEl!.textContent).toBe('multi line selection');
      expect(selEl.querySelector('.typorai-message-selection-label')).toBeNull();
      expect(selEl.querySelector('.typorai-message-selection-meta')).toBeNull();
    });

    it('truncates long selection text with an ellipsis', () => {
      const { renderer } = createRenderer();
      ...
      const headerEl = selEl.querySelector('.typorai-message-selection-header');
      expect(headerEl!.textContent).toBe(`${'x'.repeat(96)}...`);
    });

    it('is collapsed by default and expands to show full text on click', () => {
      const { renderer } = createRenderer();
      const selEl = renderer.renderMessageSelection(createMockEl(), {
        text: 'full text body',
        notePath: 'notes/x.md',
      ...
      const headerEl = selEl.querySelector('.typorai-message-selection-header') as any;
      const contentEl = selEl.querySelector('.typorai-message-selection-content') as any;
      expect(contentEl.hasClass('typorai-hidden')).toBe(true);

      headerEl.click();
      expect(contentEl.hasClass('typorai-hidden')).toBe(false);
      expect(selEl.hasClass('expanded')).toBe(true);
      const preEl = contentEl.querySelector('.typorai-message-selection-pre');
      expect(preEl.textContent).toBe('full text body');

      headerEl.click();
      expect(contentEl.hasClass('typorai-hidden')).toBe(true);
      expect(selEl.hasClass('expanded')).toBe(false);
    });
  });
});
