...
describe('ToolCallRenderer', () => {
  ...
  describe('renderToolCall', () => {
    ...
    it('should start expanded when requested', () => {
      ...
      const header = toolEl.querySelector('.typorai-tool-header');
      const content = toolEl.querySelector('.typorai-tool-content');

      expect(toolCall.isExpanded).toBe(true);
      expect(toolEl.hasClass('expanded')).toBe(true);
      expect(content?.hasClass('typorai-hidden')).toBe(false);
  ...
  describe('renderStoredToolCall', () => {
    ...
    it('renders AskUserQuestion answers from result text when resolvedAnswers is missing', () => {
      ...
      const answerEls = toolEl.querySelectorAll('.typorai-ask-review-a-text');
    ...
    it('renders AskUserQuestion options during fallback state', () => {
      ...
      const toolEl = renderStoredToolCall(parentEl, toolCall);
      const labelEls = toolEl.querySelectorAll('.typorai-ask-item-label');
      const descEls = toolEl.querySelectorAll('.typorai-ask-item-desc');
      const checkEls = toolEl.querySelectorAll('.typorai-ask-check');
  ...
  describe('updateToolCallResult', () => {
    it('should update status indicator', () => {
      ...
      const statusEl = toolEl.querySelector('.typorai-tool-status');
    ...
    it('shows raw AskUserQuestion result when answers cannot be parsed', () => {
      ...
      const promptEl = toolEl.querySelector('.typorai-ask-review-prompt');
  ...
  describe('getToolLabel', () => {
    ...
    it('should label WebSearch open_page actions with the URL', () => {
      ...
      })).toBe('WebSearch: Open page: https://example.com/docs');
    });
  ...
  describe('WebSearch expanded rendering', () => {
    it('renders Codex search actions instead of the placeholder result text', () => {
      ...
      const toolEl = renderStoredToolCall(parentEl, toolCall);
      const lines = Array.from(toolEl.querySelectorAll('.typorai-tool-line')).map(line => line.textContent);
    ...
    it('renders Codex open_page actions from tool input even without a rich result body', () => {
      ...
      const toolEl = renderStoredToolCall(parentEl, toolCall);
      const links = toolEl.querySelectorAll('.typorai-tool-link');
      const lines = Array.from(toolEl.querySelectorAll('.typorai-tool-line')).map(line => line.textContent);

      expect(lines).toContain('Open page');
      expect(links).toHaveLength(1);
      expect(links[0].getAttribute('href')).toBe('https://example.com/docs');
      expect(links[0].querySelector('.typorai-tool-link-title')?.textContent).toBe('https://example.com/docs');
    });
  });
  ...
  describe('apply_patch expanded rendering', () => {
    it('renders parsed patch diffs from tool input', () => {
      ...
      const toolCall = createToolCall({
        ...
        input: {
          patch: [
            ...
            '@@',
            "-import { TyporaPluginRuntime } from 'obsidian';",
            "+import { TyporaPluginRuntime, Notice } from 'obsidian';",
            '*** End Patch',
          ].join('\n'),
        },
      ...
      const toolEl = renderStoredToolCall(parentEl, toolCall);
      const headers = Array.from(toolEl.querySelectorAll('.typorai-tool-patch-header')).map(el => el.textContent);
      const statusEl = toolEl.querySelector('.typorai-tool-status');
      const diffTexts = Array.from(toolEl.querySelectorAll('.typorai-diff-text')).map(el => el.textContent);

      expect(headers).toHaveLength(0);
      expect(statusEl?.hasClass('typorai-write-edit-stats')).toBe(true);
      ...
      expect(diffTexts).toContain("import { TyporaPluginRuntime } from 'obsidian';");
      expect(diffTexts).toContain("import { TyporaPluginRuntime, Notice } from 'obsidian';");
    });
    ...
    it('renders stored apply_patch collapsed by default', () => {
      ...
      expect(content?.hasClass('typorai-hidden')).toBe(true);
    ...
    it('keeps the error status icon for failed apply_patch calls', () => {
      ...
      expect(statusEl?.hasClass('typorai-write-edit-stats')).toBe(false);
      expect(setIcon).toHaveBeenCalledWith(expect.anything(), 'x');
    });
    ...
    it('falls back to file changes when patch text is unavailable', () => {
      ...
      const toolEl = renderStoredToolCall(parentEl, toolCall);
      const lines = Array.from(toolEl.querySelectorAll('.typorai-tool-line')).map(el => el.textContent);
  ...
  describe('renderTodoWriteResult', () => {
    it('should render todo items', () => {
      ...
      expect(container.hasClass('typorai-todo-panel-content')).toBe(true);
      expect(container.hasClass('typorai-todo-list-container')).toBe(true);
    });
  ...
  describe('updateToolCallResult for TodoWrite', () => {
    it('should update todo status and content', () => {
      ...
      const statusEl = parentEl.querySelector('.typorai-tool-status');
      expect(statusEl?.hasClass('status-completed')).toBe(true);
    });
