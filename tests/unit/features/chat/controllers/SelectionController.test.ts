...
jest.mock('@/i18n/i18n', () => ({
  t: (key: string, params?: Record<string, string | number>) => {
    if (key === 'chat.selection.quotePreview' && params) {
      return `${params.text} [${params.lineCount} lines]`;
    }
    return key;
  },
}));
...
function createMockIndicator() {
  ...
  indicator.addClass('typorai-selection-indicator');
  indicator.addClass('typorai-hidden');
  const previewTextEl = createMockEl();
  previewTextEl.addClass('typorai-selection-preview-text');
  indicator.appendChild(previewTextEl);
  return indicator;
}
...
function createMockContextRow() {
  const elements: Record<string, any> = {
    '.typorai-selection-indicator': createMockIndicator(),
    '.typorai-canvas-indicator': createMockEl(),
    '.typorai-file-indicator': null,
    '.typorai-image-preview': null,
  };
  elements['.typorai-canvas-indicator'].addClass('typorai-canvas-indicator');
  elements['.typorai-canvas-indicator'].addClass('typorai-hidden');
...
describe('SelectionController', () => {
  ...
  beforeEach(() => {
    ...
    app = {
      workspace: {
        getActiveDocumentView: jest.fn().mockReturnValue(view),
      },
    };

    controller = new SelectionController(app, indicatorEl, inputEl, contextRowEl, undefined, focusScopeEl, jest.fn(), undefined);
  ...
  it('captures selection and updates indicator', () => {
    ...
    expect(indicatorEl.querySelector('.typorai-selection-preview-text').textContent).toBe('selected text');
    expect(indicatorEl.hasClass('typorai-selection-preview-has-selection')).toBe(true);
  ...
  it('clears selection immediately when deselected without input handoff intent', () => {
    ...
    expect(indicatorEl.hasClass('typorai-selection-preview-has-selection')).toBe(false);
    expect(hideSelectionHighlight).toHaveBeenCalledWith(editorView);
  });

  it('clears the preview text after clearAfterSend is called', () => {
    controller.start();
    jest.advanceTimersByTime(250);
    expect(indicatorEl.hasClass('typorai-selection-preview-has-selection')).toBe(true);
    ...
    expect(controller.hasSelection()).toBe(false);
    expect(indicatorEl.hasClass('typorai-selection-preview-has-selection')).toBe(false);
    expect(indicatorEl.querySelector('.typorai-selection-preview-text').textContent).toBe('');
  });
  ...
  it('preserves selection when focus moves into the chat sidebar', () => {
    ...
    app.workspace.getActiveDocumentView.mockReturnValue(null);
    const sidebarButton = {};
  ...
  describe('Reading mode (preview)', () => {
    ...
    beforeEach(() => {
      ...
      app.workspace.getActiveDocumentView.mockReturnValue(readingView);
    });
    ...
    it('captures selection via document.getSelection() in reading mode', () => {
      ...
      expect(indicatorEl.querySelector('.typorai-selection-preview-text').textContent).toBe('reading selection');
      expect(indicatorEl.hasClass('typorai-selection-preview-has-selection')).toBe(true);
    });
    ...
    it('preserves raw reading mode text and omits line metadata', () => {
      ...
      expect(indicatorEl.querySelector('.typorai-selection-preview-text').textContent).toBe('reading selection second line');
    });
    ...
    it('clears CSS highlight when reading mode selection is deselected', () => {
      ...
      expect(mockHighlights.delete).toHaveBeenCalledWith('typorai-selection');
    });
    ...
    it('replaces source selection metadata when switching the same text into preview mode', () => {
      ...
      app.workspace.getActiveDocumentView.mockReturnValue(sourceView);
  ...
  it('keeps context row visible when canvas selection indicator is visible', () => {
    ...
    canvasIndicator.addClass('typorai-canvas-indicator');
    contextRowEl.querySelector.mockImplementation((selector: string) => {
      if (selector === '.typorai-canvas-indicator') return canvasIndicator;
      return null;
    });
  ...
  it('captures quick action selections into the quote preview context', () => {
    controller.captureQuickActionSelection(' quick action text ');

    expect(controller.getContext()).toEqual({
      notePath: 'Typora selection',
      mode: 'selection',
      selectedText: 'quick action text',
      lineCount: 1,
    });
    expect(indicatorEl.querySelector('.typorai-selection-preview-text').textContent).toBe('quick action text');
    expect(indicatorEl.hasClass('typorai-selection-preview-has-selection')).toBe(true);
  });

  it('updates standalone quote preview text without removing the clear button', () => {
    const standaloneIndicator = createMockIndicator();
    const previewTextEl = createMockEl();
    const clearButtonEl = createMockEl();
    standaloneIndicator.querySelector = jest.fn((selector: string) => {
      if (selector === '.typorai-selection-preview-text') return previewTextEl;
      if (selector === '.typorai-selection-preview-clear') return clearButtonEl;
      return null;
    });
    controller = new SelectionController(app, standaloneIndicator, inputEl, contextRowEl, undefined, focusScopeEl, jest.fn(), undefined);

    controller.captureQuickActionSelection('quoted text');

    expect(previewTextEl.textContent).toBe('quoted text');
    expect(clearButtonEl.textContent).toBe('');
    expect(standaloneIndicator.hasClass('typorai-selection-preview-has-selection')).toBe(true);
  });
});
