...
class MockContextMenu {
  static instances: MockContextMenu[] = [];
  items: Array<{ title: string }> = [];
  showAtMouseEvent = jest.fn();
  hide = jest.fn();

  constructor() { MockContextMenu.instances.push(this); }

  addItem(callback: (item: { setTitle(value: string): unknown; setIcon(): unknown; setDisabled(): unknown; onClick(): unknown }) => void): this {
    const item = {
      title: '',
...
jest.mock('@/ui/ContextMenu', () => ({ ContextMenu: MockContextMenu }));
...
describe('ConversationController', () => {
  ...
  describe('loadActive with existing conversation', () => {
    it('should restore persisted external context paths into the runtime', async () => {
      const agentService = {
        syncConversationState: jest.fn(),
      };
      deps = createMockDeps({
      ...
      controller = new ConversationController(deps);
      deps.state.currentConversationId = 'conv-with-context';
      const conversation = {
        id: 'conv-with-context',
        messages: [],
        sessionId: null,
        externalContextPaths: ['C:/workspace/project', 'C:/workspace/README.md'],
      };
      (deps.plugin.getConversationById as jest.Mock).mockResolvedValue(conversation);

      await controller.loadActive();

      expect(agentService.syncConversationState).toHaveBeenCalledWith(
        conversation,
  ...
  describe('History Rendering', () => {
    ...
    describe('updateHistoryDropdown with conversations', () => {
      it('should render conversation items when conversations exist', () => {
        ...
        expect(list.hasClass('typorai-history-list')).toBe(true);
      ...
      it('should show "No conversations" when list is empty', () => {
        ...
        const list = dropdown.children[1];
        expect(list.children[0].hasClass('typorai-history-empty')).toBe(true);
      });

      it('should localize the history dropdown header', () => {
        setLocale('zh-CN');
        (deps.plugin.getConversationList as jest.Mock).mockReturnValue([]);

        controller.updateHistoryDropdown();

        expect(dropdown.children[0].children[0].textContent).toBe('\u5bf9\u8bdd');
        setLocale('en');
      });
      ...
      it('should sort conversations by lastResponseAt descending', () => {
        ...
        const list = dropdown.children[1];
        const firstTitle = list.children[0].querySelector('.typorai-history-item-title');
      ...
      it('should show loading indicator for pending title generation', () => {
        ...
        const loadingEl = item.querySelector('.typorai-action-loading');
      ...
      it('should show regenerate button for failed title generation', () => {
        ...
        const list = dropdown.children[1];
        const item = list.children[0];
        const actions = item.querySelector('.typorai-history-item-actions');
        expect(actions).toBeTruthy();
        // regenerate button + rename button + delete button = 3 children
        expect(actions!.children.length).toBe(3);
      });

      it('should make the whole current history row actionable', () => {
        deps.state.currentConversationId = 'conv-1';
        ...
        expect(item._eventListeners?.get('click')).toHaveLength(1);
      });
      ...
      it('should not delete while streaming', async () => {
        ...
        const list = dropdown.children[1];
        const item = list.children[0];
        const deleteBtn = item.querySelector('.typorai-delete-btn');
    ...
    describe('renderHistoryDropdown', () => {
      ...
      it('should highlight conversations already open in a tab', () => {
        ...
        const openItemDate = openItem.querySelector('.typorai-history-item-date');
      ...
      it('should display the current tab number when available', () => {
        ...
        const currentItemDate = currentItem.querySelector('.typorai-history-item-date');
      ...
      it('should display running status for a conversation open in another tab', () => {
        ...
        const runningItemDate = runningItem.querySelector('.typorai-history-item-date');
      ...
      it('should display another-pane status without a local tab number', () => {
        ...
        const runningOtherPaneDate = runningOtherPaneItem.querySelector('.typorai-history-item-date');
        const openOtherPaneDate = openOtherPaneItem.querySelector('.typorai-history-item-date');
      ...
      it('should render a new-tab button for closed conversations', async () => {
        ...
        const openInNewTabBtn = closedItem.querySelector('.typorai-open-new-tab-btn');
      ...
      it('should not render a new-tab button for already-open conversations', () => {
        ...
        expect(openItem.querySelector('.typorai-open-new-tab-btn')).toBeNull();
      });
  ...
  describe('History Item Interactions', () => {
    ...
    it('should invoke rename handler when clicking rename button', () => {
      ...
      const titleEl = item.querySelector('.typorai-history-item-title');
      if (titleEl) {
    ...
    it('should delete non-current conversation without calling loadActive', async () => {
      ...
      const deleteBtn = otherItem.querySelector('.typorai-delete-btn');
      const clickHandlers = deleteBtn!._eventListeners?.get('click');
  ...
  describe('Greeting Time Branches', () => {
    it.each([
      ...
      const hasTimeBased = patterns.some(p => greeting.includes(p));
