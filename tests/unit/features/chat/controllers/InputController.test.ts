...
describe('InputController - Message Queue', () => {
  ...
  describe('Queue indicator UI', () => {
    it('should show queue indicator when message is queued', () => {
      ...
      expect(queueIndicatorEl.querySelector('.typorai-queue-indicator-text')?.textContent).toBe('⌙ Queued: test message');
    ...
    it('should withdraw queued message to the composer for editing', () => {
      ...
      const editButton = queueIndicatorEl
        .querySelectorAll('.typorai-queue-indicator-icon-action')
    ...
    it('should truncate long message preview in indicator', () => {
      ...
      const queueIndicatorEl = deps.state.queueIndicatorEl as any;
      const text = queueIndicatorEl.querySelector('.typorai-queue-indicator-text')?.textContent as string;
    ...
    it('should show [images] when queue message has only images', () => {
      ...
      const queueIndicatorEl = deps.state.queueIndicatorEl as any;
      expect(queueIndicatorEl.querySelector('.typorai-queue-indicator-text')?.textContent).toBe('⌙ Queued: [images]');
    });
    ...
    it('should show Codex steer action when queued message can be steered', () => {
      ...
      const queueIndicatorEl = deps.state.queueIndicatorEl as any;
      expect(queueIndicatorEl.querySelector('.typorai-queue-indicator-action')?.textContent).toBe('Steer Now');
    });
    ...
    it('should steer the queued Codex message when the action is clicked', async () => {
      ...
      const queueIndicatorEl = deps.state.queueIndicatorEl as any;
      queueIndicatorEl.querySelector('.typorai-queue-indicator-action')?.click();
      await Promise.resolve();
      await Promise.resolve();
      ...
      expect(deps.state.queuedMessage).toBeNull();
      expect(queueIndicatorEl.querySelector('.typorai-queue-indicator-text')?.textContent)
        .toBe('⌙ Steering... queued follow-up');
      expect(queueIndicatorEl.querySelector('.typorai-queue-indicator-action')).toBeNull();
      expect(queueIndicatorEl.style.display).toBe('flex');
  ...
  describe('Built-in commands - /resume', () => {
    ...
    it('should reject /resume when the provider lacks native history support', async () => {
      ...
      expect(mockNotice).toHaveBeenCalledWith('/resume is not supported by the current provider.');
  ...
  describe('ensureServiceInitialized failure', () => {
    ...
    it('should leave the draft untouched when ensureServiceInitialized returns false', async () => {
      deps = createSendableDeps({
      ...
      expect(deps.streamController.showThinkingIndicator).not.toHaveBeenCalled();
      expect(deps.streamController.hideThinkingIndicator).not.toHaveBeenCalled();
      expect(deps.state.isStreaming).toBe(false);
  ...
  describe('handleApprovalRequest', () => {
    ...
    it('should throw when input container has no parent', async () => {
      ...
      await expect(controller.handleApprovalRequest('bash', {}, 'test'))
        .rejects.toThrow('The input area is unavailable.');
    });
    ...
    it.each([
      ...
      const items = parentEl.querySelectorAll('typorai-ask-item');
      const target = items.find((item: any) => {
        const label = item.querySelector('typorai-ask-item-label');
    ...
    it('should render header metadata when approvalOptions provided', async () => {
      ...
      const reasonEl = parentEl.querySelector('typorai-ask-approval-reason');
      expect(reasonEl?.textContent).toBe('Command is destructive');

      const pathEl = parentEl.querySelector('typorai-ask-approval-blocked-path');
      expect(pathEl?.textContent).toBe('/usr/bin/rm');

      const agentEl = parentEl.querySelector('typorai-ask-approval-agent');
    ...
    it('should render provider-supplied approval options and network-specific context', async () => {
      ...
      const descEl = parentEl.querySelector('typorai-ask-approval-desc');
      expect(descEl?.textContent).toContain('api.openai.com');

      const items = parentEl.querySelectorAll('typorai-ask-item');
      const labels = items
        .map((item: any) => item.querySelector('typorai-ask-item-label')?.textContent)
        .filter(Boolean);
