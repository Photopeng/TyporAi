...
const mockOnProviderAvailabilityChanged = jest.fn();
...
jest.mock('@/core/providers/ProviderRegistry', () => ({
  ProviderRegistry: {
    ...
    hasProvider: (providerId: string) => ['claude', 'codex', 'opencode', 'typora'].includes(providerId),
    isEnabled: (providerId: string, settings: Record<string, any>) =>
      settings?.providerConfigs?.[providerId]?.enabled !== false,
    resolveSettingsProviderId: (settings: Record<string, any>) => settings.settingsProvider ?? 'claude',
...
beforeEach(() => {
  ...
  mockOnProviderAvailabilityChanged.mockImplementation(() => undefined);
...
describe('TabManager - Tab Lifecycle', () => {
  ...
  describe('createTab', () => {
    ...
    it('uses the settings provider for a new blank tab created from a bound conversation', async () => {
      const plugin = createMockPlugin({
        settings: { settingsProvider: 'codex' },
        getConversationById: jest.fn().mockResolvedValue({ id: 'conv-api', providerId: 'typora' }),
      });
      const manager = createManager({
        ...
        tabFactory: (n) => createMockTabData({
          id: `tab-${n}`,
          lifecycleState: n === 1 ? 'bound_cold' : 'blank',
          conversationId: n === 1 ? 'conv-api' : null,
          providerId: n === 1 ? 'typora' : 'codex',
        }),
      });

      await manager.createTab('conv-api');
      await manager.createTab();

      expect(mockCreateTab.mock.calls[1][0]).toEqual(expect.objectContaining({
    ...
    it('retargets blank tabs to a selected provider model', async () => {
      mockGetProviderSettingsSnapshot.mockReturnValue({ model: DEFAULT_CODEX_PRIMARY_MODEL });
      const onTabProviderChanged = jest.fn();
      const manager = createManager({
        callbacks: {
          ...callbacks,
        ...
        tabFactory: (n) => createMockTabData({
          id: `tab-${n}`,
          lifecycleState: n === 1 ? 'blank' : 'bound_cold',
          conversationId: n === 1 ? null : 'conv-api',
          draftModel: n === 1 ? 'deepseek-v4-flash' : null,
          providerId: 'typora',
        }),
      });

      const blankTab = await manager.createTab();
      const boundTab = await manager.createTab('conv-api', undefined, { activate: false });

      const changed = manager.retargetBlankTabsToProvider('codex');

      expect(changed).toBe(1);
      expect(blankTab?.providerId).toBe('codex');
      expect(blankTab?.draftModel).toBe(DEFAULT_CODEX_PRIMARY_MODEL);
      expect(boundTab?.providerId).toBe('typora');
      expect(mockOnProviderAvailabilityChanged).toHaveBeenCalledWith(blankTab, expect.anything());
      expect(onTabProviderChanged).toHaveBeenCalledWith(blankTab?.id, 'codex');
    });
    ...
    it('keeps non-active restored pre-session OpenCode tabs cold until the final active tab is chosen', async () => {
      ...
      const manager = createManager({
        ...
        tabFactory: (n) => createMockTabData({
          ...
          ui: {
            cursorFlowToggle: {
              // placeholder for potential cursor flow toggle methods
            },
          },
        }),
      });
