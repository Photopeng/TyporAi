...
jest.mock('@/providers/claude/runtime/ClaudeChatRuntime', () => ({
  TyporAiService: jest.fn().mockImplementation(() => ({
...
const createMockTyporAiService = (overrides?: {
...
function createMockOptions(overrides: Partial<TestTabCreateOptions> = {}): TestTabCreateOptions {
  ...
  ProviderWorkspaceRegistry.setServices('claude', {
    ...
    cliResolver: {
      reset: jest.fn(),
      resolveFromSettings: jest.fn(() => 'claude'),
    },
  } as any);
  ProviderWorkspaceRegistry.setServices('codex', {
    ...
    cliResolver: {
      reset: jest.fn(),
      resolveFromSettings: jest.fn(() => 'codex'),
    },
  } as any);
...
describe('Tab - Creation', () => {
  describe('createTab', () => {
    ...
    it('should fall back when a restored conversation uses a removed provider', () => {
      const options = createMockOptions({
        conversation: {
          id: 'conv-pi',
          providerId: 'pi',
          title: 'Legacy Pi Conversation',
          messages: [],
          sessionId: null,
          createdAt: Date.now(),
          updatedAt: Date.now(),
      ...
      expect(tab.conversationId).toBe('conv-pi');
...
describe('Tab - Service Initialization', () => {
  ...
  describe('initializeTabService', () => {
    it('should not reinitialize if already initialized', async () => {
      ...
      tab.service = createMockTyporAiService() as any;
    ...
    it('should create TyporAiService on first initialization', async () => {
      const options = createMockOptions();
      const tab = createTab(options);
    ...
    it('should reject initialization when the selected provider is disabled', async () => {
      const plugin = createMockPlugin();
      plugin.settings.providerConfigs = {
        ...plugin.settings.providerConfigs,
      ...
      await expect(initializeTabService(tab, plugin, createMockMcpManager()))
        .rejects
        .toThrow('Claude is disabled or not configured. Enable and configure it in Settings before chatting.');
      expect(tab.serviceInitialized).toBe(false);
      expect(tab.service).toBeNull();
    });
    ...
    it('should create the runtime for the conversation provider', async () => {
      ...
      const mockRuntime = createMockTyporAiService({ providerId: 'codex' });
    ...
    it('should recreate the runtime when the conversation provider changes', async () => {
      const createChatRuntimeSpy = jest.spyOn(ProviderRegistry, 'createChatRuntime');
      const oldService = createMockTyporAiService({ providerId: 'claude' });
      const newService = createMockTyporAiService({ providerId: 'codex' });
    ...
    it('should NOT call ensureReady for blank tabs (lazy start)', async () => {
      ...
      const runtimeModule = jest.requireMock('@/providers/claude/runtime/ClaudeChatRuntime') as { TyporAiService: jest.Mock };
      runtimeModule.TyporAiService.mockImplementationOnce(() => createMockTyporAiService({ ensureReady: mockEnsureReady }));
    ...
    it('should sync existing conversations with saved external contexts', async () => {
      ...
      const runtimeModule = jest.requireMock('@/providers/claude/runtime/ClaudeChatRuntime') as { TyporAiService: jest.Mock };
      runtimeModule.TyporAiService.mockImplementationOnce(() => createMockTyporAiService({
    ...
    it('falls back blank Codex draft to Claude when Codex is disabled', () => {
      ...
      const staleService = createMockTyporAiService({ providerId: 'codex' });
    ...
    it('drops restored Typora API draft model when Typora provider is disabled', () => {
      const plugin = createMockPlugin({
        settings: {
    ...
    it('keeps API quick-action helper services on Typora when a newly enabled provider takes over the draft model', () => {
      ...
      expect(tab.providerId).toBe('codex');
      expect(createInstructionRefineServiceSpy).toHaveBeenLastCalledWith(plugin, 'typora');
    ...
    it('maps shared permission mode selections onto managed OpenCode modes', async () => {
      const plugin = createMockPlugin({
        settings: {
          ...
          providerConfigs: {
            opencode: {
              availableModes: [
                { id: 'typorai-yolo', name: 'YOLO' },
                { id: 'typorai-safe', name: 'Safe' },
                { id: 'plan', name: 'Plan' },
              ],
              enabled: true,
              selectedMode: 'typorai-yolo',
            },
          },
          savedProviderEffort: {
            claude: 'high',
      ...
      expect(plugin.settings.providerConfigs.opencode.selectedMode).toBe('typorai-safe');
...
describe('Tab - Destruction', () => {
  describe('destroyTab', () => {
    ...
    it('should unsubscribe from ready state changes when tab is destroyed', async () => {
      ...
      const runtimeModule = jest.requireMock('@/providers/claude/runtime/ClaudeChatRuntime') as { TyporAiService: jest.Mock };
      runtimeModule.TyporAiService.mockImplementationOnce(() => createMockTyporAiService({ onReadyStateChange: mockOnReadyStateChange }));
  ...
  describe('setupServiceCallbacks', () => {
    function setupAutoTurnTest() {
      ...
      const addMessage = jest.fn(() => {
        ...
        msgEl.createDiv({ cls: 'typorai-message-content' });
        return msgEl;
      });
...
describe('Tab - UI Initialization', () => {
  ...
  describe('initializeTabUI', () => {
    ...
    it('should create selection indicator element', () => {
      ...
      expect(tab.dom.selectionIndicatorEl!.hasClass('typorai-selection-preview')).toBe(true);
    });
    ...
    it('should create input toolbar components', () => {
      ...
      expect(tab.ui.cursorFlowToggle).toBeNull();
...
describe('Tab - Controller Initialization', () => {
  ...
  describe('initializeTabControllers', () => {
    ...
    describe('cursor flow', () => {
      function getOnNewSelectionCallback(): (text: string, source: 'document' | 'ai-response') => void {
        const call = (SelectionController as unknown as jest.Mock).mock.calls[0];
        return call[6];
      }
      ...
      it('does not focus the input immediately on selection (debounced)', () => {
        jest.useFakeTimers();
        const options = createMockOptions();
        options.plugin.settings.cursorFlowEnabled = true;
        const tab = createTab(options);
        initializeTabUI(tab, options.plugin);
        initializeTabControllers(tab, options.plugin, {} as any, options.mcpManager);
        const focusSpy = jest.spyOn(tab.dom.inputEl, 'focus');

        getOnNewSelectionCallback()('hello world', 'document');

        expect(focusSpy).not.toHaveBeenCalled();
      });

      it('focuses the input after the debounce delay once selection is stable', () => {
        jest.useFakeTimers();
        const options = createMockOptions();
        options.plugin.settings.cursorFlowEnabled = true;
        const tab = createTab(options);
      ...
      it('restarts the debounce timer on every selection change during a drag', () => {
        jest.useFakeTimers();
        const options = createMockOptions();
        options.plugin.settings.cursorFlowEnabled = true;
        const tab = createTab(options);
        ...
        initializeTabControllers(tab, options.plugin, {} as any, options.mcpManager);
        const focusSpy = jest.spyOn(tab.dom.inputEl, 'focus');
        const onNewSelection = getOnNewSelectionCallback();

        // Simulate a growing drag selection: each poll tick (250ms) reports
        // a larger selection before the debounce window elapses.
        onNewSelection('hello', 'document');
        jest.advanceTimersByTime(250);
        onNewSelection('hello world', 'document');
        jest.advanceTimersByTime(250);
        onNewSelection('hello world again', 'document');

        // Only 250ms since the last change 鈥?still under the 400ms delay.
        jest.advanceTimersByTime(250);
        expect(focusSpy).not.toHaveBeenCalled();

        // Selection has now been stable for the full debounce window.
        jest.advanceTimersByTime(150);
        expect(focusSpy).toHaveBeenCalledTimes(1);
      });

      it('does not focus the input when cursor flow is disabled', () => {
        jest.useFakeTimers();
        const options = createMockOptions();
        options.plugin.settings.cursorFlowEnabled = false;
        const tab = createTab(options);
        initializeTabUI(tab, options.plugin);
        initializeTabControllers(tab, options.plugin, {} as any, options.mcpManager);
        const focusSpy = jest.spyOn(tab.dom.inputEl, 'focus');
      ...
      it('does not focus the input for AI-response selections', () => {
        jest.useFakeTimers();
        const options = createMockOptions();
        options.plugin.settings.cursorFlowEnabled = true;
        const tab = createTab(options);
        ...
        getOnNewSelectionCallback()('hello world', 'ai-response');
        jest.advanceTimersByTime(1000);

        expect(focusSpy).not.toHaveBeenCalled();
      });

      it('does not focus the input if it is already focused', () => {
        jest.useFakeTimers();
        const options = createMockOptions();
        options.plugin.settings.cursorFlowEnabled = true;
        const tab = createTab(options);
        initializeTabUI(tab, options.plugin);
        initializeTabControllers(tab, options.plugin, {} as any, options.mcpManager);
        // The mock ownerDocument's `activeElement` getter proxies to the real
        // global `document`, which this suite's `node` test environment does
        // not provide by default 鈥?stub the minimum needed for this case.
        const previousDocument = (globalThis as any).document;
        (globalThis as any).document = { activeElement: tab.dom.inputEl };
        const focusSpy = jest.spyOn(tab.dom.inputEl, 'focus');
      ...
      it('clears the pending focus timer on teardown', () => {
        jest.useFakeTimers();
        const options = createMockOptions();
        options.plugin.settings.cursorFlowEnabled = true;
        const tab = createTab(options);
...
describe('Tab - Event Handler Behavior', () => {
  ...
  describe('wireTabInputEvents - keydown handlers', () => {
    ...
    it('should dispatch streaming cancel through the runtime command when registered', async () => {
      mockInstructionModeManager.handleTriggerKey.mockReturnValue(false);
      mockInstructionModeManager.handleKeydown.mockReturnValue(false);
      mockSlashCommandDropdown.handleKeydown.mockReturnValue(false);
      mockFileContextManager.handleMentionKeydown.mockReturnValue(false);
      ...
      const cancelCommand = jest.fn();
      registry.register('conversation.cancel', 'Cancel', cancelCommand);
      tab.commandRegistry = registry;
    ...
    it('should dispatch send through the runtime command when registered', async () => {
      mockInstructionModeManager.handleTriggerKey.mockReturnValue(false);
      mockInstructionModeManager.handleKeydown.mockReturnValue(false);
      mockSlashCommandDropdown.handleKeydown.mockReturnValue(false);
      mockFileContextManager.handleMentionKeydown.mockReturnValue(false);
      ...
      registry.register('conversation.send', 'Send', sendCommand);
      tab.commandRegistry = registry;
    ...
    it('should ignore keydown events already handled by runtime hotkeys', () => {
      const { fireKeydown } = setupKeydownTab();

      fireKeydown({ key: 'Enter', defaultPrevented: true, preventDefault: jest.fn() });
...
describe('Tab - Service Initialization Error Handling', () => {
  ...
  it('should skip re-initialization if already initialized', async () => {
    ...
    tab.serviceInitialized = true;
    const originalService = createMockTyporAiService() as any;
...
describe('Tab - Blank Tab Model Selector', () => {
  ...
  it('returns Typora API models by default', () => {
    const typoraModels = [
      { value: 'deepseek-chat', label: 'deepseek-chat' },
    ];
    jest.spyOn(ProviderRegistry, 'getEnabledProviderIds').mockReturnValue(['typora']);
    jest.spyOn(ProviderRegistry, 'getProviderDisplayName').mockImplementation((providerId) => (
      providerId === 'typora' ? 'Typora' : 'Codex'
    ));
    jest.spyOn(ProviderRegistry, 'getChatUIConfig').mockImplementation((providerId?: string) => ({
      getModelOptions: () => providerId === 'typora' ? typoraModels : [],
      getProviderIcon: jest.fn().mockReturnValue(null),
    } as any));

    const result = getBlankTabModelOptions({});
    expect(result).toEqual(typoraModels.map(m => ({ ...m, group: 'Typora' })));
  });

  it('returns Typora + configured CLI models when a CLI provider is enabled and configured', () => {
    const typoraModels = [
      { value: 'deepseek-chat', label: 'deepseek-chat' },
    ];
    const codexModels = [
    ...
    jest.spyOn(ProviderRegistry, 'getEnabledProviderIds').mockReturnValue(['codex', 'typora']);
    jest.spyOn(ProviderRegistry, 'getCapabilities').mockImplementation((providerId: any) => ({
      providerId,
      supportsPersistentRuntime: providerId !== 'typora',
    } as any));
    jest.spyOn(ProviderRegistry, 'getProviderDisplayName').mockImplementation((providerId) => (
      providerId === 'codex' ? 'Codex' : 'Typora'
    ));
    jest.spyOn(ProviderRegistry, 'getChatUIConfig').mockImplementation((providerId?: string) => ({
      getModelOptions: () => providerId === 'codex' ? codexModels : typoraModels,
      getProviderIcon: jest.fn().mockReturnValue(null),
    } as any));
    ProviderWorkspaceRegistry.setServices('codex', {
      cliResolver: {
    ...
    expect(result).toEqual([
      ...
      ...typoraModels.map(m => ({ ...m, group: 'Typora' })),
    ]);
  });
});
...
describe('Tab - Cross-Provider Model Rejection', () => {
  ...
  it('allows same-provider model change on bound tab', async () => {
    ...
    jest.spyOn(ProviderRegistry, 'getChatUIConfig').mockReturnValue({
      getModelOptions: jest.fn().mockReturnValue([]),
      ownsModel: jest.fn((model: string) => model === 'opus' || model.startsWith('gpt-') || /^o\d/.test(model)),
      isAdaptiveReasoningModel: jest.fn().mockReturnValue(false),
      getReasoningOptions: jest.fn().mockReturnValue([]),
      getDefaultReasoningValue: jest.fn().mockReturnValue('off'),
      getContextWindowSize: jest.fn().mockReturnValue(200000),
...
describe('Tab - Blank Tab Draft Model Change', () => {
  ...
  it('swaps dropdown provider catalog on blank tab model change', async () => {
    ...
    // Switch to Codex model 鈫?should swap catalog
    await toolbarCallbacks.onModelChange(DEFAULT_CODEX_PRIMARY_MODEL);
...
describe('Tab - First Send Binding', () => {
  it('derives provider from draft model on first send (Claude)', async () => {
    ...
    runtimeModule.TyporAiService.mockImplementationOnce(() => createMockTyporAiService({ ensureReady: mockEnsureReady }));
    const createChatRuntimeSpy = jest.spyOn(ProviderRegistry, 'createChatRuntime')
      .mockReturnValue(createMockTyporAiService() as any);
  ...
  it('derives provider from draft model on first send (Codex)', async () => {
    const createChatRuntimeSpy = jest.spyOn(ProviderRegistry, 'createChatRuntime')
      .mockReturnValue(createMockTyporAiService({ providerId: 'codex' }) as any);
