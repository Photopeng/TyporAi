...
function createMockCallbacks(overrides: Partial<MentionDropdownCallbacks> = {}): MentionDropdownCallbacks {
  ...
  return {
    ...
    getCachedWorkspaceFolders: jest.fn().mockReturnValue([]),
    getCachedWorkspaceFiles: jest.fn().mockReturnValue([]),
    normalizeWorkspacePath: jest.fn((path: string | undefined | null) => path ?? null),
    ...overrides,
  };
}
  ...
  describe('result limiting', () => {
    it('limits vault file results to 100 items', () => {
      ...
      const limitedCallbacks = createMockCallbacks({
        getCachedWorkspaceFiles: jest.fn().mockReturnValue(largeFileSet),
      });
  ...
  describe('vault folder mentions', () => {
    it('limits vault folder results to 50 items', () => {
      ...
      const localCallbacks = createMockCallbacks({
        getCachedWorkspaceFolders: jest.fn().mockReturnValue(largeFolderSet),
      });
    ...
    it('renders vault folder text in @path/ format', () => {
      ...
      const nameEl = itemEl.querySelector('.typorai-mention-name-folder');
