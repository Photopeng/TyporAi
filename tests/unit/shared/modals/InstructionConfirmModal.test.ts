...
function clickButton(root: any, text: string): void {
  const buttons = findAllByClass(root, 'typorai-instruction-btn');
...
describe('InstructionModal', () => {
  describe('onOpen', () => {
    it('renders the raw instruction text', () => {
      ...
      const originalEl = findByClass(contentEl, 'typorai-instruction-original');
    ...
    it('starts in loading state', () => {
      ...
      const loadingEl = findByClass(contentEl, 'typorai-instruction-loading');
      expect(loadingEl).not.toBeNull();
      expect(loadingEl.classList.contains('typorai-hidden')).toBe(false);

      const clarificationEl = findByClass(contentEl, 'typorai-instruction-clarification-section');
      expect(clarificationEl.classList.contains('typorai-hidden')).toBe(true);

      const confirmationEl = findByClass(contentEl, 'typorai-instruction-confirmation-section');
      expect(confirmationEl.classList.contains('typorai-hidden')).toBe(true);
    });
    ...
    it('renders Cancel button in loading state', () => {
      ...
      const buttons = findAllByClass(contentEl, 'typorai-instruction-btn');
  ...
  describe('showClarification', () => {
    it('transitions to clarification state', () => {
      ...
      const loadingEl = findByClass(contentEl, 'typorai-instruction-loading');
      expect(loadingEl.classList.contains('typorai-hidden')).toBe(true);

      const clarificationEl = findByClass(contentEl, 'typorai-instruction-clarification-section');
      expect(clarificationEl.classList.contains('typorai-hidden')).toBe(false);
    });
    ...
    it('displays the clarification text', () => {
      ...
      const clarificationTextEl = findByClass(contentEl, 'typorai-instruction-clarification');
  ...
  describe('showConfirmation', () => {
    it('transitions to confirmation state', () => {
      ...
      const confirmationEl = findByClass(contentEl, 'typorai-instruction-confirmation-section');
      expect(confirmationEl.classList.contains('typorai-hidden')).toBe(false);
    });
    ...
    it('displays the refined instruction', () => {
      ...
      const refinedEl = findByClass(contentEl, 'typorai-instruction-refined');
