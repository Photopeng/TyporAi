...
describe('getProviderForModel', () => {
  ...
  it('routes unconfigured extended Claude model ids to the Typora default provider', () => {
    expect(getProviderForModel('claude-sonnet-4-5-20250514')).toBe('typora');
    expect(getProviderForModel('claude-opus-4-6-20250616')).toBe('typora');
  });
  ...
  it('routes unknown models to the Typora default provider', () => {
    expect(getProviderForModel('some-unknown-model')).toBe('typora');
  });
  ...
  it('routes an unqualified custom model to Typora without provider settings', () => {
    expect(getProviderForModel('my-custom-model')).toBe('typora');
  });
  ...
  it('can resolve blank-tab routing within enabled providers only', () => {
    ...
    expect(getEnabledProviderForModel(DEFAULT_CODEX_PRIMARY_MODEL, settings)).toBe('typora');
  });
});
