...
describe('Tab Lifecycle - Model-Driven Provider Routing', () => {
  describe('getProviderForModel', () => {
    ...
    it('defaults unknown models to the Typora API provider', () => {
      expect(getProviderForModel('custom-model')).toBe('typora');
      expect(getProviderForModel('')).toBe('typora');
    });

    it('does not route "obsidian" to codex (no digit after o)', () => {
      expect(getProviderForModel('obsidian')).toBe('typora');
    });
  });
});
