...
function createMockPlugin(settings: Record<string, unknown> = {}) {
  return {
    ...
    app: {
      vault: {
        adapter: {
          basePath: '/tmp/typorai-test-vault',
        },
      },
    },
  } as any;
...
describe('OpencodeAuxQueryRunner', () => {
  ...
  beforeEach(() => {
    ...
    mockPrepareOpencodeLaunchArtifacts.mockResolvedValue({
      configPath: '/tmp/typorai-opencode-aux/config.json',
      configContent: '{"default_agent":"typorai-aux-passive"}\n',
      databasePath: null,
      launchKey: 'launch-key',
      systemPromptPath: '/tmp/typorai-opencode-aux/system.md',
    });
  });
  ...
  it('launches an auxiliary ACP session and streams assistant text', async () => {
    ...
    expect(mockPrepareOpencodeLaunchArtifacts).toHaveBeenCalledWith(expect.objectContaining({
      ...
      defaultAgentId: 'typorai-aux-passive',
      managedAgents: [expect.objectContaining({ id: 'typorai-aux-passive' })],
  ...
  it('passes a host process transport to the auxiliary ACP subprocess', async () => {
    const processTransport: ProcessTransportFactory = { start: jest.fn() };
    const runner = new OpencodeAuxQueryRunner(createMockPlugin(), {
      agentProfile: 'passive',
      artifactPurpose: 'title-gen',
    }, processTransport);

    await runner.query({ systemPrompt: 'Use this custom system prompt.' }, 'Generate a title');
  ...
  it('rejects permission prompts even for the read-only aux profile', async () => {
    ...
    }, undefined, undefined, path.posix);
  ...
  it('rejects aux reads outside the workspace root', () => {
    ...
    (runner as any).sessionCwds.set('session-1', '/tmp/typorai-test-vault');
    ...
    expect((runner as any).resolveSessionPath('session-1', '/tmp/typorai-test-vault/notes/today.md')).toBe(
      '/tmp/typorai-test-vault/notes/today.md',
    );
  });
});
