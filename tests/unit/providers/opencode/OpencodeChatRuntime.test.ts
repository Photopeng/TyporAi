...
describe('OpencodeChatRuntime', () => {
  ...
  it('persists the same Typora-aware prompt that it sends to the runtime', () => {
    const runtime = new OpencodeChatRuntime(createMockPlugin());

    const turn = runtime.prepareTurn({
      text: 'Summarize this document',
      typoraDocument: {
        path: 'C:\\Users\\27758\\Desktop\\demo.md',
        content: '# Demo',
      },
    });

    expect(turn.persistedContent).toBe(turn.prompt);
    expect(turn.persistedContent).toContain('<current_typora_document path="C:\\Users\\27758\\Desktop\\demo.md">');
  });
  ...
  it('marks missing saved sessions invalidated without creating a replacement command session', async () => {
    ...
    jest.spyOn(launchArtifacts, 'prepareOpencodeLaunchArtifacts').mockImplementation(async (params) => {
      ...
      return {
        configPath: '/tmp/typorai-opencode-config.json',
        configContent: '{}\n',
        databasePath: '/persisted/opencode.db',
        launchKey: 'launch-key',
        systemPromptPath: '/tmp/typorai-opencode-system.md',
      };
    });
  ...
  it('falls back to the managed YOLO mode when a saved custom mode is not managed by TyporAi', () => {
    const plugin = createMockPlugin({
      settings: {
        permissionMode: 'yolo',
        providerConfigs: {
        ...
        providerConfigs: {
          opencode: {
            availableModes: [
              ...
              { id: OPENCODE_SAFE_MODE_ID, name: 'typorai-safe' },
              { id: 'plan', name: 'plan' },
            ],
            selectedMode: '',
          },
