...
describe('mapOpencodeMessages', () => {
  it('maps stored OpenCode messages into TyporAi chat messages', () => {
...
describe('loadOpencodeSessionMessages', () => {
  ...
  beforeEach(() => {
    tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'typorai-opencode-history-'));
  });
  ...
  it('loads conversation content without selecting raw message metadata', async () => {
    ...
    try {
      ...
      await expect(loadOpencodeSessionMessages(sessionId, { databasePath: dbPath }, {
        fileProbe: {
