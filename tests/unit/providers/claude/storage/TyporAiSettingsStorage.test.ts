...
import {
  ...
  TyporAiSettingsStorage,
} from '@/providers/claude/storage/TyporAiSettingsStorage';
...
describe('TyporAiSettingsStorage', () => {
  let storage: TyporAiSettingsStorage;
  ...
  beforeEach(() => {
    ...
    storage = new TyporAiSettingsStorage(mockAdapter);
  });
  ...
  describe('load', () => {
    ...
    it('loads legacy .claude settings and migrates them to .typorai', async () => {
      ...
      mockAdapter.read.mockImplementation(async (path: string) => {
        if (path === LEGACY_TYPORAI_SETTINGS_PATH) {
    ...
    it('migrates current legacy hostname-scoped provider settings to the opaque device key', async () => {
      ...
      expect(writtenContent.providerConfigs).not.toHaveProperty('pi');
    });
  ...
  describe('exists', () => {
    it('should return true when the new file exists', async () => {
      mockAdapter.exists.mockImplementation(async (path: string) => (
        path === TYPORAI_SETTINGS_PATH
      ));

      const result = await storage.exists();

      expect(result).toBe(true);
      expect(mockAdapter.exists).toHaveBeenCalledWith(TYPORAI_SETTINGS_PATH);
    });
    ...
    it('should return true when only the legacy file exists', async () => {
      ...
      expect(result).toBe(true);
      expect(mockAdapter.exists).toHaveBeenCalledWith(TYPORAI_SETTINGS_PATH);
      expect(mockAdapter.exists).toHaveBeenCalledWith(LEGACY_TYPORAI_SETTINGS_PATH);
    });
