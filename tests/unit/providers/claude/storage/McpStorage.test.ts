...
type MockAdapter = WorkspaceFileAdapter & { _store: Record<string, string> };

// Mock WorkspaceFileAdapter with minimal implementation for McpStorage tests
...
describe('McpStorage', () => {
  describe('load', () => {
    ...
    it('loads servers with disabledTools from _typorai metadata', async () => {
      const config = {
        mcpServers: {
    ...
    it('applies defaults when no _typorai metadata exists', async () => {
      const config = {
        mcpServers: {
          alpha: { command: 'alpha-cmd' },
        },
    ...
    it('loads description from _typorai metadata', async () => {
      const config = {
  ...
  describe('save', () => {
    it('saves disabledTools to _typorai metadata', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      expect(saved._typorai.servers.alpha.disabledTools).toEqual(['tool_a', 'tool_b']);
    });
    ...
    it('omits disabledTools from metadata when empty', async () => {
      ...
      const saved = JSON.parse(adapter._store['.claude/mcp.json']);
      // No _typorai since all fields are default
      expect(saved._typorai).toBeUndefined();
    });

    it('preserves existing _typorai metadata when saving', async () => {
      const existing = {
        mcpServers: {
          alpha: { command: 'alpha-cmd' },
        },
      ...
      const saved = JSON.parse(adapter._store['.claude/mcp.json']);
      expect(saved._typorai.customField).toBe('should be preserved');
      expect(saved._typorai.servers.alpha.disabledTools).toEqual(['tool_a']);
    });
    ...
    it('saves description to _typorai metadata', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.claude/mcp.json']);
      expect(saved._typorai.servers.alpha.description).toBe('A test server');
    });

    it('stores enabled=false in _typorai when different from default', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.claude/mcp.json']);
      expect(saved._typorai.servers.alpha.enabled).toBe(false);
    });

    it('stores contextSaving=false in _typorai when different from default', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.claude/mcp.json']);
      expect(saved._typorai.servers.alpha.contextSaving).toBe(false);
    });

    it('removes _typorai.servers when all metadata is default', async () => {
      const existing = {
        mcpServers: { alpha: { command: 'cmd' } },
        _typorai: { servers: { alpha: { enabled: false } } },
      };
      const adapter = createMockAdapter({
        '.claude/mcp.json': JSON.stringify(existing),
      });
    ...
    it('preserves non-servers _typorai fields when removing servers', async () => {
      const existing = {
        mcpServers: { alpha: { command: 'cmd' } },
        _typorai: {
          customField: 'keep',
      ...
      const saved = JSON.parse(adapter._store['.claude/mcp.json']);
      expect(saved._typorai).toEqual({ customField: 'keep' });
    });
