...
describe('SessionStorage', () => {
  let mockAdapter: jest.Mocked<WorkspaceFileAdapter>;
  ...
  beforeEach(() => {
    mockAdapter = {
      ...
      delete: jest.fn(),
      listFiles: jest.fn(),
    } as unknown as jest.Mocked<WorkspaceFileAdapter>;

    storage = new SessionStorage(mockAdapter);
  });

  describe('SESSIONS_PATH', () => {
    it('should be .typorai/sessions', () => {
      expect(SESSIONS_PATH).toBe('.typorai/sessions');
    });
  });
  ...
  describe('getMetadataPath', () => {
    it('returns correct file path for session id', () => {
      ...
      expect(path).toBe('.typorai/sessions/session-abc.meta.json');
    });
  });
  ...
  describe('saveMetadata', () => {
    it('serializes metadata to JSON and writes to file', async () => {
      ...
      expect(mockAdapter.write).toHaveBeenCalledWith(
        '.typorai/sessions/session-456.meta.json',
        expect.any(String)
      );
  ...
  describe('listAllConversations - provider routing', () => {
    it('preserves providerId from metadata', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.typorai/sessions/claude-session.meta.json',
        '.typorai/sessions/codex-session.meta.json',
      ]);
    ...
    it('defaults providerId to claude for legacy conversations', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.typorai/sessions/old.meta.json',
      ]);
      ...
      expect(metas[0].providerId).toBe('typora');
    });
  });
  ...
  describe('deleteMetadata', () => {
    it('deletes the meta.json file', async () => {
      ...
      expect(mockAdapter.delete).toHaveBeenCalledWith('.typorai/sessions/session-del.meta.json');
    });
  });
  ...
  describe('listMetadata', () => {
    it('returns metadata for .meta.json files', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.typorai/sessions/native-1.meta.json',
        '.typorai/sessions/native-2.meta.json',
      ]);
    ...
    it('skips files that fail to load', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.typorai/sessions/good.meta.json',
        '.typorai/sessions/bad.meta.json',
      ]);
  ...
  describe('listAllConversations', () => {
    it('returns metadata from listMetadata as ConversationMeta[]', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.typorai/sessions/session-1.meta.json',
        '.typorai/sessions/session-2.meta.json',
      ]);
    ...
    it('preserves titleGenerationStatus', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.typorai/sessions/session-status.meta.json',
      ]);
