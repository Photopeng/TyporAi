...
import type { TyporaPluginRuntime } from '@/typora/platform';
...
function createMockPlugin(options: {
  ...
  return { plugin: plugin as unknown as TyporaPluginRuntime, adapter, files, folders };
}
...
describe('StorageService convenience methods', () => {
  ...
  const typoraiSettingsJson = JSON.stringify({
    userName: 'Test',
    model: 'haiku',
    permissionMode: 'yolo',
  });
  ...
  describe('updateTyporAiSettings', () => {
    it('updates partial typorai settings', async () => {
      const { plugin, files } = createMockPlugin({
        initialFiles: {
          '.typorai/typorai-settings.json': typoraiSettingsJson,
        },
      });
      const storage = new StorageService(plugin);
      await storage.initialize();

      await storage.updateTyporAiSettings({ userName: 'NewUser' });

      const saved = JSON.parse(files.get('.typorai/typorai-settings.json')!) as Record<string, unknown>;
      expect(saved.userName).toBe('NewUser');
    });
  });

  describe('saveTyporAiSettings', () => {
    it('saves full typorai settings', async () => {
      const { plugin, files } = createMockPlugin({
        initialFiles: {
          '.typorai/typorai-settings.json': typoraiSettingsJson,
        },
      ...
      const existing = await storage.loadTyporAiSettings();
      existing.userName = 'FullSave';
      await storage.saveTyporAiSettings(existing);
  ...
  describe('loadTyporAiSettings', () => {
    it('loads typorai settings', async () => {
      const { plugin } = createMockPlugin({
        initialFiles: {
          '.typorai/typorai-settings.json': typoraiSettingsJson,
        },
      ...
      const settings = await storage.loadTyporAiSettings();
      expect(settings.userName).toBe('Test');
      expect(settings.model).toBe('haiku');
    });

    it('migrates legacy settings into .typorai during initialization', async () => {
      const { plugin, files } = createMockPlugin({
        initialFiles: {
          '.claude/typorai-settings.json': typoraiSettingsJson,
        },
      });
      const storage = new StorageService(plugin);
  ...
  describe('getAdapter', () => {
    it('returns the workspace file adapter instance', () => {
      const { plugin } = createMockPlugin({});
      const storage = new StorageService(plugin);
