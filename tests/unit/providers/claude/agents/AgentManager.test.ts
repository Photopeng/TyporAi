...
const originalLoadAgents = AgentManager.prototype.loadAgents;
const nativePath = (filePath: string): string => filePath.replace(/\//g, path.sep);
const fileProbe = {
  exists: (filePath: string) => mockFs.existsSync(nativePath(filePath)),
  isFile: (filePath: string) => mockFs.statSync(nativePath(filePath)).isFile(),
  readText: (filePath: string) => mockFs.readFileSync(nativePath(filePath), 'utf8') as string,
  list: (filePath: string) => (mockFs.readdirSync(nativePath(filePath), { withFileTypes: true }) as fs.Dirent[])
...
describe('AgentManager', () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(AgentManager.prototype, 'loadAgents').mockImplementation(function (this: AgentManager) {
      this.setFileProbe(fileProbe);
      return originalLoadAgents.call(this);
    });
  ...
  describe('loadAgents', () => {
    ...
    it('isolates errors per category so one failure does not block others', async () => {
      ...
      // TyporaWorkspace agents load normally, global dir doesn't exist
      ...
      // TyporaWorkspace agent should still be loaded despite plugin error
  ...
  describe('setBuiltinAgentNames', () => {
    ...
    it('excludes file-loaded agents from built-in list', async () => {
      ...
      // TyporaWorkspace has an agent file matching an init agent name
      mockFs.existsSync.mockReturnValue(true);
      (mockFs.readdirSync as jest.Mock).mockImplementation((dir: string) => {
