...
describe('encodeClaudeTurn', () => {
  ...
  it('should append current Typora document context', () => {
    const request: ChatTurnRequest = {
    ...
    expect(result.persistedContent).toContain('<current_typora_document path="C:\\Users\\27758\\Desktop\\demo.md">');
    expect(result.persistedContent).toContain('# Demo');
    expect(result.persistedContent).toContain('<current_typora_selection>');
    expect(result.persistedContent).toContain('Hello');
  });
