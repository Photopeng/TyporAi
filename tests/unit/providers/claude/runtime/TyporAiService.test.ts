...
import { TyporAiService } from '@/providers/claude/runtime/ClaudeChatRuntime';
...
describe('TyporAiService', () => {
  let mockPlugin: Partial<TyporAiPlugin>;
  let mockMcpManager: MockMcpServerManager;
  let service: TyporAiService;
  ...
  beforeEach(() => {
    ...
    service = new TyporAiService(mockPlugin as TyporAiPlugin, {
      ...
      pathService: {
        join: (...parts: readonly string[]) => parts.join('/').replace(/\/{2,}/g, '/'),
        dirname: (value: string) => value.slice(0, Math.max(0, value.lastIndexOf('/'))) || '.',
        normalize: (value: string) => value.replace(/\\/g, '/'),
        isAbsolute: (value: string) => value.startsWith('/') || /^[A-Za-z]:[\\/]/.test(value),
      },
    } as any);
  });
  ...
  describe('applyDynamicUpdates', () => {
    ...
    it('should not update model when unchanged', async () => {
      await (service as any).applyDynamicUpdates({ model: (service as any).currentConfig.model });
    ...
    it('should restart when leaving SAFE so tool restrictions are rebuilt', async () => {
      ...
      expect((service as any).startPersistentQuery).toHaveBeenCalledTimes(2);
      expect(mockPersistentQuery.setPermissionMode).not.toHaveBeenCalled();
    });

    it('should ignore claudeSafeMode changes while the document gate is SAFE', async () => {
      (mockPlugin as any).settings.permissionMode = 'normal';
    ...
    it('should keep default SDK permissions when claudeSafeMode switches to acceptEdits', async () => {
      (mockPlugin as any).settings.permissionMode = 'normal';
      (mockPlugin as any).settings.claudeSafeMode = 'default';
      (service as any).currentConfig = (service as any).buildPersistentQueryConfig(
        '/mock/vault/path',
    ...
    it('should restart into SAFE defaults when leaving YOLO with auto opt-in enabled', async () => {
      (mockPlugin as any).settings.permissionMode = 'yolo';
      (mockPlugin as any).settings.claudeSafeMode = 'auto';
      (service as any).currentConfig = (service as any).buildPersistentQueryConfig(
        '/mock/vault/path',
