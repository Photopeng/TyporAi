...
function createMockSettings(overrides: Partial<TyporAiSettings> = {}): TyporAiSettings {
  return {
    permissions: [],
    permissionMode: 'yolo',
...
describe('QueryOptionsBuilder', () => {
  ...
  describe('buildPersistentQueryConfig', () => {
    ...
    it('keeps the SDK in default permission mode while the document gate is SAFE', () => {
      const ctx = createMockContext({
  ...
  describe('buildPersistentQueryOptions', () => {
    ...
    it('keeps SAFE mode read-only when claudeSafeMode is "auto"', () => {
      const ctx = {
        ...createMockContext({
          settings: createMockSettings({ permissionMode: 'normal', claudeSafeMode: 'auto' }),
        }),
