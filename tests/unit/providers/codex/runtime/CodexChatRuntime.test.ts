...
describe('CodexChatRuntime', () => {
  ...
  describe('ensureReady - app-server lifecycle', () => {
    it('passes a host-owned process transport to the app-server process', async () => {
      const processTransport: ProcessTransportFactory = { start: jest.fn() };
      runtime.cleanup();
      runtime = new CodexChatRuntime(createMockPlugin(), processTransport);
    ...
    it('sends initialize and initialized', async () => {
      ...
      expect(mockTransportRequest).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          clientInfo: { name: 'typorai', version: '1.0.0' },
        }),
      );
  ...
  describe('query - new thread', () => {
    ...
    it('derives WSL transcript and memories roots from thread paths when initialize omits codexHome', async () => {
      runtime.cleanup();
      runtime = new CodexChatRuntime(createMockPlugin({ permissionMode: 'plan' }));
  ...
  describe('query - permission modes', () => {
    ...
    it('uses a non-promptable read-only sandbox for normal SAFE mode', async () => {
      const plugin = createMockPlugin({ permissionMode: 'normal' });
    ...
    it('sends sandboxPolicy with external context writable roots in plan mode', async () => {
      const planRuntime = new CodexChatRuntime(createMockPlugin({ permissionMode: 'plan' }));
      const turn = createTurn('inspect both locations');
      turn.request.externalContextPaths = ['/external/a', '/external/b'];

      await collectChunks(planRuntime.query(turn));
    ...
    it('fails with a clear error when an external context path cannot be mapped into WSL', async () => {
      ...
      const planRuntime = new CodexChatRuntime(createMockPlugin({ permissionMode: 'plan' }));
      const chunks = await collectChunks(planRuntime.query(turn));
  ...
  describe('query - codexSafeMode read-only', () => {
    ...
    it('reasserts the SAFE read-only policy on already-loaded threads', async () => {
