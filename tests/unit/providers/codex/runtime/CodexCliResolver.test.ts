...
describe('CodexCliResolver', () => {
  ...
  it('auto-detects from the runtime PATH when no configured path is valid', () => {
    mockedExists.mockImplementation((filePath: string) => filePath.replace(/\\/g, '/') === '/custom/bin/codex');
    mockedStat.mockImplementation((filePath: string) => ({
      isFile: () => filePath.replace(/\\/g, '/') === '/custom/bin/codex',
    }));
    ...
    expect(resolved?.replace(/\\/g, '/')).toBe('/custom/bin/codex');
  });
