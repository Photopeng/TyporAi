...
import {
  ...
  parseCodexSessionFile as parseCodexSessionFileWithProbe,
...
const nodeFileProbe = {
  exists: (target: string) => fs.existsSync(target),
  isFile: (target: string) => { try { return fs.statSync(target).isFile(); } catch { return false; } },
  readText: (target: string) => fs.readFileSync(target, 'utf8'),
  list: (target: string) => fs.readdirSync(target, { withFileTypes: true })
    .map(entry => ({ name: entry.name, isFile: entry.isFile() })),
};

function parseCodexSessionFile(filePath: string) {
  return parseCodexSessionFileWithProbe(filePath, nodeFileProbe);
}
