...
function createMockAdapter(files: Record<string, string> = {}): WorkspaceFileAdapter {
  return {
  ...
  } as unknown as WorkspaceFileAdapter;
}

/** Simulates a home-level adapter with separate files. */
function createMockHomeAdapter(files: Record<string, string> = {}): WorkspaceFileAdapter {
...
describe('CodexSkillStorage', () => {
  describe('scanAll', () => {
    it('scans skills from vault .codex/skills', async () => {
      const workspaceAdapter = createMockAdapter({
...
Do codex things`,
      ...
      const storage = new CodexSkillStorage(workspaceAdapter);
...
Home agent task`,
      ...
      const storage = new CodexSkillStorage(workspaceAdapter, homeAdapter);
      const skills = await storage.scanAll();
...
description: TyporaWorkspace version
---
TyporaWorkspace prompt`,
      });
      const homeAdapter = createMockHomeAdapter({
        '.codex/skills/shared/SKILL.md': `---
...
Home prompt`,
      ...
      expect(skills).toHaveLength(1);
      expect(skills[0].provenance).toBe('vault');
      expect(skills[0].description).toBe('TyporaWorkspace version');
    });

    it('returns empty array when no directories exist', async () => {
      const workspaceAdapter = createMockAdapter({});
      (workspaceAdapter.exists as jest.Mock).mockResolvedValue(false);
...
description: TyporaWorkspace skill
---
Task`,
      });
      const homeAdapter = createMockHomeAdapter({
