...
describe('encodeCodexTurn', () => {
  ...
  it('should include current note context', () => {
    ...
    expect(result.prompt).toContain('notes/todo.md');
    expect(result.persistedContent).toBe(result.prompt);
  });
  ...
  it('should include current Typora document context', () => {
    const request: ChatTurnRequest = {
      text: 'Summarize this',
      typoraDocument: {
    ...
    expect(result.prompt).toContain('<current_typora_document path="C:\\Users\\27758\\Desktop\\demo.md">');
    expect(result.prompt).toContain('# Demo');
    expect(result.prompt).toContain('<current_typora_selection>');
  ...
  it('should combine all context sections', () => {
    ...
    expect(result.prompt).toContain('<linked_note>');
    expect(result.prompt).toContain('<current_typora_document');
