...
describe('McpStorage', () => {
  ...
  describe('load/save', () => {
    it('should preserve unknown top-level keys and merge _typorai', async () => {
      const initial = {
        mcpServers: {
          legacy: { command: 'node' },
        },
    ...
    it('should keep existing _typorai fields when metadata is defaulted', async () => {
      const initial = {
        mcpServers: {
          legacy: { command: 'node' },
        },
      ...
      const saved = JSON.parse(files.get(MCP_CONFIG_PATH) || '{}') as Record<string, unknown>;
      expect(saved._typorai).toEqual({ extra: { keep: true } });
    ...
    it('should remove _typorai when only servers metadata exists', async () => {
      const initial = {
        mcpServers: {
          legacy: { command: 'node' },
        },
