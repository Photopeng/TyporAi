...
const fileProbe = {
  ...
  list: (filePath: string) => (fs.readdirSync(filePath, { withFileTypes: true }) as fs.Dirent[])
...
describe('TyporAiService', () => {
  ...
  beforeEach(() => {
    ...
    service = new TyporAiService(mockPlugin, createMockMcpManager());
  });
  ...
  describe('persistent query dynamic updates', () => {
    ...
    it('restarts with SAFE tool restrictions when going from YOLO to normal', async () => {
      // Start in YOLO mode
      ...
      expect(getQueryCallCount()).toBe(2);
    });

    it('restarts without SAFE tool restrictions when going from normal to YOLO', async () => {
