...
- **统一 CLI 设置**：Claude、Codex、OpenCode 使用一致的分组顺序、保存反馈与“保存并重启 Provider”流程。
- **CLI 诊断**：设置页可检查 CLI 路径、版本、宿主架构、登录状态和启动结果，并复制脱敏诊断信息。
- **OpenCode 分叉**：可从截断的本地对话历史创建新的 ACP 会话，不会修改源 OpenCode 会话。
- **更安全的 API 检查**：连接测试校验 HTTP(S) URL、配置模型和流式 SSE 能力，且不发送文档、选区或历史内容。
...
需要 Node.js 24 和桌面版 Typora。部署会修改 Typora 的入口 HTML（Windows/Linux 为 `window.html`，macOS 为 `Typora.app/Contents/Resources/TypeMark/index.html`）；请先完全退出 Typora。Windows 权限不足时使用管理员终端；macOS 的 Typora 位于系统 `/Applications` 且提示 `EACCES` 时，先执行 `npm run build:release`，再执行 `sudo node scripts/deploy-typora.mjs install`。部署脚本会把插件文件仍写入原登录用户的资料目录。
...
```text
Windows: %APPDATA%\Typora\plugins\typorai
macOS:   ~/Library/Application Support/abnerworks.Typora/plugins/typorai
Linux:   ${XDG_CONFIG_HOME:-~/.config}/Typora/plugins/typorai
```

Typora 加载器会按平台注入到：

```text
Windows: C:\Program Files\Typora\resources\window.html
macOS:   /Applications/Typora.app/Contents/Resources/TypeMark/index.html
Linux:   /usr/share/typora/resources/window.html
```

macOS 同时支持安装在 `~/Applications/Typora.app` 的 Typora；非默认位置可通过 `TYPORA_MACOS_APP_PATH` 指定。Typora 升级会替换应用包内的入口文件，升级后执行 `npm run repair:typora` 即可恢复加载器。
