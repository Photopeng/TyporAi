...
export async function loadOpencodeSessionMessages(
  ...
  options: { processTransport?: ProcessTransportFactory; fileProbe?: FileProbe } = {},
): Promise<ChatMessage[]> {
  const databasePath = resolveExistingOpencodeDatabasePath(providerState?.databasePath, undefined, {
    fileSystem: options.fileProbe
      ? { exists: path => options.fileProbe!.exists(path), readDirectory: path => options.fileProbe!.list(path).map(entry => entry.name) }
      : undefined,
  });
  if (!databasePath || databasePath === ':memory:' || !options.fileProbe?.exists(databasePath)) {
    return [];
  }

  const rows = await loadOpencodeSessionRows(databasePath, sessionId, options);
  if (!rows) {
