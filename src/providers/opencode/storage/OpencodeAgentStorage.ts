...
export class OpencodeAgentStorage {
  constructor(
    private workspaceAdapter: Pick<WorkspaceFileAdapter, 'exists' | 'read' | 'write' | 'delete' | 'listFilesRecursive' | 'ensureFolder'>,
  ) {}

  async loadAll(): Promise<OpencodeAgentDefinition[]> {
    return this.scanAdapter(this.workspaceAdapter);
  }
  ...
  async load(agent: OpencodeAgentDefinition): Promise<OpencodeAgentDefinition | null> {
    ...
    try {
      if (!(await this.workspaceAdapter.exists(filePath))) return null;
  ...
  async save(agent: OpencodeAgentDefinition, previous?: OpencodeAgentDefinition | null): Promise<void> {
    ...
    await this.workspaceAdapter.ensureFolder(joinPosixPath(filePath, '..'));
    const content = serializeOpencodeAgentMarkdown(agent);
    await this.workspaceAdapter.write(filePath, content);

    if (previousPath && previousPath !== filePath) {
      await this.workspaceAdapter.delete(previousPath);
    }
  }
  ...
  private async scanAdapter(
    adapter: Pick<WorkspaceFileAdapter, 'read' | 'listFilesRecursive'>,
