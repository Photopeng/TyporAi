...
export class OpencodeChatRuntime implements ChatRuntime {
  ...
  private async startProcess(params: {
    ...
    const processEnv: NodeJS.ProcessEnv = {
      ...
      PATH: getEnhancedPath(
        ...
        this.pathService?.isAbsolute(params.command) ? params.command : undefined,
      ),
    };
  ...
  private async writeTextFile(
    ...
    const resolvedPath = this.resolveSessionPath(request.sessionId, request.path);
    if (!this.fileStore || !this.pathService) throw new Error('OpenCode file access is unavailable');
    await this.fileStore.ensureDirectory(this.pathService.dirname(resolvedPath));
    await this.fileStore.writeAtomic(resolvedPath, request.content);
    return {};
  }
  ...
  private resolveSessionPath(sessionId: string, rawPath: string): string {
    ...
    if (this.pathService.isAbsolute(rawPath)) {
      return rawPath;
    }
    ...
    const cwd = this.sessionCwds.get(sessionId)
      ...
      ?? process.cwd();
    return this.pathService.normalize(this.pathService.join(cwd, rawPath));
  }
