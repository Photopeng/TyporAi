...
import type { FileStore, PathService, ProcessTransportFactory } from '../../../core/ports';
...
const OPENCODE_AUX_AGENT_IDS: Record<OpencodeAuxAgentProfile, string> = {
  passive: 'typorai-aux-passive',
  readonly: 'typorai-aux-readonly',
};
...
export class OpencodeAuxQueryRunner implements AuxQueryRunner {
  ...
  private async readTextFile(
    ...
    if (!this.fileStore) throw new Error('OpenCode file access is unavailable');
    const content = await this.fileStore.readText(resolvedPath);
  ...
  private resolveSessionPath(sessionId: string, rawPath: string): string {
    const cwd = this.sessionCwds.get(sessionId)
      ...
      ?? process.cwd();
    if (!this.pathService) throw new Error('OpenCode path service is unavailable');
    const resolvedPath = this.pathService.isAbsolute(rawPath)
      ? this.pathService.normalize(rawPath)
      : this.pathService.normalize(this.pathService.join(cwd, rawPath));
    const relative = this.pathService.relative?.(cwd, resolvedPath) ?? resolvedPath;
    if (relative === '' || (!relative.startsWith('..') && !this.pathService.isAbsolute(relative))) {
      return resolvedPath;
    }
...
function buildOpencodeAuxAgentConfig(profile: OpencodeAuxAgentProfile): OpencodeManagedAgentConfig {
  ...
  if (profile === 'readonly') {
    return {
      definition: {
        description: 'Internal TyporAi read-only agent for OpenCode auxiliary tasks.',
        mode: 'primary',
        permission: {
          '*': 'deny',
          codesearch: 'allow',
  ...
  return {
    definition: {
      description: 'Internal TyporAi no-tool agent for OpenCode auxiliary tasks.',
      mode: 'primary',
      permission: {
        '*': 'deny',
        external_directory: 'deny',
