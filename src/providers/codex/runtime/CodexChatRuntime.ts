...
function resolveCodexSandboxConfig(
  ...
  _codexSafeMode: CodexSafeMode = 'workspace-write',
...
export class CodexChatRuntime implements ChatRuntime {
  ...
  async *query(
    ...
    this.canceled = false;
    await this.cleanupActiveInputBundles();
    this.chunkBuffer = [];
    ...
    try {
      ...
      if (turn.isCompact) {
        ...
        const turnInputBundle = await this.buildInput(turn.prompt, turn.request.images, skillInputs);
      ...
      if (!this.session.getSessionFilePath()) {
        ...
        if (threadId) {
          const transcriptRoot = this.resolveTranscriptRootHost(
            this.session.getSessionFilePath() ?? this.currentThreadPath,
          );
          const sessionFilePath = transcriptRoot && this.fileProbe
            ? findCodexSessionFile(threadId, transcriptRoot, this.fileProbe)
            : null;
          if (sessionFilePath) {
  ...
  async steer(turn: PreparedChatTurn): Promise<boolean> {
    ...
    const skillInputs = await this.resolveSkillInputs(turn.request.text);
    const inputBundle = await this.buildInput(turn.prompt, turn.request.images, skillInputs);
    ...
    try {
      ...
      await this.disposeInputBundle(inputBundle);
      throw error;
    }
  }
  ...
  private async disposeInputBundle(bundle: CodexInputBundle): Promise<void> {
  ...
  private async cleanupActiveInputBundles(): Promise<void> {
  ...
  private buildTurnSandboxPolicy(
    ...
    const memoriesDirTarget = deriveCodexMemoriesDirFromSessionsRoot(transcriptRootTargetHint)
      ...
      ?? (
        this.launchSpec?.target.method === 'wsl'
          ? null
          : this.resolveHostPath('.codex', 'memories')
      );
    ...
    const writableRoots = [
      ...
      memoriesDirTarget,
      this.mapHostPathToTarget(this.getTemporaryRoot()),
      this.launchSpec?.target.platformFamily === 'unix' ? '/tmp' : null,
      this.mapHostPathToTarget(this.environmentService?.get('TMPDIR') ?? undefined),
  ...
  private async buildInput(text: string, images?: ImageAttachment[], skills?: SkillInput[]): Promise<CodexInputBundle> {
    ...
    const cleanup = async (): Promise<void> => {
      ...
      try {
        await this.fileStore?.remove(tempDir).catch(() => undefined);
      } catch { /* best-effort cleanup */ }
    };

    try {
      if (images && images.length > 0) {
        if (!this.fileStore || !this.pathService) {
          throw new Error('Codex image attachments require Typora host file services.');
        }
        tempDir = this.pathService.join(
          this.getTemporaryRoot(),
          `typorai-codex-images-${Date.now()}-${Math.random().toString(16).slice(2)}`,
        );
        await this.fileStore.ensureDirectory(tempDir);
        for (let i = 0; i < images.length; i++) {
          const img = images[i];
          if (!img.mediaType.startsWith('image/')) continue;

          const filename = toAttachmentFilename(img, i);
          const filePath = this.pathService.join(tempDir, `${i + 1}-${filename}`);
          await this.fileStore.writeBinary(filePath, decodeBase64(img.data));
  ...
  private getTemporaryRoot(): string {
    return this.environmentService?.get('TMPDIR')
      ?? this.environmentService?.get('TEMP')
      ?? this.environmentService?.get('TMP')
      ?? (this.launchSpec?.target.platformFamily === 'windows' ? 'C:\\Temp' : '/tmp');
  }

  private resolveHostPath(...parts: string[]): string | null {
    const home = this.environmentService?.homeDirectory();
    if (!home || !this.pathService) return null;
    return this.pathService.join(home, ...parts);
  }
