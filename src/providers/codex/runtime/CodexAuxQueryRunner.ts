...
export class CodexAuxQueryRunner {
  ...
  private async startProcess(): Promise<void> {
    this.launchSpec = await resolveCodexAppServerLaunchSpec(
      this.plugin,
      'codex',
      this.processTransport,
    );
    this.process = this.processTransport
      ? new CodexAppServerProcess(this.launchSpec, this.processTransport)
      : new CodexAppServerProcess(this.launchSpec);
    ...
    const initializeResult = await this.transport.request<InitializeResult>('initialize', {
      clientInfo: { name: 'typorai-aux', version: '1.0.0' },
