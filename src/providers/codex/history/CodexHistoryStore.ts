...
function getPathModuleForSessionPath(sessionPath: string): PortablePathFlavor {
  if (sessionPath.includes('\\') || /^[A-Za-z]:/.test(sessionPath)) {
    return {
      basename: basenamePortable,
      dirname: dirnameWindowsPath,
      join: joinWindowsPath,
...
export function findCodexSessionFile(
  ...
  if (!threadId || !SAFE_SESSION_ID_PATTERN.test(threadId) || !fileProbe.exists(root)) {
    return null;
  }

  const pathModule = getPathModuleForSessionPath(root);
  const directPath = pathModule.join(root, `${threadId}.jsonl`);
  if (fileProbe.exists(directPath)) {
    return directPath;
  }
  ...
  while (stack.length > 0) {
    ...
    try {
      entries = fileProbe.list(current)
        .map(entry => ({ name: entry.name, isDirectory: () => !entry.isFile, isFile: () => entry.isFile }));
    } catch {
      continue;
    }
...
export function parseCodexSessionFile(filePath: string, fileProbe: FileProbe): ChatMessage[] {
  let content: string;
  try {
    content = fileProbe.readText(filePath);
  } catch {
    return [];
  }
