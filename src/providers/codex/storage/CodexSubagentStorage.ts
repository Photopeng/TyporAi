...
export class CodexSubagentStorage {
  constructor(
    private workspaceAdapter: Pick<WorkspaceFileAdapter, 'exists' | 'read' | 'write' | 'delete' | 'listFiles' | 'ensureFolder'>,
  ) {}
  ...
  async save(agent: CodexSubagentDefinition, previous?: CodexSubagentDefinition | null): Promise<void> {
    ...
    await this.workspaceAdapter.ensureFolder(CODEX_AGENTS_PATH);
  ...
  private async scanAdapter(
    adapter: Pick<WorkspaceFileAdapter, 'read' | 'listFiles'>,
