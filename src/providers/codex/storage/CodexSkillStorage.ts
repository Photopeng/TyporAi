...
import { joinPosixPath, joinWindowsPath, normalizePosixPath, normalizeWindowsPath } from '../../../utils/portablePath';
...
export function resolveCodexSkillLocationFromPath(
  ...
  const normalize = usesWindowsPathSemantics ? normalizeWindowsPath : normalizePosixPath;
  const join = usesWindowsPathSemantics ? joinWindowsPath : joinPosixPath;
  const separator = usesWindowsPathSemantics ? '\\' : '/';
  ...
  for (const [rootId, rootPath] of Object.entries(ROOT_PATH_BY_ID) as Array<[CodexSkillRootId, string]>) {
    const rootDir = normalize(join(normalizedVaultPath, rootPath));
    const rootPrefix = rootDir.endsWith(separator) ? rootDir : `${rootDir}${separator}`;
    const comparableRoot = usesWindowsPathSemantics ? rootPrefix.toLowerCase() : rootPrefix;
    const comparableSkill = usesWindowsPathSemantics ? normalizedSkillPath.toLowerCase() : normalizedSkillPath;
    const relative = comparableSkill.startsWith(comparableRoot)
      ? normalizedSkillPath.slice(rootPrefix.length)
      : '..';
...
export class CodexSkillStorage {
  constructor(
    private workspaceAdapter: CodexSkillStorageAdapter,
    private homeAdapter?: CodexSkillStorageAdapter,
  ) {}

  async scanAll(): Promise<CodexSkillEntry[]> {
    const vaultSkills = await this.scanRoots(this.workspaceAdapter, ALL_SCAN_ROOTS, 'vault');
  ...
  async scanVault(): Promise<CodexSkillEntry[]> {
    return this.scanRoots(this.workspaceAdapter, ALL_SCAN_ROOTS, 'vault');
  }
  ...
  async save(input: CodexSkillSaveInput): Promise<void> {
    ...
    await this.workspaceAdapter.ensureFolder(dirPath);
    const markdown = serializeSlashCommandMarkdown(
      { name: input.name, description: input.description },
      input.content,
    );
    await this.workspaceAdapter.write(filePath, markdown);
  ...
  async delete(location: CodexSkillLocation): Promise<void> {
    ...
    await this.workspaceAdapter.delete(filePath);
    await this.workspaceAdapter.deleteFolder(dirPath);
  }
  ...
  async load(location: CodexSkillLocation): Promise<CodexSkillEntry | null> {
    ...
    try {
      const content = await this.workspaceAdapter.read(filePath);
