/**
 * TyporAi - Claude Agent SDK wrapper
 *
...
import type { PermissionMode,TyporAiSettings } from '../../../core/types/settings';
...
export class TyporAiService implements ChatRuntime {
  ...
  // Notifier mirrored from `CreateChatRuntimeOptions.onExternalContextPathsChanged`
  // so the owning tab state can keep its file context chips in sync.
  private onExternalContextPathsChanged?: (paths: string[]) => void;

  // Modular components
  private sessionManager = new SessionManager();
  private mcpManager: McpServerManager;
  private processTransport: ProcessTransportFactory | undefined;
  private notificationService: NotificationService = { show: () => undefined };
  private fileBackups?: FileBackupService;
  ...
  private getLegacyPluginDeps(): TyporAiPlugin & {
    agentManager?: Pick<AppAgentManager, 'setBuiltinAgentNames'>;
    pluginManager?: AppPluginManager;
  } {
    return this.plugin;
  ...
  constructor(plugin: TyporAiPlugin, services: ClaudeRuntimeServices | McpServerManager) {
    this.plugin = plugin;
    ...
    if ('mcpManager' in services) {
      ...
      this.processTransport = services.processTransport;
      this.notificationService = services.notificationService ?? this.notificationService;
      this.fileBackups = services.fileBackups;
  ...
  setExternalContextPathsListener(
    listener: ((paths: string[]) => void) | undefined,
  ): void {
    this.onExternalContextPathsChanged = listener;
  }
   ...
   */
  private getScopedSettings(): TyporAiSettings {
  ...
  private async flushAutoTurnBuffer(): Promise<void> {
    ...
    try {
      await this._autoTurnCallback?.({ chunks, metadata });
          ...
          this.notificationService.show(t('renderer.subagent.backgroundTaskRenderFailed'), 'error');
    }
  }
    ...
    await applyClaudeDynamicUpdates(
      {
        ...
        setCurrentExternalContextPaths: (paths) => {
          ...
          // Mirror the latest paths into the owning tab so the file
          // context manager stays in sync across tab switches.
          this.onExternalContextPathsChanged?.(paths);
        },
  ...
  async rewind(
    ...
    return executeClaudeRewind(userMessageId, {
      ...
      fileBackups: this.fileBackups,
      paths: this.pathService,
    });
  }
  ...
  private createApprovalCallback(): CanUseTool {
    return createClaudeApprovalCallback({
      ...
      getPermissionMode: () => this.plugin.settings.permissionMode,
      notifyDocumentEditBlocked: () => {
        this.notificationService.show(t('inlineEdit.errors.documentEditingBlocked'), 'warning');
      },
      resolveSDKPermissionMode: (mode) => this.resolveSDKPermissionMode(mode),
