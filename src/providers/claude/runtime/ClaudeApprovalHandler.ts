...
export interface ClaudeApprovalHandlerDeps {
  ...
  notifyDocumentEditBlocked?: () => void;
...
export function createClaudeApprovalCallback(
  ...
  return async (toolName, input, options): Promise<PermissionResult> => {
    if (!isDocumentEditingAllowed(deps.getPermissionMode()) && isDocumentMutationTool(toolName)) {
      deps.notifyDocumentEditBlocked?.();
      return {
        behavior: 'deny',
        message: t('inlineEdit.errors.documentEditingBlocked'),
      };
    }
    ...
    if (toolName === TOOL_ASK_USER_QUESTION && askUserQuestionCallback) {
      try {
        ...
        // the SDK doesn't inject isOther into the canUseTool input. TyporAi
