...
import { getDocumentMutationToolsForMode } from '../../../core/security/documentEditing';
...
export class QueryOptionsBuilder {
  static needsRestart(
    ...
    // Note: Permission mode is handled dynamically via setPermissionMode() in TyporAiService.
  ...
  static buildPersistentQueryConfig(
    ...
    const disallowedToolsKey = [
      ...ctx.mcpManager.getAllDisallowedMcpTools(),
      ...getDocumentMutationToolsForMode(ctx.settings.permissionMode),
    ].join('|');
  ...
  static resolveClaudeSdkPermissionMode(
    ...
    _claudeSafeMode: ClaudeSafeMode = 'acceptEdits',
  ): SDKPermissionMode {
  ...
  private static buildBaseOptions(
    ...
    options.spawnClaudeCodeProcess = createCustomSpawnFunction(ctx.enhancedPath, ctx.processTransport);
