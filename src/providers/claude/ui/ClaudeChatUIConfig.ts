...
export const claudeChatUIConfig: ProviderChatUIConfig = {
  ...
  applyModelDefaults(model: string, settings: unknown): void {
    ...
    const runtimeModel = toClaudeRuntimeModelId(model);
    // Always normalize through `normalizeEffortLevel` so a user-supplied valid
    // effort level (e.g. `'max'` on a custom model) is preserved instead of
    // being silently overwritten by the model default.
