 ...
 * 1. TyporaPluginRuntime agents: {installPath}/agents/*.md (namespaced as plugin-name:agent-name)
 * 2. TyporaWorkspace agents: {vaultPath}/.claude/agents/*.md
...
export class AgentManager {
  ...
  constructor(vaultPath: string, pluginManager: PluginManager, private fileProbe?: FileProbe) {
  ...
  private loadPluginAgents(): void {
    for (const plugin of this.pluginManager.getPlugins()) {
      ...
      const agentsDir = `${plugin.installPath}/${PLUGIN_AGENTS_DIR}`;
      if (!this.fileProbe?.exists(agentsDir)) continue;
  ...
  private loadVaultAgents(): void {
    this.loadAgentsFromDirectory(`${this.vaultPath}/${VAULT_AGENTS_DIR}`, 'vault');
  }
  ...
  private loadGlobalAgents(): void {
    ...
    if (home) this.loadAgentsFromDirectory(`${home}/.claude/agents`, 'global');
  }
  ...
  private loadAgentsFromDirectory(
    ...
    if (!this.fileProbe?.exists(dir)) return;
  ...
  private listMarkdownFiles(dir: string): string[] {
    ...
    try {
      const entries = this.fileProbe?.list(dir) ?? [];
  ...
  private parseAgentDefinition(
    ...
    try {
      const content = this.fileProbe?.readText(filePath);
