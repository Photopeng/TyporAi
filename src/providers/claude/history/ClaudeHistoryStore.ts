...
import type { HistoryFileProbe } from './sdkSessionPaths';
import {
  deleteSDKSession,
...
export async function loadSDKSessionMessages(
  ...
  const result = await readSDKSession(vaultPath, sessionId, fileProbe);
      ...
      for (const toolCall of msg.toolCalls) {
        ...
        if (subagent) {
          ...
          if (subagent.agentId && isValidAgentId(subagent.agentId)) {
            sidecarLoads.push({
              subagent,
              promise: loadSubagentToolCalls(vaultPath, sessionId, subagent.agentId, fileProbe),
            });
          }
        }
      }
