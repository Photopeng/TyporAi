...
async function enrichAsyncSubagentToolCalls(
  ...
  for (const subagent of Object.values(subagentData)) {
    ...
    for (const sessionId of uniqueSessionIds) {
      ...
      if (!loader) {
        loader = loadSubagentToolCalls(vaultPath, sessionId, subagent.agentId, fileProbe);
...
export class ClaudeConversationHistoryService implements ProviderConversationHistoryService {
  ...
  async hydrateConversationHistory(
    ...
    for (const sessionId of allSessionIds) {
      if (!sdkSessionExists(vaultPath, sessionId, this.fileProbe)) {
        missingSessionCount++;
        continue;
      }
      ...
      const truncateAt = isCurrentSession
        ...
        : undefined;
      const result = await loadSDKSessionMessages(vaultPath, sessionId, truncateAt, this.fileProbe);
  ...
  async deleteConversationSession(
    ...
    await deleteSDKSession(vaultPath, sessionId, this.fileProbe);
  }
}
