...
import { DEFAULT_TYPORAI_SETTINGS } from './defaultSettings';
...
function normalizeChatViewPlacement(
  ...
  return DEFAULT_TYPORAI_SETTINGS.chatViewPlacement;
}
...
const REMOVED_PROVIDER_CONFIG_IDS = new Set(['pi']);
...
function normalizeProviderConfigs(value: unknown): ProviderConfigMap {
  ...
  for (const [providerId, config] of Object.entries(value as Record<string, unknown>)) {
    if (REMOVED_PROVIDER_CONFIG_IDS.has(providerId)) {
      continue;
    }
...
export class TyporAiSettingsStorage {
  constructor(private adapter: WorkspaceFileAdapter) {}

  async load(): Promise<StoredTyporAiSettings> {
    ...
    if (
      settingsPath !== TYPORAI_SETTINGS_PATH
      || (
  ...
  async save(settings: StoredTyporAiSettings): Promise<void> {
  ...
  async exists(): Promise<boolean> {
    if (await this.adapter.exists(TYPORAI_SETTINGS_PATH)) {
      return true;
    }

    return this.adapter.exists(LEGACY_TYPORAI_SETTINGS_PATH);
  }

  async update(updates: Partial<StoredTyporAiSettings>): Promise<void> {
  ...
  private getDefaults(): StoredTyporAiSettings {
    return DEFAULT_TYPORAI_SETTINGS;
  }

  private async getLoadPath(): Promise<string | null> {
    if (await this.adapter.exists(TYPORAI_SETTINGS_PATH)) {
      return TYPORAI_SETTINGS_PATH;
    }

    if (await this.adapter.exists(LEGACY_TYPORAI_SETTINGS_PATH)) {
      return LEGACY_TYPORAI_SETTINGS_PATH;
    }
