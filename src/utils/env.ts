...
import { joinPosixPath } from './portablePath';
...
// Linux applications expose CLIs through stable paths; transient AppImage mounts are excluded.
...
/** Typora is a GUI app with a minimal PATH, so add common binary locations. */
function getExtraBinaryPaths(): string[] {
  ...
  if (isWindows) {
    ...
    const voltaHome = process.env.VOLTA_HOME;
    if (voltaHome) {
      paths.push(joinPosixPath(voltaHome, 'bin'));
    }

    const asdfRoot = process.env.ASDF_DATA_DIR || process.env.ASDF_DIR;
    if (asdfRoot) {
      paths.push(joinPosixPath(asdfRoot, 'shims'));
      paths.push(joinPosixPath(asdfRoot, 'bin'));
    }
    ...
    if (home) {
      paths.push(joinPosixPath(home, '.local', 'bin'));
      paths.push(joinPosixPath(home, '.bun', 'bin'));
      paths.push(joinPosixPath(home, '.opencode', 'bin'));
      paths.push(joinPosixPath(home, '.docker', 'bin'));
      paths.push(joinPosixPath(home, '.volta', 'bin'));
...
export function getMissingNodeError(cliPath: string, enhancedPath?: string): string | null {
  ...
  return 'Claude Code CLI requires Node.js, but Node was not found on PATH. Install Node.js or use the native Claude Code binary, then restart Typora.';
}
