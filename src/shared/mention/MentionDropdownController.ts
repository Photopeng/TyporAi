...
export interface MentionDropdownCallbacks {
  ...
  getCachedWorkspaceFolders: () => Array<Pick<FolderMentionItem, 'name' | 'path'>>;
  getCachedWorkspaceFiles: () => WorkspaceFileEntry[];
  normalizeWorkspacePath: (path: string | undefined | null) => string | null;
}
...
export class MentionDropdownController {
  ...
  constructor(
    ...
    this.dropdown = new SelectableDropdown<MentionItem>(this.containerEl, {
      listClassName: 'typorai-mention-dropdown',
      itemClassName: 'typorai-mention-item',
      emptyClassName: 'typorai-mention-empty',
      fixed: this.fixed,
      fixedClassName: 'typorai-mention-dropdown-fixed',
    });
  }
  ...
  private appendVaultItems(searchLower: string): number {
    type ScoredItem =
      ...
      | { type: 'file'; name: string; path: string; startsWithQuery: boolean; mtime: number };
    ...
    const allFiles = this.callbacks.getCachedWorkspaceFiles();
    const getMtime = (file: WorkspaceFileEntry): number =>
      file.mtime ?? (file as unknown as { stat?: { mtime?: number } }).stat?.mtime ?? 0;
    ...
    for (const f of allFiles) {
      ...
      for (let i = 1; i < parts.length; i++) {
        ...
        if (mtime > existing) {
          folderMtimeMap.set(folderPath, mtime);
        }
      }
    }

    const scoredFolders: ScoredItem[] = this.callbacks.getCachedWorkspaceFolders()
      .map(f => ({
        name: f.name,
    ...
    for (const item of merged) {
      if (item.type === 'folder') {
        ...
        this.filteredMentionItems.push({ type: 'file', name: item.name, path: item.path });
      }
    }
  ...
  private renderMentionDropdown(): void {
    this.dropdown.render({
      ...
      renderItem: (item, itemEl) => {
        const iconEl = itemEl.createSpan({ cls: 'typorai-mention-icon' });
        switch (item.type) {
          case 'mcp-server':
        ...
        const textEl = itemEl.createSpan({ cls: 'typorai-mention-text' });

        switch (item.type) {
          case 'mcp-server':
            textEl.createSpan({ cls: 'typorai-mention-name' }).setText(`@${item.name}`);
            break;
          case 'agent-folder':
            textEl.createSpan({
              cls: 'typorai-mention-name typorai-mention-name-agent-folder',
            }).setText(`@${item.name}/`);
            break;
          case 'agent': {
            // Show ID (which is namespaced for plugin agents) for consistency with inserted text
            textEl.createSpan({
              cls: 'typorai-mention-name typorai-mention-name-agent',
            }).setText(`@${item.id}`);
            if (item.description) {
              textEl.createSpan({ cls: 'typorai-mention-agent-desc' }).setText(item.description);
            }
            break;
          }
          case 'context-folder':
            textEl.createSpan({
              cls: 'typorai-mention-name typorai-mention-name-folder',
            }).setText(`@${item.name}/`);
            break;
          case 'context-file':
            textEl.createSpan({
              cls: 'typorai-mention-name typorai-mention-name-context',
            }).setText(item.name);
            break;
          case 'folder':
            textEl.createSpan({
            ...
            break;
          default:
            textEl.createSpan({ cls: 'typorai-mention-path' }).setText(item.path || item.name);
        }
      },
  ...
  private positionFixed(): void {
    ...
    dropdownEl.setCssProps({
      '--typorai-fixed-dropdown-bottom': `${window.innerHeight - inputRect.top + 4}px`,
      '--typorai-fixed-dropdown-left': `${inputRect.left}px`,
      '--typorai-fixed-dropdown-width': `${Math.max(inputRect.width, 280)}px`,
    });
  }
  ...
  private selectMentionItem(): void {
    ...
    switch (selectedItem.type) {
      ...
      case 'folder': {
        const normalizedPath = this.callbacks.normalizeWorkspacePath(selectedItem.path);
