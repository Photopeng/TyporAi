/**
 * TyporAi - Resume session dropdown
 *
...
export class ResumeSessionDropdown {
  ...
  constructor(
    ...
    this.dropdownEl = this.containerEl.createDiv({ cls: 'typorai-resume-dropdown' });
    this.render();
  ...
  private updateSelection(): void {
    const items = this.dropdownEl.querySelectorAll('.typorai-resume-item');
  ...
  private render(): void {
    ...
    const header = this.dropdownEl.createDiv({ cls: 'typorai-resume-header' });
    header.createSpan({ text: t('chat.resume.title') });

    if (this.conversations.length === 0) {
      this.dropdownEl.createDiv({ cls: 'typorai-resume-empty', text: t('chat.resume.empty') });
      return;
    }

    const list = this.dropdownEl.createDiv({ cls: 'typorai-resume-list' });

    for (let i = 0; i < this.conversations.length; i++) {
      const conv = this.conversations[i];
      const isCurrent = conv.id === this.currentConversationId;

      const item = list.createDiv({ cls: 'typorai-resume-item' });
      if (isCurrent) item.addClass('current');
      if (i === this.selectedIndex) item.addClass('selected');

      const iconEl = item.createDiv({ cls: 'typorai-resume-item-icon' });
