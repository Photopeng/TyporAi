...
├── settings/       # base (shared .typorai-sp-* panel layout), env-snippets, slash-settings, mcp-settings, plugin-settings, agent-settings
├── accessibility.css
...
- **Prefix**: TyporAi-owned classes should use the `.typorai-` prefix; shared Obsidian host selectors and generic state classes may remain unprefixed
- **BEM-lite**: Prefer `.typorai-{block}`, `.typorai-{block}-{element}`, `.typorai-{block}--{modifier}` for TyporAi-owned selectors
