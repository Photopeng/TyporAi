...
Main sidebar chat interface. `TyporAiView` assembles tabs, controllers, renderers, and provider-backed services around the shared `ChatRuntime` boundary.
...
```text
TyporAiView (lifecycle + assembly)
├── ChatState
├── Controllers
...
- `TyporAiView.onClose()` must abort active tabs and dispose runtimes
