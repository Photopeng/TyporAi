...
export class BangBashModeManager {
  ...
  constructor(
    ...
    this.notificationService = callbacks.notificationService ?? new NoticeAdapter();
  ...
  private enterMode(): boolean {
    ...
    wrapper.classList.add('typorai-input-bang-bash-mode');
  ...
  private exitMode(): void {
    const wrapper = this.callbacks.getInputWrapper();
    if (wrapper) {
      wrapper.classList.remove('typorai-input-bang-bash-mode');
    }
  ...
  private async submit(): Promise<void> {
    ...
    try {
      ...
      this.notificationService.show(
        t('chat.bangBash.commandFailed', { error: e instanceof Error ? e.message : String(e) }),
        'error',
      );
    } finally {
