...
export class StatusPanel {
  ...
  constructor(private readonly notifications: NotificationService = new NoticeAdapter()) {}
  ...
  private createPanel(): void {
    ...
    this.panelEl.className = 'typorai-status-panel';

    // Bash output container - hidden by default
    this.bashOutputContainerEl = ownerDocument.createElement('div');
    this.bashOutputContainerEl.className = 'typorai-status-panel-bash typorai-hidden';

    this.bashHeaderEl = ownerDocument.createElement('div');
    this.bashHeaderEl.className = 'typorai-tool-header typorai-status-panel-bash-header';
    ...
    this.bashContentEl.className = 'typorai-status-panel-bash-content';
    ...
    this.todoContainerEl.className = 'typorai-status-panel-todos typorai-hidden';
    this.panelEl.appendChild(this.todoContainerEl);

    // Todo header (collapsed view)
    this.todoHeaderEl = ownerDocument.createElement('div');
    this.todoHeaderEl.className = 'typorai-status-panel-header';
    ...
    this.todoContentEl.className = 'typorai-status-panel-content typorai-todo-list-container typorai-hidden';
  ...
  updateTodos(todos: TodoItem[] | null): void {
    ...
    if (!todos || todos.length === 0) {
      this.todoContainerEl.addClass('typorai-hidden');
      this.todoHeaderEl.empty();
      this.todoContentEl.empty();
      return;
    }

    this.todoContainerEl.removeClass('typorai-hidden');
  ...
  private renderTodoHeader(completedCount: number, totalCount: number, currentTask: TodoItem | undefined): void {
    ...
    icon.className = 'typorai-status-panel-icon';
    setIcon(icon, getToolIcon(TOOL_TODO_WRITE));
    this.todoHeaderEl.appendChild(icon);

    // Label
    const label = ownerDocument.createElement('span');
    label.className = 'typorai-status-panel-label';
    ...
    if (!this.isTodoExpanded) {
      ...
      if (completedCount === totalCount && totalCount > 0) {
        ...
        status.className = 'typorai-status-panel-status status-completed';
      ...
      if (currentTask) {
        ...
        current.className = 'typorai-status-panel-current';
  ...
  private updateTodoDisplay(): void {
    ...
    // Show/hide content
    this.todoContentEl.toggleClass('typorai-hidden', !this.isTodoExpanded);
  ...
  private renderBashOutputs(options: { scroll?: boolean } = {}): void {
    ...
    if (this.currentBashOutputs.size === 0) {
      this.bashOutputContainerEl.addClass('typorai-hidden');
      return;
    }

    this.bashOutputContainerEl.removeClass('typorai-hidden');
    ...
    headerIconEl.className = 'typorai-tool-icon';
    ...
    headerLabelEl.className = 'typorai-tool-label';
    ...
    previewEl.className = 'typorai-tool-current';
    previewEl.classList.toggle('typorai-hidden', !this.isBashExpanded);
    this.bashHeaderEl.appendChild(previewEl);

    const summaryStatusEl = ownerDocument.createElement('span');
    summaryStatusEl.className = 'typorai-tool-status';
    if (!this.isBashExpanded && latest) {
      ...
      summaryStatusEl.classList.add('typorai-hidden');
    }
    ...
    actionsEl.className = 'typorai-status-panel-bash-actions';
    ...
    this.bashContentEl.toggleClass('typorai-hidden', !this.isBashExpanded);
  ...
  private renderBashEntry(info: PanelBashOutput, ownerDocument: Document): HTMLElement {
    ...
    entryEl.className = 'typorai-tool-call typorai-status-panel-bash-entry';

    const entryHeaderEl = ownerDocument.createElement('div');
    entryHeaderEl.className = 'typorai-tool-header';
    ...
    entryIconEl.className = 'typorai-tool-icon';
    ...
    entryLabelEl.className = 'typorai-tool-label';
    ...
    entryStatusEl.className = 'typorai-tool-status';
    ...
    contentEl.className = 'typorai-tool-content';
    const isEntryExpanded = this.bashEntryExpanded.get(info.id) ?? true;
    contentEl.classList.toggle('typorai-hidden', !isEntryExpanded);
    ...
    rowEl.className = 'typorai-tool-result-row';

    const textEl = ownerDocument.createElement('span');
    textEl.className = 'typorai-tool-result-text';
  ...
  private async copyLatestBashOutput(): Promise<void> {
    ...
    try {
      ...
      this.notifications.show(t('chat.bangBash.copyFailed'), 'error');
    }
  }
  ...
  private appendActionButton(
    ...
    el.className = `typorai-status-panel-bash-action typorai-status-panel-bash-action-${name}`;
