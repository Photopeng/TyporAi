...
export class ImageContextManager {
  ...
  constructor(
    ...
    notifications: NotificationService = new NoticeAdapter(),
  ) {
    ...
    const fileIndicator = this.previewContainerEl.querySelector('.typorai-file-indicator');
    this.imagePreviewEl = this.previewContainerEl.createDiv({ cls: 'typorai-image-preview' });
  ...
  private setupDragAndDrop() {
    const inputWrapper = this.containerEl.querySelector('.typorai-input-wrapper') as HTMLElement;
    if (!inputWrapper) return;

    this.dropOverlay = inputWrapper.createDiv({ cls: 'typorai-drop-overlay' });
    const dropContent = this.dropOverlay.createDiv({ cls: 'typorai-drop-content' });
    ...
    dropContent.createSpan({ text: t('chat.imageAttachments.dropHere') });
  ...
  private handleDragLeave(e: DragEvent) {
    ...
    const inputWrapper = this.containerEl.querySelector('.typorai-input-wrapper');
    if (!inputWrapper) {
  ...
  private async addImageFromFile(file: File, source: 'paste' | 'drop'): Promise<boolean> {
    if (!this.enabled) {
      this.notifications.show(t('chat.imageAttachments.unsupportedByProvider'), 'warning');
      return false;
    }

    if (file.size > MAX_IMAGE_SIZE) {
      this.notifyImageError(t('chat.imageAttachments.exceedsLimit', { size: this.formatSize(MAX_IMAGE_SIZE) }));
      return false;
    }

    const mediaType = this.getMediaType(file.name) || (file.type as ImageMediaType);
    if (!mediaType) {
      this.notifyImageError(t('chat.imageAttachments.unsupportedType'));
      return false;
    }
    ...
    try {
      ...
      this.notifyImageError(t('chat.imageAttachments.attachFailed'), error);
      return false;
    }
  }
  ...
  private updateImagePreview() {
    ...
    if (this.attachedImages.size === 0) {
      this.imagePreviewEl.removeClass('typorai-visible-flex');
      this.imagePreviewEl.addClass('typorai-hidden');
      return;
    }

    this.imagePreviewEl.addClass('typorai-visible-flex');
    this.imagePreviewEl.removeClass('typorai-hidden');
  ...
  private renderImagePreview(id: string, image: ImageAttachment) {
    const previewEl = this.imagePreviewEl.createDiv({ cls: 'typorai-image-chip' });

    const thumbEl = previewEl.createDiv({ cls: 'typorai-image-thumb' });
    ...
    const removeEl = previewEl.createSpan({ cls: 'typorai-image-remove' });
    removeEl.setText('\u00D7');
    removeEl.setAttribute('aria-label', t('chat.imageAttachments.remove'));
  ...
  private notifyImageError(message: string, error?: unknown) {
    ...
    if (error instanceof Error) {
      if (error.message.includes('ENOENT') || error.message.includes('no such file')) {
        userMessage = `${message} (${t('chat.imageAttachments.fileNotFound')})`;
      } else if (error.message.includes('EACCES') || error.message.includes('permission denied')) {
        userMessage = `${message} (${t('chat.imageAttachments.permissionDenied')})`;
      }
    }
    this.notifications.show(userMessage, 'error');
  }
}
