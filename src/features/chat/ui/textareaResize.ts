...
export function autoResizeTextarea(textarea: HTMLTextAreaElement): void {
  const viewHeight = textarea.closest('.typorai-container')?.clientHeight ?? window.innerHeight;
  const maxHeight = calculateTextareaMaxHeight(viewHeight);

  textarea.setCssProps({
    '--typorai-textarea-min-height': `${TEXTAREA_BASE_MIN_HEIGHT}px`,
    '--typorai-textarea-max-height': `${maxHeight}px`,
  });
  ...
  textarea.setCssProps({
    '--typorai-textarea-min-height': `${minHeight}px`,
    '--typorai-textarea-max-height': `${maxHeight}px`,
  });
}
