...
import type { ChatMessage, ChatSelectionRef, ImageAttachment, SubagentInfo, ToolCallInfo } from '../../../core/types';
import { t } from '../../../i18n/i18n';
import type TyporAiPlugin from '../../../main';
import { ContextMenu as Menu } from '../../../ui/ContextMenu';
import { renderMarkdown as renderMarkdownContent } from '../../../ui/MarkdownRenderer';
...
export class MessageRenderer {
  private app: TyporAiPlugin['app'];
  private plugin: TyporAiPlugin;
  private component: RendererLifecycle;
  ...
  private createUserMessageFrame(msg: ChatMessage): UserMessageFrame {
    const textToShow = this.getUserMessageTextToShow(msg);
    const hasImages = (msg.images?.length ?? 0) > 0;
    const hasSelection = msg.selection !== undefined;

    if (!hasImages && !hasSelection) {
      return {
        parentEl: this.messagesEl,
        rootEl: this.messagesEl,
        textToShow,
      };
    }

    const groupEl = this.messagesEl.createDiv({ cls: 'typorai-message-user-stack' });
    if (hasImages && msg.images) {
      this.renderMessageImages(groupEl, msg.images);
    }
    if (msg.selection) {
      this.renderMessageSelection(groupEl, msg.selection);
    }
  ...
  addMessage(msg: ChatMessage): HTMLElement {
    const userFrame = msg.role === 'user'
      ? this.createUserMessageFrame(msg)
      : null;

    // Skip empty bubble for image-only messages
    if (msg.role === 'user') {
      if (!userFrame?.textToShow) {
        this.scrollToBottom();
        return userFrame?.rootEl ?? this.messagesEl;
      }
    }

    const messageParentEl = userFrame?.parentEl ?? this.messagesEl;
    const msgEl = messageParentEl.createDiv({
      cls: `typorai-message typorai-message-${msg.role}`,
      attr: {
    ...
    const contentEl = msgEl.createDiv({ cls: 'typorai-message-content', attr: { dir: 'auto' } });

    if (msg.role === 'user') {
      const textToShow = userFrame?.textToShow ?? this.getUserMessageTextToShow(msg);
      if (textToShow) {
        const textEl = contentEl.createDiv({ cls: 'typorai-text-block' });
  ...
  updateLiveUserMessage(msg: ChatMessage): void {
    ...
    const toolbar = msgEl.querySelector<HTMLElement>('.typorai-user-msg-actions');
    if (toolbar) {
      toolbar.querySelectorAll('.typorai-user-msg-copy-btn').forEach((el) => el.remove());
    }
  ...
  renderMessages(
    ...
    const newWelcomeEl = this.messagesEl.createDiv({ cls: 'typorai-welcome' });
    newWelcomeEl.createDiv({ cls: 'typorai-welcome-greeting', text: getGreeting() });
  ...
  private renderInterruptMessage(): void {
    const msgEl = this.messagesEl.createDiv({ cls: 'typorai-message typorai-message-assistant' });
    const contentEl = msgEl.createDiv({ cls: 'typorai-message-content', attr: { dir: 'auto' } });
    this.appendInterruptIndicator(contentEl);
  }
  ...
  private appendInterruptIndicator(contentEl: HTMLElement): void {
    const textEl = contentEl.createDiv({ cls: 'typorai-text-block' });
    textEl.createSpan({ cls: 'typorai-interrupted', text: t('renderer.message.interrupted') });
    textEl.appendText(' ');
    textEl.createSpan({
      cls: 'typorai-interrupted-hint',
      text: t('renderer.message.interruptedHint'),
    });
  }
  ...
  private renderAssistantContent(msg: ChatMessage, contentEl: HTMLElement): void {
    if (msg.contentBlocks && msg.contentBlocks.length > 0) {
      ...
      for (const block of msg.contentBlocks) {
        if (block.type === 'thinking') {
          ...
          const boundaryEl = contentEl.createDiv({ cls: 'typorai-compact-boundary' });
          boundaryEl.createSpan({ cls: 'typorai-compact-boundary-label', text: t('renderer.message.conversationCompacted') });
    ...
    if (msg.durationSeconds && msg.durationSeconds > 0 && !hasCompactBoundary) {
      const flavorWord = msg.durationFlavorWord || t('renderer.message.baked');
      const footerEl = contentEl.createDiv({ cls: 'typorai-response-footer' });
      footerEl.createSpan({
        text: t('renderer.message.bakedFor', { word: flavorWord, duration: formatDurationMmSs(msg.durationSeconds) }),
  ...
  private resolveTaskSubagent(toolCall: ToolCallInfo, modeHint?: 'sync' | 'async'): SubagentInfo {
    ...
    const description = (toolCall.input?.description as string) || t('renderer.message.subagentTask');
  ...
  renderMessageImages(containerEl: HTMLElement, images: ImageAttachment[]): void {
    const imagesEl = containerEl.createDiv({ cls: 'typorai-message-images' });

    for (const image of images) {
      const imageWrapper = imagesEl.createDiv({ cls: 'typorai-message-image' });
  ...
  /**
   * Renders a compact, single-line selection reference above a user message
   * bubble: just the truncated selected text. Click to expand and reveal the
   * full text.
   */
  renderMessageSelection(containerEl: HTMLElement, selection: ChatSelectionRef): HTMLElement {
    const wrapperEl = containerEl.createDiv({ cls: 'typorai-message-selection' });

    const headerEl = wrapperEl.createEl('button', {
      cls: 'typorai-message-selection-header',
      attr: { type: 'button' },
    });
    headerEl.setText(this.getSelectionPreviewText(selection.text));

    const contentEl = wrapperEl.createDiv({
      cls: 'typorai-message-selection-content typorai-hidden',
    });
    contentEl.createEl('pre', { cls: 'typorai-message-selection-pre', text: selection.text });

    const state: CollapsibleState = { isExpanded: false };
    setupCollapsible(wrapperEl, headerEl, contentEl, state, {
      baseAriaLabel: t('renderer.message.selectionAria'),
    });

    return wrapperEl;
  }

  private getSelectionPreviewText(selectedText: string): string {
    const compactText = selectedText.replace(/\s+/g, ' ').trim();
    if (compactText.length <= 96) {
      return compactText;
    }
  ...
  showFullImage(image: ImageAttachment): void {
    ...
    const overlay = ownerDocument.body.createDiv({ cls: 'typorai-image-modal-overlay' });
    const modal = overlay.createDiv({ cls: 'typorai-image-modal' });
    ...
    const closeBtn = modal.createDiv({ cls: 'typorai-image-modal-close' });
    closeBtn.setText('\u00D7');
  ...
  async renderContent(
    ...
    try {
      ...
      await renderMarkdownContent(processedMarkdown, el);

      // Wrap pre elements and move buttons outside scroll area
      el.querySelectorAll('pre').forEach((pre) => {
        // Skip if already wrapped
        if (pre.parentElement?.classList.contains('typorai-code-wrapper')) return;

        // Create wrapper
        const wrapper = createEl('div', { cls: 'typorai-code-wrapper' });
            ...
            label.addEventListener('click', () => {
              runRendererAction(async () => {
                ...
                try {
                  ...
                  label.setText(t('renderer.message.copied'));
        ...
        // Move Typora's copy button outside pre into wrapper
      ...
      el.createDiv({
        cls: 'typorai-render-error',
        text: t('renderer.message.failedToRender'),
      });
    }
  }
  ...
  addTextCopyButton(textEl: HTMLElement, markdown: string): void {
    const copyBtn = textEl.createSpan({ cls: 'typorai-text-copy-btn' });
    ...
    copyBtn.addEventListener('click', (e) => {
      ...
      runRendererAction(async () => {
        ...
        copyBtn.empty();
        copyBtn.setText(t('renderer.message.copied'));
  ...
  refreshActionButtons(msg: ChatMessage, allMessages?: ChatMessage[], index?: number): void {
    ...
    if (canRewind && this.rewindCallback && !msgEl.querySelector('.typorai-message-rewind-btn')) {
      this.addRewindButton(msgEl, msg.id);
    }
    if (canFork && this.forkCallback && !msgEl.querySelector('.typorai-message-fork-btn')) {
      this.addForkButton(msgEl, msg.id);
    }
  ...
  private cleanupLiveMessageEl(
    ...
    const needsRewind = expectedActions.canRewind
      && this.rewindCallback
      && !msgEl.querySelector('.typorai-message-rewind-btn');
    const needsFork = expectedActions.canFork
      && this.forkCallback
      && !msgEl.querySelector('.typorai-message-fork-btn');
  ...
  private getOrCreateActionsToolbar(msgEl: HTMLElement): HTMLElement {
    const existing = msgEl.querySelector<HTMLElement>('.typorai-user-msg-actions');
    if (existing) return existing;
    return msgEl.createDiv({ cls: 'typorai-user-msg-actions' });
  }

  private addUserCopyButton(msgEl: HTMLElement, content: string): void {
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const copyBtn = toolbar.createSpan({ cls: 'typorai-user-msg-copy-btn' });
    setIcon(copyBtn, 'copy');
    copyBtn.setAttribute('aria-label', t('renderer.message.copyMessageAria'));
  ...
  private addRewindButton(msgEl: HTMLElement, messageId: string): void {
    ...
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const btn = toolbar.createSpan({ cls: 'typorai-message-rewind-btn' });
        ...
        .onClick(() => {
          runRendererAction(async () => {
            try {
              ...
              new Notice(t('chat.rewind.failed', { error: err instanceof Error ? err.message : t('common.unknown') }));
            }
          });
        });
    });
  ...
  private addForkButton(msgEl: HTMLElement, messageId: string): void {
    ...
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const btn = toolbar.createSpan({ cls: 'typorai-message-fork-btn' });
    if (toolbar.firstChild !== btn) toolbar.insertBefore(btn, toolbar.firstChild);
    ...
    btn.addEventListener('click', (e) => {
      ...
      runRendererAction(async () => {
        try {
          ...
          new Notice(t('chat.fork.failed', { error: err instanceof Error ? err.message : t('common.unknown') }));
        }
      });
    });
  }
