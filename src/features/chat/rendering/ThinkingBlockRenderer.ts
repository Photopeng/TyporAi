...
export function createThinkingBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'typorai-thinking-block' });

  // Header (clickable to expand/collapse)
  const header = wrapperEl.createDiv({ cls: 'typorai-thinking-header' });
  header.setAttribute('tabindex', '0');
  header.setAttribute('role', 'button');
  header.setAttribute('aria-expanded', 'false');
  header.setAttribute('aria-label', t('renderer.thinking.ariaExpand'));

  // Label with timer
  const labelEl = header.createSpan({ cls: 'typorai-thinking-label' });
  const startTime = Date.now();
  labelEl.setText(t('renderer.thinking.elapsed', { seconds: 0 }));

  // Start timer interval to update label every second
  const timerInterval = window.setInterval(() => {
    const elapsed = Math.floor((Date.now() - startTime) / 1000);
    labelEl.setText(t('renderer.thinking.elapsed', { seconds: elapsed }));
  }, 1000);

  // Collapsible content (collapsed by default)
  const contentEl = wrapperEl.createDiv({ cls: 'typorai-thinking-content' });
...
export function finalizeThinkingBlock(state: ThinkingBlockState): number {
  ...
  state.labelEl.setText(t('renderer.thinking.thoughtFor', { seconds: durationSeconds }));

  // Collapse when done and sync state
  const header = state.wrapperEl.querySelector('.typorai-thinking-header');
  if (header) {
...
export function renderStoredThinkingBlock(
  ...
  // Label with duration
  const labelEl = header.createSpan({ cls: 'typorai-thinking-label' });
  const labelText = durationSeconds !== undefined ? t('renderer.thinking.thoughtFor', { seconds: durationSeconds }) : t('renderer.thinking.thought');
