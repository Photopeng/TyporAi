...
export function renderDiffContent(
  ...
  if (allInserts && diffLines.length > NEW_FILE_DISPLAY_CAP) {
    const hunkEl = containerEl.createDiv({ cls: 'typorai-diff-hunk' });
    for (const line of diffLines.slice(0, NEW_FILE_DISPLAY_CAP)) {
      const lineEl = hunkEl.createDiv({ cls: 'typorai-diff-line typorai-diff-insert' });
      const prefixEl = lineEl.createSpan({ cls: 'typorai-diff-prefix' });
      prefixEl.setText('+');
      const contentEl = lineEl.createSpan({ cls: 'typorai-diff-text' });
      contentEl.setText(line.text || ' ');
    }
    const remaining = diffLines.length - NEW_FILE_DISPLAY_CAP;
    const separator = containerEl.createDiv({ cls: 'typorai-diff-separator' });
    separator.setText(t('renderer.diff.moreLines', { count: remaining }));
    return;
  }
  ...
  if (hunks.length === 0) {
    // No changes
    const noChanges = containerEl.createDiv({ cls: 'typorai-diff-no-changes' });
    noChanges.setText(t('renderer.diff.noChanges'));
    return;
  }
  ...
  hunks.forEach((hunk, hunkIndex) => {
    ...
    for (const line of hunk.lines) {
      const lineEl = hunkEl.createDiv({ cls: `typorai-diff-line typorai-diff-${line.type}` });
