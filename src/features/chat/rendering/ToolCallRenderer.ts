  ...
  switch (name) {
    case TOOL_TODO_WRITE: {
      ...
      if (todos && Array.isArray(todos) && todos.length > 0) {
        ...
        return t('renderer.tool.tasksProgress', { completed, total: todos.length });
      }
      return t('renderer.tool.tasks');
    }
    case TOOL_ENTER_PLAN_MODE:
      return t('renderer.tool.enteringPlanMode');
    case TOOL_EXIT_PLAN_MODE:
      return t('renderer.tool.planComplete');
    default:
      return name;
  }
}
...
export function getToolLabel(name: string, input: Record<string, unknown>): string {
  switch (name) {
    case TOOL_READ:
      return t('renderer.tool.readFile', { path: shortenPath(getInputText(input, 'file_path')) || t('renderer.tool.file') });
    case TOOL_WRITE:
      return t('renderer.tool.writeFile', { path: shortenPath(getInputText(input, 'file_path')) || t('renderer.tool.file') });
    case TOOL_EDIT:
      return t('renderer.tool.editFile', { path: shortenPath(getInputText(input, 'file_path')) || t('renderer.tool.file') });
    case TOOL_BASH: {
      const cmd = getInputText(input, 'command', 'command');
      return t('renderer.tool.command', { command: cmd.length > 40 ? cmd.substring(0, 40) + '...' : cmd });
    }
    case TOOL_GLOB:
      return t('renderer.tool.toolSearchLabel', { pattern: getInputText(input, 'pattern', 'files') });
    case TOOL_GREP:
      return t('renderer.tool.pattern', { pattern: getInputText(input, 'pattern', 'pattern') });
    case TOOL_WEB_SEARCH: {
      return getWebSearchLabel(input, 40);
    }
    case TOOL_WEB_FETCH: {
      const url = getInputText(input, 'url', 'url');
      return t('renderer.tool.url', { url: url.length > 40 ? url.substring(0, 40) + '...' : url });
    }
    case TOOL_LS:
      return t('renderer.tool.list', { path: shortenPath(getInputText(input, 'path')) || '.' });
    case TOOL_TODO_WRITE: {
      const todos = input.todos as Array<{ status: string }> | undefined;
    ...
    case TOOL_SKILL: {
      ...
      return t('renderer.tool.skill', { name: skillName });
    }
    case TOOL_TOOL_SEARCH: {
      const tools = parseToolSearchQuery(getInputText(input, 'query'));
      return t('renderer.tool.toolSearch', { query: tools || t('renderer.tool.tools') });
    }
    case TOOL_ENTER_PLAN_MODE:
      return t('renderer.tool.enteringPlanMode');
    case TOOL_EXIT_PLAN_MODE:
...
function getAgentLifecycleSummary(name: string, input: Record<string, unknown>): string {
  switch (name) {
    ...
    case 'wait': {
      ...
      const parts: string[] = [];
      if (ids > 0) parts.push(t('renderer.tool.agents', { count: ids, plural: ids === 1 ? '' : 's' }));
...
function getWebSearchSummary(input: Record<string, unknown>, maxLength: number): string {
  ...
  switch (data.actionType) {
    case 'open_page':
      return truncateText(`${t('renderer.tool.openPage')}: ${data.url || 'page'}`, maxLength);
    case 'find_in_page': {
      const target = data.pattern ? t('renderer.tool.findInPageWithPattern', { pattern: data.pattern }) : t('renderer.tool.findInPage');
...
function getWebSearchLabel(input: Record<string, unknown>, maxLength: number): string {
  ...
  return t('renderer.tool.webSearch', { query: summary || t('renderer.tool.search') });
}

function appendToolLink(parent: HTMLElement, title: string, url: string): void {
  const linkEl = parent.createEl('a', { cls: 'typorai-tool-link' });
  linkEl.setAttribute('href', url);
  linkEl.setAttribute('target', '_blank');
  linkEl.setAttribute('rel', 'noopener noreferrer');

  const iconEl = linkEl.createSpan({ cls: 'typorai-tool-link-icon' });
  setIcon(iconEl, 'external-link');

  linkEl.createSpan({ cls: 'typorai-tool-link-title', text: title });
}
...
function renderWebSearchActionExpanded(container: HTMLElement, input: Record<string, unknown>): boolean {
  ...
  const linesEl = container.createDiv({ cls: 'typorai-tool-lines' });

  switch (data.actionType) {
    case 'open_page':
      linesEl.createDiv({ cls: 'typorai-tool-line', text: t('renderer.tool.openPage') });
      if (data.url) {
        appendToolLink(linesEl, data.url, data.url);
      } else {
        linesEl.createDiv({ cls: 'typorai-tool-line', text: t('renderer.tool.urlUnavailable') });
      }
      return true;

    case 'find_in_page':
      linesEl.createDiv({ cls: 'typorai-tool-line', text: t('renderer.tool.findInPage') });
      if (data.url) {
        appendToolLink(linesEl, data.url, data.url);
      } else {
        linesEl.createDiv({ cls: 'typorai-tool-line', text: t('renderer.tool.urlUnavailable') });
      ...
      if (data.pattern) {
        linesEl.createDiv({ cls: 'typorai-tool-line', text: t('renderer.tool.patternLabel', { pattern: data.pattern }) });
      }
      return true;
    ...
    default: {
      ...
      linesEl.createDiv({
        cls: 'typorai-tool-line',
        text: primaryQuery ? t('renderer.tool.queryLabel', { query: primaryQuery }) : t('renderer.tool.webSearch', { query: '' }),
      });

      const alternateQueries = data.queries.filter(query => query !== primaryQuery);
      for (const query of alternateQueries.slice(0, 4)) {
        linesEl.createDiv({ cls: 'typorai-tool-line', text: t('renderer.tool.altQueryLabel', { query }) });
      }
      if (alternateQueries.length > 4) {
        linesEl.createDiv({
          cls: 'typorai-tool-truncated',
          text: t('renderer.tool.moreQueries', { count: alternateQueries.length - 4 }),
        });
      }
      return true;
    }
...
function renderWebSearchExpanded(
  ...
  if (parsed && parsed.links.length > 0) {
    const linksEl = container.createDiv({ cls: 'typorai-tool-lines' });
    for (const link of parsed.links) {
      appendToolLink(linksEl, link.title, link.url);
    }

    if (parsed.summary) {
      const summaryEl = container.createDiv({ cls: 'typorai-tool-web-summary' });
  ...
  container.createDiv({ cls: 'typorai-tool-empty', text: t('renderer.tool.noResult') });
}

function renderFileSearchExpanded(container: HTMLElement, result: string): void {
  const lines = result.split(/\r?\n/).filter(line => line.trim());
  if (lines.length === 0) {
    container.createDiv({ cls: 'typorai-tool-empty', text: t('renderer.tool.noMatchesFound') });
    return;
  }
...
function renderLinesExpanded(
  ...
  for (const line of displayLines) {
    ...
    const lineEl = linesEl.createDiv({ cls: 'typorai-tool-line' });
  ...
  if (truncated) {
    linesEl.createDiv({
      cls: 'typorai-tool-truncated',
      text: t('renderer.tool.moreLines', { count: lines.length - maxLines }),
    });
  }
}
...
function renderToolSearchExpanded(container: HTMLElement, result: string): void {
  ...
  for (const name of toolNames) {
    const lineEl = container.createDiv({ cls: 'typorai-tool-search-item' });
    const iconEl = lineEl.createSpan({ cls: 'typorai-tool-search-icon' });
...
function renderWebFetchExpanded(container: HTMLElement, result: string): void {
  const maxChars = 500;
  const linesEl = container.createDiv({ cls: 'typorai-tool-lines' });
  const lineEl = linesEl.createDiv({ cls: 'typorai-tool-line typorai-tool-line-wrap' });

  if (result.length > maxChars) {
    lineEl.setText(result.slice(0, maxChars));
    linesEl.createDiv({
      cls: 'typorai-tool-truncated',
      text: t('renderer.tool.moreChars', { count: result.length - maxChars }),
    });
  } else {
...
function renderApplyPatchExpanded(
  ...
  if (changes.length > 0) {
    ...
    for (const change of changes as unknown[]) {
      ...
      linesEl.createDiv({ cls: 'typorai-tool-line', text: pathText });
    }
    return;
  }
...
function renderApplyPatchDiffSections(
  ...
  for (const fileDiff of fileDiffs) {
    const sectionEl = container.createDiv({ cls: 'typorai-tool-patch-section' });

    if (fileDiff.operation === 'delete' && fileDiff.diffLines.length === 0) {
      sectionEl.createDiv({ cls: 'typorai-tool-empty', text: t('renderer.tool.fileDeleted') });
      continue;
    }

    if (fileDiff.diffLines.length === 0) {
      sectionEl.createDiv({ cls: 'typorai-tool-empty', text: t('renderer.tool.noTextualDiff') });
      continue;
    }

    const diffRow = sectionEl.createDiv({ cls: 'typorai-write-edit-diff-row' });
    const diffEl = diffRow.createDiv({ cls: 'typorai-write-edit-diff' });
...
function getDiffStatsAriaLabel(stats: DiffStats): string {
  return t('renderer.tool.changesAria', { added: stats.added, removed: stats.removed });
}
...
function resetStatusElement(statusEl: HTMLElement, statusClass: string, ariaLabel: string): void {
  statusEl.className = 'typorai-tool-status';
  statusEl.empty();
...
function setTodoWriteStatus(statusEl: HTMLElement, input: Record<string, unknown>): void {
  ...
  const ariaLabel = isComplete ? t('renderer.tool.statusCompleted') : t('renderer.tool.statusInProgress');
...
function setToolStatus(statusEl: HTMLElement, status: ToolCallInfo['status']): void {
  resetStatusElement(statusEl, `status-${status}`, t('renderer.tool.statusLabel', { status }));
...
function setApplyPatchHeaderRight(statusEl: HTMLElement, toolCall: ToolCallInfo): void {
  ...
  statusEl.className = 'typorai-tool-status typorai-write-edit-stats';
  statusEl.empty();
...
export function renderTodoWriteResult(
  ...
  container.empty();
  container.addClass('typorai-todo-panel-content');
  container.addClass('typorai-todo-list-container');

  const todos = input.todos as TodoItem[] | undefined;
  if (!todos || !Array.isArray(todos)) {
    const item = container.createSpan({ cls: 'typorai-tool-result-item' });
    item.setText(t('renderer.tool.tasksUpdated'));
    return;
  }
...
function createToolElementStructure(
  ...
  const toolEl = parentEl.createDiv({ cls: 'typorai-tool-call' });
  if (toolCall.name === TOOL_BASH) {
    toolEl.addClass('typorai-tool-call-bash');
  }

  const header = toolEl.createDiv({ cls: 'typorai-tool-header' });
  header.setAttribute('tabindex', '0');
  header.setAttribute('role', 'button');

  const iconEl = header.createSpan({ cls: 'typorai-tool-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setToolIcon(iconEl, toolCall.name);

  const nameEl = header.createSpan({ cls: 'typorai-tool-name' });
  nameEl.setText(getToolName(toolCall.name, toolCall.input));

  const summaryEl = header.createSpan({ cls: 'typorai-tool-summary' });
  ...
  const statusEl = header.createSpan({ cls: 'typorai-tool-status' });

  const content = toolEl.createDiv({ cls: 'typorai-tool-content' });
...
function renderAskUserQuestionResult(container: HTMLElement, toolCall: ToolCallInfo): boolean {
  ...
  const reviewEl = container.createDiv({ cls: 'typorai-ask-review' });
  for (let i = 0; i < questions.length; i++) {
    const q = questions[i];
    const answer = formatAnswer(
      (q.id ? answers[q.id] : undefined) ?? answers[q.question]
    );
    const pairEl = reviewEl.createDiv({ cls: 'typorai-ask-review-pair' });
    pairEl.createDiv({ text: `${i + 1}.`, cls: 'typorai-ask-review-num' });
    const bodyEl = pairEl.createDiv({ cls: 'typorai-ask-review-body' });
    bodyEl.createDiv({ text: q.question, cls: 'typorai-ask-review-q-text' });
    bodyEl.createDiv({
      text: answer || t('renderer.tool.notAnswered'),
      cls: answer ? 'typorai-ask-review-a-text' : 'typorai-ask-review-empty',
    });
  }
...
function renderAskUserQuestionFallback(container: HTMLElement, toolCall: ToolCallInfo, initialText?: string): void {
  ...
  if (questions.length === 0) {
    contentFallback(container, initialText || toolCall.result || t('renderer.tool.waitingForAnswer'));
    return;
  }

  if (initialText || toolCall.result) {
    container.createDiv({
      cls: 'typorai-ask-review-prompt',
      text: initialText || toolCall.result || t('renderer.tool.waitingForAnswer'),
    });
  }
  ...
  for (let questionIndex = 0; questionIndex < questions.length; questionIndex++) {
    ...
    const reviewEl = container.createDiv({ cls: 'typorai-ask-review' });
    const pairEl = reviewEl.createDiv({ cls: 'typorai-ask-review-pair' });
    pairEl.createDiv({ text: `${questionIndex + 1}.`, cls: 'typorai-ask-review-num' });
    const bodyEl = pairEl.createDiv({ cls: 'typorai-ask-review-body' });
    bodyEl.createDiv({ text: question.question, cls: 'typorai-ask-review-q-text' });

    if (!Array.isArray(question.options) || question.options.length === 0) {
      bodyEl.createDiv({ cls: 'typorai-ask-review-empty', text: t('renderer.tool.noOptionsRecorded') });
      continue;
    }

    const listEl = bodyEl.createDiv({ cls: 'typorai-ask-list' });
...
function renderAskUserQuestionOption(
  ...
  const itemEl = parentEl.createDiv({ cls: 'typorai-ask-item is-disabled' });

  if (isMultiSelect) {
    itemEl.createDiv({ cls: 'typorai-ask-check', text: '[ ] ' });
  } else {
    itemEl.createDiv({ cls: 'typorai-ask-item-num', text: `${optionIndex + 1}. ` });
  }

  const contentEl = itemEl.createDiv({ cls: 'typorai-ask-item-content' });
  const labelRowEl = contentEl.createDiv({ cls: 'typorai-ask-label-row' });
  labelRowEl.createDiv({ cls: 'typorai-ask-item-label', text: option.label });

  if (option.description) {
    contentEl.createDiv({ cls: 'typorai-ask-item-desc', text: option.description });
  }
}

function contentFallback(container: HTMLElement, text: string): void {
  const resultRow = container.createDiv({ cls: 'typorai-tool-result-row' });
  const resultText = resultRow.createSpan({ cls: 'typorai-tool-result-text' });
...
function renderBashContent(
  ...
  if (command) {
    const cmdEl = container.createDiv({ cls: 'typorai-tool-bash-command' });
    cmdEl.setText(t('renderer.tool.bashPrefix', { command }));
  }
  if (initialText) {
...
function createCurrentTaskPreview(
  ...
  const currentTaskEl = header.createSpan({ cls: 'typorai-tool-current' });
...
function createTodoToggleHandler(
  ...
  return (expanded: boolean) => {
    ...
    if (currentTaskEl) {
      currentTaskEl.toggleClass('typorai-hidden', expanded);
    }
    if (statusEl) {
      statusEl.toggleClass('typorai-hidden', expanded);
    }
  };
}
...
function renderToolContent(
  ...
  if (toolCall.name === TOOL_TODO_WRITE) {
    content.addClass('typorai-tool-content-todo');
    renderTodoWriteResult(content, toolCall.input);
  } else if (toolCall.name === TOOL_ASK_USER_QUESTION) {
    content.addClass('typorai-tool-content-ask');
    if (initialText) {
      renderAskUserQuestionFallback(content, toolCall, t('renderer.tool.waitingForAnswer'));
...
export function renderToolCall(
  ...
  renderToolContent(content, toolCall, t('renderer.tool.running'));
...
export function updateToolCallResult(
  ...
  if (toolCall.name === TOOL_TODO_WRITE) {
    const statusEl = toolEl.querySelector('.typorai-tool-status') as HTMLElement;
    if (statusEl) {
      setTodoWriteStatus(statusEl, toolCall.input);
    }
    const content = toolEl.querySelector('.typorai-tool-content') as HTMLElement;
    if (content) {
      renderTodoWriteResult(content, toolCall.input);
    }
    const nameEl = toolEl.querySelector('.typorai-tool-name') as HTMLElement;
    if (nameEl) {
      nameEl.setText(getToolName(toolCall.name, toolCall.input));
    }
    const currentTaskEl = toolEl.querySelector('.typorai-tool-current') as HTMLElement;
    if (currentTaskEl) {
...
/** Stored, non-streaming tool calls are collapsed by default. */
