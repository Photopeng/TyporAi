...
export function createWriteEditBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'typorai-write-edit-block' });
  wrapperEl.dataset.toolId = toolCall.id;

  // Header (clickable to collapse/expand)
  const headerEl = wrapperEl.createDiv({ cls: 'typorai-write-edit-header' });
  headerEl.setAttribute('tabindex', '0');
  headerEl.setAttribute('role', 'button');

  // File icon
  const iconEl = headerEl.createDiv({ cls: 'typorai-write-edit-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setIcon(iconEl, getToolIcon(toolName));

  const nameEl = headerEl.createDiv({ cls: 'typorai-write-edit-name' });
  nameEl.setText(toolName);
  const summaryEl = headerEl.createDiv({ cls: 'typorai-write-edit-summary' });
  summaryEl.setText(fileNameOnly(filePath) || 'file');

  // Populated when diff is computed
  const statsEl = headerEl.createDiv({ cls: 'typorai-write-edit-stats' });

  const statusEl = headerEl.createDiv({ cls: 'typorai-write-edit-status status-running' });
  statusEl.setAttribute('aria-label', t('renderer.writeEdit.statusRunning'));

  // Content area (collapsed by default)
  const contentEl = wrapperEl.createDiv({ cls: 'typorai-write-edit-content' });

  // Initial loading state
  const loadingRow = contentEl.createDiv({ cls: 'typorai-write-edit-diff-row' });
  const loadingEl = loadingRow.createDiv({ cls: 'typorai-write-edit-loading' });
  loadingEl.setText(t('renderer.writeEdit.writing'));
...
export function updateWriteEditWithDiff(state: WriteEditState, diffData: ToolDiffData): void {
  ...
  // Render diff content
  const row = state.contentEl.createDiv({ cls: 'typorai-write-edit-diff-row' });
  const diffEl = row.createDiv({ cls: 'typorai-write-edit-diff' });
...
export function finalizeWriteEditBlock(state: WriteEditState, isError: boolean): void {
  ...
  state.statusEl.className = 'typorai-write-edit-status';
  state.statusEl.empty();

  if (isError) {
    state.statusEl.addClass('status-error');
    setIcon(state.statusEl, 'x');
    state.statusEl.setAttribute('aria-label', t('renderer.writeEdit.statusError'));

    // Show error in content if no diff was shown
    if (!state.diffLines) {
      state.contentEl.empty();
      const row = state.contentEl.createDiv({ cls: 'typorai-write-edit-diff-row' });
      const errorEl = row.createDiv({ cls: 'typorai-write-edit-error' });
      errorEl.setText(state.toolCall.result || t('renderer.writeEdit.defaultError'));
    }
  } else if (!state.diffLines) {
    // Success but no diff data - clear the "Writing..." loading text and show DONE
    state.contentEl.empty();
    const row = state.contentEl.createDiv({ cls: 'typorai-write-edit-diff-row' });
    const doneEl = row.createDiv({ cls: 'typorai-write-edit-done-text' });
    doneEl.setText(t('renderer.writeEdit.done'));
  }
...
export function renderStoredWriteEdit(
  ...
  const statusEl = headerEl.createDiv({ cls: 'typorai-write-edit-status' });
  if (isError) {
  ...
  const row = contentEl.createDiv({ cls: 'typorai-write-edit-diff-row' });
  ...
  if (toolCall.diffData && toolCall.diffData.diffLines.length > 0) {
    ...
    const doneEl = row.createDiv({ cls: 'typorai-write-edit-done-text' });
    doneEl.setText(isError ? t('renderer.writeEdit.error') : t('renderer.writeEdit.done'));
  }
