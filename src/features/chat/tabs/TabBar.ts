...
export class TabBar {
  ...
  private build(): void {
    this.containerEl.addClass('typorai-tab-badges');
  ...
  private renderBadge(item: TabBarItem): void {
    ...
    let stateClass = 'typorai-tab-badge-idle';
    if (item.isActive) {
      stateClass = 'typorai-tab-badge-active';
    } else if (item.needsAttention) {
      stateClass = 'typorai-tab-badge-attention';
    } else if (item.isStreaming) {
      stateClass = 'typorai-tab-badge-streaming';
    }
    ...
    const badgeEl = this.containerEl.createDiv({
      cls: [
        'typorai-tab-badge',
        stateClass,
        isTitleExpanded ? 'typorai-tab-badge-expanded' : '',
      ].filter(Boolean).join(' '),
      text: this.getBadgeLabel(item),
    });

    // Typora uses aria-label for hover tooltips here; adding title causes duplicate tooltip text.
  ...
  destroy(): void {
    this.containerEl.empty();
    this.containerEl.removeClass('typorai-tab-badges');
  ...
  private toggleBadgeTitle(item: TabBarItem, badgeEl: HTMLElement): void {
    ...
    badgeEl.toggleClass('typorai-tab-badge-expanded', isTitleExpanded);
    badgeEl.setAttribute('data-title-expanded', isTitleExpanded ? 'true' : 'false');
  }
