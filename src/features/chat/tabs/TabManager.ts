...
export class TabManager implements TabManagerInterface {
  ...
  private runtimeFactory?: (options: CreateChatRuntimeOptions) => ChatRuntime;
  private providerServiceFactory?: ProviderServiceFactory;
  private processTransport?: ProcessTransportFactory;
  private platform?: PlatformInfo['operatingSystem'];
  private commandRegistry?: CommandRegistry;
  private documentSnapshot?: () => TyporaDocumentSnapshot | null;
  ...
  setRuntimeFactory(factory: (options: CreateChatRuntimeOptions) => ChatRuntime): void {
    ...
    for (const tab of this.tabs.values()) tab.runtimeFactory = factory;
  }

  setProviderServiceFactory(factory: ProviderServiceFactory): void {
    this.providerServiceFactory = factory;
    for (const tab of this.tabs.values()) tab.providerServiceFactory = factory;
  }

  setProcessTransport(processTransport: ProcessTransportFactory): void {
    this.processTransport = processTransport;
    for (const tab of this.tabs.values()) tab.processTransport = processTransport;
  }

  setPlatform(platform: PlatformInfo['operatingSystem']): void {
    this.platform = platform;
    for (const tab of this.tabs.values()) tab.platform = platform;
  }

  setCommandRegistry(commandRegistry: CommandRegistry): void {
    this.commandRegistry = commandRegistry;
    for (const tab of this.tabs.values()) tab.commandRegistry = commandRegistry;
  }

  setDocumentSnapshotProvider(provider: () => TyporaDocumentSnapshot | null): void {
    this.documentSnapshot = provider;
    for (const tab of this.tabs.values()) tab.documentSnapshot = provider;
  }

  setFileWatchService(fileWatchService: FileWatchService): void {
    this.fileWatchService = fileWatchService;
    for (const tab of this.tabs.values()) tab.fileWatchService = fileWatchService;
  }

  setNotificationService(notificationService: NotificationService): void {
    this.notificationService = notificationService;
    for (const tab of this.tabs.values()) tab.notificationService = notificationService;
  }
  ...
  async createTab(
    ...
    // Inherit a blank active tab's provider so deliberate draft model selection
    // carries forward. From a bound conversation, new blank tabs follow the
    // global settings provider instead of the historical conversation provider.
    const activeTab = this.getActiveTab();
    const settingsProviderId = ProviderRegistry.resolveSettingsProviderId(this.plugin.settings);
    const defaultProviderId = conversation
      ? undefined
      : (activeTab?.lifecycleState === 'blank'
    ...
    tab.runtimeFactory = this.runtimeFactory;
    tab.providerServiceFactory = this.providerServiceFactory;
    tab.processTransport = this.processTransport;
    tab.platform = this.platform;
    tab.commandRegistry = this.commandRegistry;
    tab.documentSnapshot = this.documentSnapshot;
    tab.fileWatchService = this.fileWatchService;
    tab.notificationService = this.notificationService;

    // Mirror the conversation's external context paths into the tab state
    // so the file context manager shows them immediately (before the
    // runtime's first applyDynamicUpdates).
    tab.state.setCurrentExternalContextPaths?.(
  ...
  retargetBlankTabsToProvider(providerId: ProviderId): number {
    if (!ProviderRegistry.hasProvider(providerId) || !ProviderRegistry.isEnabled(providerId, this.plugin.settings)) {
      return 0;
    }
    ...
    for (const tab of this.tabs.values()) {
      ...
      if (previousProviderId !== tab.providerId || previousDraftModel !== tab.draftModel) {
        changed++;
    ...
    if (changed > 0) {
      this.invalidateProviderCommandCaches();
      this.primeProviderRuntime(providerId);
    }
  ...
  private async handleForkRequest(context: ForkContext): Promise<void> {
    ...
    if (target === 'new-tab') {
      ...
      if (!tab) {
        const maxTabs = this.getMaxTabs();
        this.notificationService.show(t('chat.fork.maxTabsReached', { count: String(maxTabs) }), 'warning');
        return;
      }
      this.notificationService.show(t('chat.fork.notice'), 'info');
    } else {
      const success = await this.forkInCurrentTab(context);
      if (!success) {
        this.notificationService.show(t('chat.fork.failed', { error: t('chat.fork.errorNoActiveTab') }), 'error');
        return;
      }
      this.notificationService.show(t('chat.fork.noticeCurrentTab'), 'info');
    }
  }
  ...
  private async ensureProviderTabRuntimeReady(
    ...
    runtime.syncConversationState(context.conversation, context.conversation?.externalContextPaths ?? []);
