import { isCliProviderConfigured } from '../../../core/providers/cliProviderSelection';
...
import { decodeProviderModelSelectionId } from '../../../core/providers/modelSelection';
...
import { isDocumentEditingAllowed, isDocumentMutationTool } from '../../../core/security/documentEditing';
import { TOOL_AGENT_OUTPUT } from '../../../core/tools/toolNames';
import type { ChatMessage, Conversation, StreamChunk,TyporAiSettings } from '../../../core/types';
...
import { getTabProviderId, resolveRegisteredProviderId } from './providerResolution';
import type { TabData, TabDOMElements, TabId, TabManagerViewHost, TabProviderContext } from './types';
import { generateTabId } from './types';

// Selection polling runs every 250ms (see SelectionController.SELECTION_POLL_INTERVAL),
// so a single mouse drag can fire onNewSelection several times as the range
// grows. Cursor flow waits for the selection to be stable for this long
// before stealing focus, so it never interrupts an in-progress drag.
const CURSOR_FLOW_FOCUS_DELAY_MS = 400;

interface TabLifecycleHost {
...
function getSharedSelectionFocusScopeEls(component: TabLifecycleHost): HTMLElement[] {
...
export function getBlankTabModelOptions(
  ...
  return ProviderRegistry.getEnabledProviderIds(settings).flatMap((providerId) => {
    if (
      providerId !== 'typora'
      && ProviderRegistry.getCapabilities(providerId).supportsPersistentRuntime
      && !isCliProviderConfigured(settings, providerId)
    ) {
      return [];
    }
...
function resolveBlankTabModel(
  ...
  const targetProviderId = ProviderRegistry.hasProvider(providerId)
...
function resolveRestoredBlankTabModel(
  plugin: TyporAiPlugin,
  restoredDraftModel: string,
  defaultProviderId?: ProviderId,
): string {
  const settings = plugin.settings as unknown as Record<string, unknown>;
  if (!restoredDraftModel) {
    return resolveBlankTabModel(plugin, defaultProviderId);
  }

  const restoredProviderId = getEnabledProviderForModel(restoredDraftModel, settings);
  const decodedSelection = decodeProviderModelSelectionId(restoredDraftModel);
  const explicitlyTargetsProvider = decodedSelection?.providerId === restoredProviderId
    || restoredDraftModel.startsWith(`${restoredProviderId}:`);
  const restoredProviderOwnsModel = ProviderRegistry
    .getChatUIConfig(restoredProviderId)
    .ownsModel(restoredDraftModel, settings);
  if (
    ProviderRegistry.isEnabled(restoredProviderId, settings)
    && (restoredProviderOwnsModel || explicitlyTargetsProvider)
  ) {
    return restoredDraftModel;
  }

  return resolveBlankTabModel(plugin, ProviderRegistry.resolveSettingsProviderId(settings));
}
...
function sendTabInputMessage(
  ...
  e.preventDefault();
  if (getRegisteredCommandRegistry(tab.commandRegistry, 'conversation.send')) {
    void executeRegisteredCommand(tab.commandRegistry, 'conversation.send')
      .catch(() => tab.notificationService.show(t('chat.input.sendFailed'), 'error'));
    return true;
  }
...
function refreshTabProviderUI(tab: TabData, plugin: TyporAiPlugin): void {
...
function syncTabProviderServices(
  ...
  const providerId = resolveEditModeProviderId(plugin, tab.providerId) ?? 'typora';
  tab.services.instructionRefineService = tab.providerServiceFactory
    ? tab.providerServiceFactory.createInstructionRefineService(plugin, providerId)
    : ProviderRegistry.createInstructionRefineService(plugin, providerId);
  tab.services.subagentManager.setTaskResultInterpreter?.(
    ProviderRegistry.getTaskResultInterpreter(tab.providerId)
  );
}

function assertChatProviderAvailable(
  plugin: TyporAiPlugin,
  providerId: ProviderId,
): void {
  const settings = plugin.settings as unknown as Record<string, unknown>;
  ...
  const isAvailable = capabilities.supportsPersistentRuntime
    ? isCliProviderConfigured(settings, providerId)
    : ProviderRegistry.isEnabled(providerId, settings);

  if (!isAvailable) {
    throw new Error(t('chat.input.providerUnavailable', { provider: displayName }));
  }
}

function ensureTitleGenerationService(tab: TabData, plugin: TyporAiPlugin): void {
  if (!tab.services.titleGenerationService) {
    tab.services.titleGenerationService = tab.providerServiceFactory
      ? tab.providerServiceFactory.createTitleGenerationService(plugin)
 ...
 */
export function onProviderAvailabilityChanged(tab: TabData, plugin: TyporAiPlugin): void {
...
export function createTab(options: TabCreateOptions): TabData {
  ...
  const contentEl = containerEl.createDiv({ cls: 'typorai-tab-content typorai-hidden' });
  ...
  const draftModel = isBound
    ? null
    : resolveRestoredBlankTabModel(plugin, restoredDraftModel, options.defaultProviderId);
  ...
  const tab: TabData = {
    ...
    service: null,
    notificationService: new NoticeAdapter(),
...
function buildTabDOM(contentEl: HTMLElement): TabDOMElements {
  const messagesWrapperEl = contentEl.createDiv({ cls: 'typorai-messages-wrapper' });
  const messagesEl = messagesWrapperEl.createDiv({ cls: 'typorai-messages' });
  const welcomeEl = messagesEl.createDiv({ cls: 'typorai-welcome' });
  const statusPanelContainerEl = contentEl.createDiv({ cls: 'typorai-status-panel-container' });
  const inputComposerEl = contentEl.createDiv({ cls: 'typorai-input-composer' });
  const inputContainerEl = inputComposerEl.createDiv({ cls: 'typorai-input-container' });
  const queueIndicatorEl = inputContainerEl.createDiv({ cls: 'typorai-input-queue-row' });
  const navRowEl = inputContainerEl.createDiv({ cls: 'typorai-input-nav-row' });
  const inputWrapper = inputContainerEl.createDiv({ cls: 'typorai-input-wrapper' });
  const contextRowEl = inputWrapper.createDiv({ cls: 'typorai-context-row' });
...
export async function initializeTabService(
  ...
  const providerId = getTabProviderId(tab, plugin, conversation);
  assertChatProviderAvailable(plugin, providerId);
  ...
  try {
    ...
    const runtimeOptions: CreateChatRuntimeOptions = {
      plugin,
      providerId,
      onExternalContextPathsChanged: (paths) => {
        // Mirror the latest paths into the tab state so the file
        // context manager shows them immediately (even before the
        // runtime finishes restarting on the next applyDynamicUpdates).
        tab.state.setCurrentExternalContextPaths(paths);
      },
    };
    const runtime = tab.runtimeFactory
      ? tab.runtimeFactory(runtimeOptions)
      : ProviderRegistry.createChatRuntime(runtimeOptions);
    service = runtime;
...
function initializeContextManagers(tab: TabData, plugin: TyporAiPlugin): void {
  const { dom } = tab;
  ...
  tab.ui.fileContextManager = new FileContextManager(
    ...
    dom.inputEl,
    {
      ...
      getExternalContexts: () => tab.state.currentExternalContextPaths,
    },
    dom.inputContainerEl,
    tab.fileWatchService,
    tab.notificationService,
  );
 ...
 */
function initializeInstructionAndTodo(tab: TabData, plugin: TyporAiPlugin): void {
  const { dom } = tab;
  ...
  if (isBangBashEnabled(plugin.settings)) {
    ...
    if (vaultPath) {
      ...
      if (!tab.processTransport) {
        throw new Error('Process transport is required for bang-bash execution');
      }
      const bashService = new BangBashService(vaultPath, enhancedPath, tab.processTransport, undefined, tab.platform);

      tab.ui.bangBashModeManager = new BangBashModeManagerClass(
        dom.inputEl,
        {
          notificationService: tab.notificationService,
  ...
  tab.ui.statusPanel = new StatusPanel(tab.notificationService);
...
function initializeInputToolbar(
  ...
  const inputToolbar = dom.inputWrapper.createDiv({ cls: 'typorai-input-toolbar' });
  ...
  const toolbarComponents = createInputToolbar(inputToolbar, {
    notifications: tab.notificationService,
    getUIConfig: () => {
      if (tab.lifecycleState === 'blank') {
    ...
    onModelChange: async (model: string) => {
      ...
      if (modelProvider !== boundProvider) {
        tab.notificationService.show(t('chat.tab.cannotSwitchProvider'), 'warning');
        tab.ui.modelSelector?.updateDisplay();
        return;
      }
  ...
  tab.ui.cursorFlowToggle = toolbarComponents.cursorFlowToggle;
  ...
  // Wire cursor flow toggle
  tab.ui.cursorFlowToggle?.onChange((enabled) => {
    plugin.settings.cursorFlowEnabled = enabled;
    void plugin.saveSettings();
  });
  tab.ui.cursorFlowToggle?.setEnabled(plugin.settings.cursorFlowEnabled);
...
export function initializeTabUI(
  ...
  // Selection preview - independent row above the bordered input wrapper.
  dom.selectionIndicatorEl = dom.inputContainerEl.createDiv({ cls: 'typorai-selection-preview' });
  dom.inputContainerEl.insertBefore(dom.selectionIndicatorEl, dom.inputWrapper);
  dom.selectionIndicatorEl.createSpan({ cls: 'typorai-selection-preview-text' });
  const selectionClearBtn = dom.selectionIndicatorEl.createEl('button', {
    cls: 'typorai-selection-preview-clear',
    attr: {
      type: 'button',
  ...
  setTyporAiTooltip(selectionClearBtn, t('chat.selection.clearQuote'));

  dom.browserIndicatorEl = dom.contextRowEl.createDiv({ cls: 'typorai-browser-selection-indicator typorai-hidden' });

  dom.canvasIndicatorEl = dom.contextRowEl.createDiv({ cls: 'typorai-canvas-indicator typorai-hidden' });

  dom.eventCleanups.push(() => selectionClearBtn.remove());
 ...
 */
function resolveForkSource(tab: TabData, plugin: TyporAiPlugin): ForkSource | null {
  ...
  const persistedProviderId = getTabProviderId(tab, plugin, conversation);
  const sourceSessionId = tab.service
    ? tab.service.resolveSessionIdForFork(conversation ?? null)
    : ProviderRegistry
      .getConversationHistoryService(persistedProviderId)
      .resolveSessionIdForConversation(conversation);

  if (!sourceSessionId) {
    tab.notificationService.show(t('chat.fork.failed', { error: t('chat.fork.errorNoSession') }), 'error');
    return null;
  }
...
async function handleForkRequest(
  ...
  if (!getTabCapabilities(tab, plugin).supportsFork) {
    tab.notificationService.show(t('chat.fork.notSupported'), 'warning');
    return;
  }

  if (state.isStreaming) {
    tab.notificationService.show(t('chat.fork.unavailableStreaming'), 'warning');
    return;
  }
  ...
  if (userIdx === -1) {
    tab.notificationService.show(t('chat.fork.failed', { error: t('chat.fork.errorMessageNotFound') }), 'error');
    return;
  }

  if (!msgs[userIdx].userMessageId) {
    tab.notificationService.show(t('chat.fork.unavailableNoUuid'), 'warning');
    return;
  }

  const rewindCtx = findRewindContext(msgs, userIdx);
  if (!rewindCtx.hasResponse || !rewindCtx.prevAssistantUuid) {
    tab.notificationService.show(t('chat.fork.unavailableNoResponse'), 'warning');
    return;
  }
...
async function handleForkAll(
  ...
  const msgs = state.messages;
  if (msgs.length === 0) {
    tab.notificationService.show(t('chat.fork.commandNoMessages'), 'warning');
    return;
  }
  ...
  if (!lastAssistantUuid) {
    tab.notificationService.show(t('chat.fork.commandNoAssistantUuid'), 'warning');
    return;
  }
...
export function initializeTabControllers(
  ...
  let cursorFlowFocusTimer: number | null = null;
  const clearCursorFlowFocusTimer = () => {
    if (cursorFlowFocusTimer !== null) {
      window.clearTimeout(cursorFlowFocusTimer);
      cursorFlowFocusTimer = null;
    }
  };
  tab.controllers.selectionController = new SelectionController(
    ...
    (text, source) => {
      ...
      if (!plugin.settings.cursorFlowEnabled || source !== 'document') {
        clearCursorFlowFocusTimer();
        return;
      }
      // Debounce: the selection controller polls every 250ms, so a single
      // mouse drag fires this callback repeatedly as the range grows. Only
      // focus once the selection has been stable for CURSOR_FLOW_FOCUS_DELAY_MS,
      // otherwise focusing mid-drag interrupts the user's in-progress selection.
      clearCursorFlowFocusTimer();
      cursorFlowFocusTimer = window.setTimeout(() => {
        cursorFlowFocusTimer = null;
        const activeElement = dom.inputEl.ownerDocument?.activeElement;
        if (activeElement !== dom.inputEl) {
          dom.inputEl.focus();
        }
      }, CURSOR_FLOW_FOCUS_DELAY_MS);
    },
    dom.messagesEl,
  );
  dom.eventCleanups.push(clearCursorFlowFocusTimer);
  const selectionClearBtn = dom.selectionIndicatorEl!.querySelector<HTMLButtonElement>('.typorai-selection-preview-clear');
  const clearSelectionHandler = (event: MouseEvent) => {
    event.preventDefault();
    event.stopPropagation();
    tab.controllers.selectionController?.clear();
    dom.inputEl.focus();
  };
  selectionClearBtn?.addEventListener('click', clearSelectionHandler);
  if (selectionClearBtn) {
    dom.eventCleanups.push(() => selectionClearBtn.removeEventListener('click', clearSelectionHandler));
  }
  ...
  tab.controllers.conversationController = new ConversationController(
    {
      ...
      ensureServiceForConversation: async (conversation) => {
        ...
        if (tab.service && tab.service.providerId === nextProviderId && conversation) {
          tab.service.syncConversationState(conversation, conversation.externalContextPaths ?? []);
        }
  ...
  tab.controllers.inputController = new InputController({
    ...
    getTyporaDocumentSnapshot: () => tab.documentSnapshot?.() ?? null,
    ...
    ensureServiceInitialized: async () => {
      ...
      try {
        ...
        tab.notificationService.show(error instanceof Error ? error.message : t('chat.input.initFailed'), 'error');
        return false;
      }
    },
    openConversation,
 ...
 */
export function wireTabInputEvents(tab: TabData, plugin: TyporAiPlugin): void {
  ...
  const keydownHandler = (e: KeyboardEvent) => {
    ...
    if (e.key === 'Escape' && !e.isComposing && state.isStreaming) {
      e.preventDefault();
      if (getRegisteredCommandRegistry(tab.commandRegistry, 'conversation.cancel')) {
        void executeRegisteredCommand(tab.commandRegistry, 'conversation.cancel')
          .catch(() => tab.notificationService.show(t('chat.input.cancelFailed'), 'error'));
      } else {
...
export function activateTab(tab: TabData): void {
  tab.dom.contentEl.removeClass('typorai-hidden');
...
export function deactivateTab(tab: TabData): void {
  tab.dom.contentEl.addClass('typorai-hidden');
 ...
 */
export function getTabTitle(tab: TabData, plugin: TyporAiPlugin): string {
...
export function setupServiceCallbacks(tab: TabData, plugin: TyporAiPlugin): void {
  if (tab.service && tab.controllers.inputController) {
    tab.service.setApprovalCallback(
      async (toolName, input, description, options) => {
        if (
          !isDocumentEditingAllowed(getTabPermissionMode(tab, plugin))
          && isDocumentMutationTool(toolName)
        ) {
          tab.notificationService.show(t('inlineEdit.errors.documentEditingBlocked'), 'warning');
          return 'deny';
        }
...
async function renderAutoTriggeredTurn(tab: TabData, result: AutoTurnResult): Promise<void> {
  ...
  if (hasVisibleContent) {
    ...
    const contentEl = msgEl?.querySelector<HTMLElement>('.typorai-message-content');
    if (contentEl) {
  ...
  try {
    ...
    if (hasVisibleContent && !hasVisibleAutoTurnMessageContent(assistantMsg)) {
      const placeholder = t('renderer.subagent.backgroundTaskCompleted');
...
export function updatePlanModeUI(tab: TabData, plugin: TyporAiPlugin, mode: string): void {
  const providerId = getTabProviderId(tab, plugin);
  const snapshot = getTabSettingsSnapshot(tab, plugin);
  const uiConfig = ProviderRegistry.getChatUIConfig(providerId);
  if (uiConfig.applyPermissionMode) {
