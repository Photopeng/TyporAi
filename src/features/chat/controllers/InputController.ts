...
import type { ApprovalDecision, ChatMessage, ChatSelectionRef, ExitPlanModeDecision, StreamChunk } from '../../../core/types';
...
const APPROVAL_DECISION_PAIRS: Array<{ decision: ApprovalDecision; labelKey: 'chat.permission.deny' | 'chat.permission.allowOnce' | 'chat.permission.alwaysAllow' }> = [
  { decision: 'deny', labelKey: 'chat.permission.deny' },
  { decision: 'allow', labelKey: 'chat.permission.allowOnce' },
  { decision: 'allow-always', labelKey: 'chat.permission.alwaysAllow' },
];

function getDefaultApprovalDecisionOptions(): ApprovalDecisionOption[] {
  return APPROVAL_DECISION_PAIRS.map(({ decision, labelKey }) => {
...
export interface InputControllerDeps {
  ...
  getTyporaDocumentSnapshot?: () => TyporaDocumentSnapshot | null;
...
export class InputController {
  ...
  constructor(deps: InputControllerDeps) {
    this.deps = deps;
    this.notifications = deps.notifications ?? new NoticeAdapter();
  }
  ...
  async sendMessage(options?: {
    ...
    // Lazy initialization: validate and prepare the selected provider before
    // consuming input or adding messages to the transcript.
    ...
    const agentService = this.getAgentService();
    if (!agentService) {
      this.notifications.show(t('chat.input.agentNotAvailable'), 'warning');
      return;
    }
    ...
    const userSelection = this.toSelectionRef(turnRequest.editorSelection);
    ...
    streamController.showThinkingIndicator(
      isCompact ? t('chat.input.compacting') : undefined,
      isCompact ? 'typorai-thinking--compact' : undefined,
    );
      ...
      if (conv?.resumeAtMessageId) {
        if (this.isResumeSessionAtStillNeeded(conv.resumeAtMessageId, state.messages.slice(0, -2))) {
          ...
          try {
            ...
            // Best effort: do not block sending.
          }
        }
      }
    }
    ...
    try {
      ...
      if (!wasInvalidated && state.streamGeneration === streamGeneration) {
        ...
        if (didCancelThisTurn && !state.pendingNewSessionPlan) {
          await streamController.appendText(
            `\n\n<span class="typorai-interrupted">${t('renderer.message.interrupted')}</span> `
            + `<span class="typorai-interrupted-hint">${t('renderer.message.interruptedHint')}</span>`,
          );
        }
        ...
        // Clear selection after successful send if there was an editor selection
        if (!wasInterrupted && turnRequest.editorSelection) {
          selectionController.clearAfterSend();
        }
        ...
        if (!didCancelThisTurn && !hasCompactBoundary) {
          ...
          if (durationSeconds > 0) {
            ...
            if (state.currentContentEl) {
              const footerEl = state.currentContentEl.createDiv({ cls: 'typorai-response-footer' });
              footerEl.createSpan({
                text: t('renderer.message.bakedFor', {
                  word: flavorWord,
                  duration: formatDurationMmSs(durationSeconds),
                }),
                cls: 'typorai-baked-duration',
              });
            }
          }
        }
        ...
        // The stream was cancelled before it could process the result; set it manually so
        ...
        if (state.pendingNewSessionPlan && finalAssistantMsg.toolCalls) {
          for (const tc of finalAssistantMsg.toolCalls) {
            if (tc.name === TOOL_EXIT_PLAN_MODE && !tc.result) {
              tc.status = 'completed';
              tc.result = t('chat.input.planApproved');
        ...
        if (planCompleted && !didCancelThisTurn) {
          ...
          if (state.streamGeneration !== streamGeneration || invalidated) {
            ...
            planAutoSendContent = t('chat.input.planImplement');
        ...
        if (!planApprovalInvalidated) {
          ...
          if (planAutoSendContent) {
            ...
            // Keep this inside the invalidation guard; if the tab was closed or
  ...
  updateQueueIndicator(): void {
    ...
    if (visibleQueuedMessage) {
      ...
      const prefix = isPendingSteerOnly ? t('chat.input.steering') : t('chat.input.queuedPrefix');
      indicatorEl.createSpan({
        cls: 'typorai-queue-indicator-text',
        text: `⌙ ${prefix} ${this.getQueuedMessageDisplay(visibleQueuedMessage)}`,
      });

      if (state.queuedMessage) {
        const actionsEl = indicatorEl.createDiv({ cls: 'typorai-queue-indicator-actions' });

        if (this.canSteerQueuedMessage()) {
          const steerButton = actionsEl.createEl('button', {
            cls: 'typorai-queue-indicator-action',
            text: this.steerInFlight ? t('chat.input.steering') : t('chat.input.steerNow'),
          });
      ...
      indicatorEl.addClass('typorai-visible-flex');
      indicatorEl.removeClass('typorai-hidden');
      return;
    }

    indicatorEl.removeClass('typorai-visible-flex');
    indicatorEl.addClass('typorai-hidden');
  }
  ...
  private toSelectionRef(ctx: EditorSelectionContext | null | undefined): ChatSelectionRef | undefined {
  ...
  private buildTurnSubmission(options: {
    ...
    const typoraDocument = !isCompact
      ? getTyporaDocumentContext(this.deps.getTyporaDocumentSnapshot?.() ?? null)
      : undefined;
  ...
  private createQueueIconButton(
    ...
    const button = parentEl.createEl('button', {
      cls: 'typorai-queue-indicator-icon-action',
      attr: {
        'aria-label': label,
        title: label,
        type: 'button',
    ...
    try {
      ...
      this.pendingProviderUserMessages.push({
        ...
        currentNote: preparedTurn.isCompact
          ...
          : preparedTurn.request.currentNotePath,
        images: request.images,
        selection: this.toSelectionRef(preparedTurn.request.editorSelection),
      });
    } catch {
      this.restoreQueuedMessageAfterSteerFailure(queuedMessage);
      this.notifications.show(t('chat.input.steerFailed'), 'error');
    }
  }
  ...
  async handleInstructionSubmit(rawInstruction: string): Promise<void> {
    ...
    try {
        ...
        rawInstruction,
        {
          onAccept: (finalInstruction) => {
            void (async (): Promise<void> => {
              ...
              this.notifications.show(t('chat.input.customSystemPromptAdded'));
          ...
          onClarificationSubmit: async (response) => {
            ...
            if (!result.success) {
              ...
              this.notifications.show(result.error || t('chat.input.processingFailed'), 'error');
              modal?.showError(result.error || t('chat.input.processingFailed'));
              return;
            }
      ...
      if (!result.success) {
        ...
        this.notifications.show(result.error || t('chat.input.refineFailed'), 'error');
        modal.showError(result.error || t('chat.input.refineFailed'));
        instructionModeManager?.clear();
        return;
      }
      ...
      if (result.clarification) {
        ...
        this.notifications.show(t('chat.input.noInstruction'), 'warning');
        modal.showError(t('chat.input.noInstruction'));
        instructionModeManager?.clear();
      }
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : t('common.unknown');
      this.notifications.show(t('chat.input.errorWithMessage', { error: errorMsg }), 'error');
  ...
  async handleApprovalRequest(
    ...
    if (!parentEl) {
      throw new Error(t('chat.input.detachedInput'));
    }

    // Build the header detached; InlineAskUserQuestion will attach it.
    const headerEl = parentEl.createDiv({ cls: 'typorai-ask-approval-info' });
    headerEl.remove();

    const toolEl = headerEl.createDiv({ cls: 'typorai-ask-approval-tool' });
    const iconEl = toolEl.createSpan({ cls: 'typorai-ask-approval-icon' });
    iconEl.setAttribute('aria-hidden', 'true');
    setToolIcon(iconEl, toolName);
    toolEl.createSpan({ text: toolName, cls: 'typorai-ask-approval-tool-name' });

    if (approvalOptions?.decisionReason) {
      headerEl.createDiv({ text: approvalOptions.decisionReason, cls: 'typorai-ask-approval-reason' });
    }
    if (approvalOptions?.blockedPath) {
      headerEl.createDiv({ text: approvalOptions.blockedPath, cls: 'typorai-ask-approval-blocked-path' });
    }
    if (approvalOptions?.agentID) {
      headerEl.createDiv({
        text: t('chat.input.approvalAgent', { agent: approvalOptions.agentID }),
        cls: 'typorai-ask-approval-agent',
      });
    }

    headerEl.createDiv({ text: description, cls: 'typorai-ask-approval-desc' });
    ...
    const input = {
      questions: [{
        question: t('chat.permission.question'),
    ...
    const result = await this.showInlineQuestion(
      ...
      undefined,
      { title: t('chat.permission.title'), headerEl, showCustomInput: false, immediateSelect: true },
    );
    ...
    if (typeof selectedValue !== 'string') {
      this.notifications.show(`Unexpected approval selection: "${String(selectedValue)}"`, 'error');
      return 'cancel';
    }
  ...
  private hideInputContainer(inputContainerEl: HTMLElement): void {
    ...
    inputContainerEl.addClass('typorai-hidden');
  }
  ...
  private restoreInputContainer(inputContainerEl: HTMLElement): void {
    ...
    if (this.inputContainerHideDepth === 0) {
      inputContainerEl.removeClass('typorai-hidden');
    }
  }
  ...
  private resetInputContainerVisibility(): void {
    if (this.inputContainerHideDepth > 0) {
      ...
      this.deps.getInputContainerEl().removeClass('typorai-hidden');
    }
  }
  ...
  private async executeBuiltInCommand(command: BuiltInCommand, args: string): Promise<void> {
    ...
    if (!isBuiltInCommandSupported(command, capabilities)) {
      this.notifications.show(t('chat.input.commandUnsupported', { command: command.name }), 'warning');
      return;
    }
    ...
    switch (command.action) {
      ...
      case 'fork': {
        if (!this.getActiveCapabilities().supportsFork) {
          this.notifications.show(t('chat.forkUnsupported'), 'warning');
          return;
        }
        if (!this.deps.onForkAll) {
          this.notifications.show(t('chat.forkUnavailable'), 'warning');
          return;
        }
      ...
      default: {
        ...
        const unknownAction = typeof (command as { action?: unknown }).action === 'string'
          ...
          : 'unknown';
        this.notifications.show(t('chat.input.unknownCommand', { command: unknownAction }), 'error');
        break;
      }
    }
  }
  ...
  private showResumeDropdown(): void {
    ...
    if (conversations.length === 0) {
      this.notifications.show(t('chat.input.resumeNoConversations'), 'warning');
      return;
    }
      ...
      state.currentConversationId,
      {
        onSelect: (id) => {
          ...
          openConversation(id).catch((err: unknown) => {
            ...
            this.notifications.show(t('chat.input.openConversationFailed', { error: msg }), 'error');
          });
        },
        onDismiss: () => {
          this.destroyResumeDropdown();
...
const MAX_TYPORA_DOCUMENT_CONTEXT_CHARS = 120_000;

export type TyporaDocumentSnapshot = {
  currentFilePath: string | null;
  currentDocument: string;
  selection: string;
};

function getTyporaDocumentContext(snapshot: TyporaDocumentSnapshot | null): ChatTurnRequest['typoraDocument'] | undefined {
  if (!snapshot?.currentDocument.trim()) {
    return undefined;
  }

  const content = snapshot.currentDocument;
  const truncated = content.length > MAX_TYPORA_DOCUMENT_CONTEXT_CHARS;
  return {
    path: snapshot.currentFilePath,
    content: truncated
      ? content.slice(0, MAX_TYPORA_DOCUMENT_CONTEXT_CHARS)
      : content,
    selection: snapshot.selection.trim() || undefined,
    truncated,
  };
}
