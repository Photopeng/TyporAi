...
export class StreamController {
  ...
  private handleRegularToolUse(
    ...
    if (existingToolCall) {
      ...
      if (Object.keys(newInput).length > 0) {
        ...
        if (toolEl) {
          const nameEl = toolEl.querySelector('.typorai-tool-name')
            ?? toolEl.querySelector('.typorai-write-edit-name');
          if (nameEl) {
            nameEl.setText(getToolName(existingToolCall.name, existingToolCall.input));
          }
          const summaryEl = toolEl.querySelector('.typorai-tool-summary')
            ?? toolEl.querySelector('.typorai-write-edit-summary');
          if (summaryEl) {
  ...
  async appendText(text: string): Promise<void> {
    ...
    if (!state.currentTextEl) {
      state.currentTextEl = state.currentContentEl.createDiv({ cls: 'typorai-text-block' });
  ...
  showThinkingIndicator(overrideText?: string, overrideCls?: string): void {
    ...
    state.setThinkingIndicatorTimeout(timerWindow.setTimeout(() => {
      ...
      const timerSpan = state.thinkingEl.createSpan({ cls: 'typorai-thinking-hint' });
      const updateTimer = () => {
  ...
  private renderCompactBoundary(): void {
    ...
    if (!state.currentContentEl) return;
    this.hideThinkingIndicator();
    const el = state.currentContentEl.createDiv({ cls: 'typorai-compact-boundary' });
    el.createSpan({
      cls: 'typorai-compact-boundary-label',
      text: t('renderer.message.conversationCompacted'),
    });
  }
  ...
  /**
   * Nudges Typora's vault after a Write/Edit/NotebookEdit so the file tree
   * refreshes. Direct `fs` writes bypass the TyporaWorkspace API, and macOS + iCloud
  ...
  private notifyVaultFileChange(input: Record<string, unknown>): void {
    ...
    window.setTimeout(() => {
      ...
      if (file && typeof file === 'object' && 'path' in file) {
        ...
        // New file — scan parent directory so Typora discovers it
