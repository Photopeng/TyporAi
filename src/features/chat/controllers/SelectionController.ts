...
const HIGHLIGHT_KEY = 'typorai-selection';
const AI_BODY_CONTENT_SELECTOR = '.typorai-message-assistant .typorai-message-content';
...
export class SelectionController {
  ...
  private onNewSelection: ((text: string, source: 'document' | 'ai-response') => void) | null;
  ...
  constructor(
    ...
    onNewSelection?: (text: string, source: 'document' | 'ai-response') => void,
    messagesEl?: HTMLElement,
  ) {
    this.app = app as SelectionHost;
    this.indicatorEl = indicatorEl;
    this.inputEl = inputEl;
    this.focusScopeEls = this.normalizeFocusScopes(focusScopeEl);
    this.contextRowEl = contextRowEl;
    this.messagesEl = messagesEl ?? null;
    this.onVisibilityChange = onVisibilityChange ?? null;
    this.onNewSelection = onNewSelection ?? null;
  }
  ...
  private poll(): void {
    const candidate = this.app.workspace.getActiveDocumentView?.();
    const view = this.isDocumentView(candidate) ? candidate : null;
    if (!view) {
    ...
    if (selectedText.trim()) {
      ...
      if (!unchanged) {
        ...
        if (selectedText.trim()) {
          this.onNewSelection?.(selectedText, 'document');
        }
      }
    } else {
      // No editor selection - check AI body selection
      if (!this.pollAiBody()) {
  ...
  private pollAiBody(): boolean {
    if (!this.messagesEl) return false;

    const selection = this.getDocumentSelection(this.messagesEl.ownerDocument);
    ...
    // Check if selection is within AI message content
    const isInAiBody = (anchorNode && this.messagesEl.querySelector(AI_BODY_CONTENT_SELECTOR)?.contains(anchorNode))
      || (focusNode && this.messagesEl.querySelector(AI_BODY_CONTENT_SELECTOR)?.contains(focusNode));

    if (!isInAiBody) return false;

    this.inputHandoffGraceUntil = null;
    const notePath = 'AI response';
    const lineCount = selectedText.split(/\r?\n/).length;
    ...
    if (!unchanged) {
      ...
      this.updateIndicator();
      this.onNewSelection?.(selectedText, 'ai-response');
    }

    return true;
  }

  private pollReadingMode(view: TyporaDocumentView): void {
  ...
  private isDocumentView(value: unknown): value is TyporaDocumentView {
    if (!value || typeof value !== 'object') return false;
    const candidate = value as Partial<TyporaDocumentView>;
  ...
  private clearWhenMarkdownContextIsUnavailable(): void {
    ...
    // Check if there's still an AI body selection
    if (this.messagesEl) {
      const selection = this.getDocumentSelection(this.messagesEl.ownerDocument);
      const selectedText = selection?.toString() ?? '';
      if (selectedText.trim()) {
  ...
  private updateIndicator(): void {
    ...
    const previewTextEl = this.indicatorEl.querySelector('.typorai-selection-preview-text');

    if (this.storedSelection) {
      const previewText = this.storedSelection.selectedText.replace(/\s+/g, ' ').trim();
      if (previewTextEl) {
  ...
  captureQuickActionSelection(selectedText: string): void {
