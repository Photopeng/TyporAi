...
import type { TyporaPanelHost } from '@/typora/platform';
import { Notice, TyporaPluginRuntime } from '@/typora/platform';

import { DEFAULT_TYPORAI_SETTINGS } from './app/settings/defaultSettings';
import { SharedStorageService } from './app/storage/SharedStorageService';
import { getProviderNeutralMessagesFromMetadata } from './core/bootstrap/SessionStorage';
import type { SharedAppStorage } from './core/bootstrap/storage';
import { normalizeSingleEnabledCliProvider } from './core/providers/cliProviderSelection';
import {
...
// Typora plugin entry point.
function isTyporAiView(value: unknown): value is TyporAiView {
  return !!value
    && typeof value === 'object'
    && typeof (value as { getTabManager?: unknown }).getTabManager === 'function';
}

export default class TyporAiPlugin extends TyporaPluginRuntime {
  settings!: TyporAiSettings;
  ...
  async onload() {
    ...
    this.registerView(
      VIEW_TYPE_TYPORAI,
      (leaf) => new TyporAiView(leaf, this)
    );

    this.addRibbonIcon('bot', 'Open TyporAi', () => {
    ...
    this.addSettingTab(new TyporAiSettingTab(this.app, this));
  }

  onunload(): void {
    void this.persistOpenTabStates();
    void ProviderWorkspaceRegistry.disposeAll();
  }

  private async persistOpenTabStates(): Promise<void> {
    // Ensures state is saved even if Typora quits without calling onClose()
  ...
  async activateView() {
    ...
    const leaf = workspace.getLeavesOfType(VIEW_TYPE_TYPORAI)[0];

    if (!leaf) {
      const newLeaf = this.getLeafForPlacement(this.settings.chatViewPlacement);
      if (newLeaf) {
        await newLeaf.setViewState({
          type: VIEW_TYPE_TYPORAI,
          active: true,
        });
      }
    }

    // Typora owns a single mounted sidebar; setViewState above activates it in place.
  }

  private getLeafForPlacement(placement: ChatViewPlacement): TyporaPanelHost | null {
    const { workspace } = this.app;
    switch (placement) {
      case 'main-tab':
  ...
  private canCreateNewTab(): boolean {
    const hasTyporAiLeaf = this.app.workspace.getLeavesOfType(VIEW_TYPE_TYPORAI).length > 0;
    const view = this.getView();
    const tabManager = view?.getTabManager();
  ...
  private async ensureViewOpen(): Promise<TyporAiView | null> {
  ...
  async loadSettings() {
    ...
    const { typorai } = await this.storage.initialize();
    this.lastKnownTabManagerState = await this.storage.getTabManagerState();

    this.settings = {
      ...DEFAULT_TYPORAI_SETTINGS,
      ...typorai,
    };
    ...
    const didNormalizeCliProviders = normalizeSingleEnabledCliProvider(this.settings);
    ...
    this.conversations = allMetadata.map(meta => {
      ...
      return {
        ...
        messages: getProviderNeutralMessagesFromMetadata(meta),
    ...
    if (changed || didNormalizeModelVariants || didNormalizeProviderSelection || didNormalizeCliProviders) {
  ...
  async saveSettings() {
    ...
    await this.storage.saveTyporAiSettings(this.settings);
  }
  ...
  getView(): TyporAiView | null {
    const leaves = this.app.workspace.getLeavesOfType(VIEW_TYPE_TYPORAI);
    return leaves.map(leaf => leaf.view).find(isTyporAiView) ?? null;
  }

  getAllViews(): TyporAiView[] {
    const leaves = this.app.workspace.getLeavesOfType(VIEW_TYPE_TYPORAI);
    return leaves.map(leaf => leaf.view).filter(isTyporAiView);
  }

  findConversationAcrossViews(conversationId: string): { view: TyporAiView; tabId: string } | null {
    for (const view of this.getAllViews()) {
      const tabManager = view.getTabManager();
      if (!tabManager) continue;
