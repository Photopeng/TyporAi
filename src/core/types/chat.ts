...
/** View type identifier for Typora. */
export const VIEW_TYPE_TYPORAI = 'typorai-view';
...
/** Selection reference captured when a user message is sent with editor text selected. */
export interface ChatSelectionRef {
  /** Full selected text (untruncated). */
  text: string;
...
export interface ChatMessage {
  ...
  /** Editor selection reference rendered above the bubble; only set when sent with text selected. */
...
export interface SessionMetadata {
  ...
  enabledMcpServers?: string[];
  usage?: UsageInfo;
  /** Provider-neutral visible messages for providers without native transcript storage. */
  messages?: ChatMessage[];
  /** Assistant checkpoint identifier for resumeAtMessageId after rewind. */
  resumeAtMessageId?: string;
}
