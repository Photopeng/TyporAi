import type { ProviderDisplayKey } from '../../i18n/types';
import type TyporAiPlugin from '../../main';
import type { CursorContext } from '../../utils/editor';
import type { SharedAppStorage } from '../bootstrap/storage';
import type { McpServerManager } from '../mcp/McpServerManager';
import type { EnvironmentService, FileBackupService, FileProbe, FileStore, HomeFileStore, HostServices, NotificationService, PathService, PlatformInfo, ProcessTransportFactory } from '../ports';
...
export const DEFAULT_CHAT_PROVIDER_ID = 'typora' as const satisfies ProviderId;

export interface CreateChatRuntimeOptions {
  plugin: TyporAiPlugin;
  providerId?: ProviderId;
  /**
   * Host-owned process boundary used by runtimes that launch provider CLIs.
   * It is optional while Typora's legacy host remains on the direct-spawn
   * compatibility path.
   */
  ...
  /**
   * Optional notifier fired whenever the runtime learns new external context
   * paths (e.g. via `applyDynamicUpdates`). The owning tab state mirrors the
   * paths so the file context manager stays in sync across tab switches.
   */
  onExternalContextPathsChanged?: (paths: string[]) => void;
}

/** Optional host capabilities for provider-owned, non-chat services. */
...
export interface ProviderRegistration {
  /** Translation key resolved via `t()` to render the provider's display name. */
  displayNameKey: ProviderDisplayKey;
...
// ---------------------------------------------------------------------------
// TyporaHostApp-level service interfaces
// ---------------------------------------------------------------------------
...
export interface ProviderWorkspaceServices {
  ...
  /** Optional late binding for hosts that become available after plugin bootstrap. */
  configureHostServices?(host: HostServices): void;
  /** Releases provider-owned bootstrap resources before the workspace registry is cleared. */
...
export interface ProviderWorkspaceInitContext {
  ...
  /** @deprecated Use workspaceFileAdapter. Kept for gradual provider migration. */
...
/** Host-bound factory for provider services used outside a chat runtime. */
export interface ProviderServiceFactory {
