...
export class ProviderSettingsCoordinator {
  ...
  static projectProviderState(
    ...
    } else if (isAdaptive && settings.effortLevel === undefined) {
      // Only fall back to the provider default when the user hasn't
      // already supplied a value (e.g. via `applyModelDefaults` which
      // normalizes a user-set valid effort level for the new model).
