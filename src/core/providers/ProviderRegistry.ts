...
import {
  ...
  type CreateProviderServiceOptions,
...
export class ProviderRegistry {
  ...
  static createTitleGenerationService(
    ...
    if (!providerId) {
      return new RoutedTitleGenerationService(plugin, options);
    }
    return this.getProviderRegistration(providerId).createTitleGenerationService(plugin, options);
  }
  ...
  static createInstructionRefineService(
    plugin: TyporAiPlugin,
    providerId: ProviderId = DEFAULT_CHAT_PROVIDER_ID,
    options?: CreateProviderServiceOptions,
  ): InstructionRefineService {
    return this.getProviderRegistration(providerId).createInstructionRefineService(plugin, options);
  }

  static createInlineEditService(
    plugin: TyporAiPlugin,
    providerId: ProviderId = DEFAULT_CHAT_PROVIDER_ID,
    options?: CreateProviderServiceOptions,
  ): InlineEditService {
    return this.getProviderRegistration(providerId).createInlineEditService(plugin, options);
  }
  ...
  static hasProvider(providerId: ProviderId | null | undefined): providerId is ProviderId {
    return !!providerId && !!this.registrations[providerId];
  }
  ...
  static getProviderDisplayName(providerId: ProviderId): string {
    return t(this.getProviderRegistration(providerId).displayNameKey);
  }
...
class RoutedTitleGenerationService implements TitleGenerationService {
  ...
  constructor(
    ...
    private readonly options?: CreateProviderServiceOptions,
  ) {}
  ...
  async generateTitle(
    ...
    const service = ProviderRegistry.createTitleGenerationService(this.plugin, providerId, this.options);
