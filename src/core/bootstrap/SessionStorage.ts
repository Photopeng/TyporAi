...
export class SessionStorage {
  ...
  toSessionMetadata(conversation: Conversation): SessionMetadata {
    ...
    const supportsNativeHistory = ProviderRegistry
      .getCapabilities(conversation.providerId)
    ...
    return {
      ...
      ...(supportsNativeHistory ? {} : { messages: conversation.messages }),
...
export function getProviderNeutralMessagesFromMetadata(
  metadata: SessionMetadata,
): ChatMessage[] {
  const providerId = metadata.providerId ?? DEFAULT_CHAT_PROVIDER_ID;
  const supportsNativeHistory = ProviderRegistry
    .getCapabilities(providerId)
    .supportsNativeHistory;

  return supportsNativeHistory ? [] : metadata.messages ?? [];
}
