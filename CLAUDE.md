...
|-------|---------|---------|
| **app** | Shared defaults and plugin-level storage helpers | `defaultSettings`, `TyporAiSettingsStorage`, `SharedStorageService` |
...
| `.typorai/typorai-settings.json` | Shared TyporAi app settings plus provider-specific configuration |
| `.claude/mcp.json` | TyporAi-managed MCP servers for Claude |
| `.claude/commands/**/*.md` | Claude slash commands |
| `.claude/skills/*/SKILL.md` | Claude skills |
| `.claude/agents/*.md` | Claude vault agents |
| `.typorai/sessions/*.meta.json` | Provider-neutral session metadata |
